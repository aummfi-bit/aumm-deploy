 ...
 * @notice Five-asset Standard pool — JPMon 16% + GHO 26% + GSon 16% + ixEDEL 16% + svZCHF 26%.
 *         Sector: Banking / Financials.
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 22) — "ixMoneta".
 *
 *      Composition lock: N-D0 build scope on the M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / GHO 26% / ixEDEL 16% / JPMon 16% / GSon 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      GHO binds the Aave GHO ERC-4626 vault `0xC71Ea051…` + RP `0x851b73…` (not bare GHO ERC-20 `0x40D16F…`; per E11 / 07a GHO row, mirrors ixAurebit slot 14).
 ...
 *      Quality Gate: two ERC-4626 (`TokenType.WITH_RATE`) yield cores — svZCHF 26 + GHO 26 — sum to
 ...
 *      JPMon (`0x03C1…`), GHO (`0xC71E…`), GSon (`0xdB57…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`). No runtime sort.
 */
library IxMonetaConfig {
    ...
    function config() internal pure returns (PoolConfig memory) {
        ...
        tokenTypes[1] = TokenType.WITH_RATE;    // GHO
        ...
        rateProviders[1] = IRateProvider(GHO_RATE_PROVIDER);
        ...
        paysYieldFees[1] = true;    // GHO (WITH_RATE + RP)
        ...
        normalizedWeights[1] = 0.26e18;  // GHO 26%
