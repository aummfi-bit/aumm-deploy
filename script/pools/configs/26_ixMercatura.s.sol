...
/**
 * @title IxMercaturaConfig
 * @notice Five-asset Standard pool — HOOD 16% + sUSDS 26% + ixEDEL 16% + svZCHF 26% + COIN 16%.
 *         Sector: Fintech / Brokers.
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 26) — "ixMercatura".
 *
 *      Composition lock: N-D0 build scope on the M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / sUSDS 26% / ixEDEL 16% / COIN 16% / HOOD 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      Name / symbol: E-D18 (lower-camel `ixMercatura` / uppercase `IXMERCATURA`).
 *      Sector label: E-D19 ("Fintech / Brokers" per manifest sector column).
 *      Salt: E-D20 (slot-derived `bytes32(uint256(26))`).
 ...
 *      a 52% numerator (0.52e18); HOOD, ixEDEL, COIN are the three `TokenType.STANDARD` tokens at 16% each.
 ...
 *      HOOD (`0x998f…`), sUSDS (`0xa393…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`), COIN (`0xF042…`). No runtime sort.
 */
library IxMercaturaConfig {
    address internal constant HOOD            = 0x998f02A9E343EF6E3E6f28700d5A20F839fD74E6;
    ...
    address internal constant COIN            = 0xF042cfa86cf1D598a75Bdb55c3507a1F39f9493b;
