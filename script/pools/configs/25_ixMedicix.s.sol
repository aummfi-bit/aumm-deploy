...
/**
 * @title IxMedicixConfig
 * @notice Five-asset Standard pool — ABBVon 16% + sUSDS 26% + JNJon 16% + ixEDEL 16% + svZCHF 26%.
 *         Sector: Healthcare.
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 25) — "ixMedicix".
 *
 *      Composition lock: N-D0 build scope on the M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / sUSDS 26% / ixEDEL 16% / JNJon 16% / ABBVon 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      Name / symbol: E-D18 (lower-camel `ixMedicix` / uppercase `IXMEDICIX`).
 *      Sector label: E-D19 ("Healthcare" per manifest sector column).
 *      Salt: E-D20 (slot-derived `bytes32(uint256(25))`).
 ...
 *      a 52% numerator (0.52e18); ABBVon, JNJon, ixEDEL are the three `TokenType.STANDARD` tokens at 16% each.
 ...
 *      ABBVon (`0x7c73…`), sUSDS (`0xa393…`), JNJon (`0xdd0E…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`). No runtime sort.
 */
library IxMedicixConfig {
    address internal constant ABBVON          = 0x7c7378143a9c8839e0502e2178F058F46c6ea504;
    address internal constant SUSDS           = 0xa3931d71877C0E7a3148CB7Eb4463524FEc27fbD;
    address internal constant JNJON           = 0xdd0E1e6162666a210905fFE8d368661B313c00e9;
    ...
    function config() internal pure returns (PoolConfig memory) {
        ...
        tokenTypes[0] = TokenType.STANDARD;     // ABBVon
        tokenTypes[1] = TokenType.WITH_RATE;    // sUSDS
        tokenTypes[2] = TokenType.STANDARD;     // JNJon
        ...
        paysYieldFees[0] = false;   // ABBVon (STANDARD)
        paysYieldFees[1] = true;    // sUSDS (WITH_RATE + RP)
        paysYieldFees[2] = false;   // JNJon (STANDARD)
        ...
        normalizedWeights[0] = 0.16e18;  // ABBVon 16%
        normalizedWeights[1] = 0.26e18;  // sUSDS 26%
        normalizedWeights[2] = 0.16e18;  // JNJon 16%
