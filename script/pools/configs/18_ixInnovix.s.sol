...
/**
 * @title IxInnovixConfig
 * @notice Five-asset Standard pool — QQQon 16% + QQQX 16% + waEthUSDC 26% + ixEDEL 16% + svZCHF 26%.
 *         Sector: US Equities (Tech Index).
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 18) — "ixInnovix".
 *
 *      Composition lock: N-D0 build scope on the M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / waEthUSDC 26% / ixEDEL 16% / QQQon 16% / QQQX 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      Name / symbol: E-D18 (lower-camel `ixInnovix` / uppercase `IXINNOVIX`).
 *      Sector label: E-D19 ("US Equities (Tech Index)" per manifest sector column).
 *      Salt: E-D20 (slot-derived `bytes32(uint256(18))`).
 ...
 *      a 52% numerator (0.52e18); QQQon, QQQX, ixEDEL are the three `TokenType.STANDARD` tokens at 16% each.
 ...
 *      QQQon (`0x0e39…`), QQQX (`0xa753…`), waEthUSDC (`0xD4fa…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`). No runtime sort.
 */
library IxInnovixConfig {
    address internal constant QQQON           = 0x0e397938C1Aa0680954093495B70A9F5e2249aBa;
    ...
    function config() internal pure returns (PoolConfig memory) {
        ...
        tokenTypes[0] = TokenType.STANDARD;     // QQQon
        tokenTypes[1] = TokenType.STANDARD;     // QQQX
        ...
        paysYieldFees[0] = false;   // QQQon (STANDARD)
        paysYieldFees[1] = false;   // QQQX (STANDARD)
        ...
        normalizedWeights[0] = 0.16e18;  // QQQon 16%
        normalizedWeights[1] = 0.16e18;  // QQQX 16%
        ...
        return PoolConfig({
            ...
            symbol: "IXINNOVIX",
            slot: 18,
            sectorLabel: "US Equities (Tech Index)",
            tokens: tokens,
