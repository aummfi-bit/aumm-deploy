...
/**
 * @title IxMedioxConfig
 * @notice Five-asset Standard pool — TIPon 16% + waEthUSDC 26% + ixEDEL 16% + svZCHF 26% + AGGon 16%.
 *         Sector: US Fixed Income (aggregate + TIPS).
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 10) — "ixMediox".
 *
 *      Composition lock: M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / waEthUSDC 26% / ixEDEL 16% / AGGon 16% / TIPon 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      Name / symbol: E-D18 (lower-camel `ixMediox` / uppercase `IXMEDIOX`).
 *      Sector label: E-D19 ("US Fixed Income (aggregate + TIPS)" per manifest sector column).
 *      Salt: E-D20 (slot-derived `bytes32(uint256(10))`).
 ...
 *      a 52% numerator (0.52e18); ixEDEL, AGGon, TIPon are the three `TokenType.STANDARD` tokens at 16% each.
 ...
 *      TIPon (`0x2Df3…`), waEthUSDC (`0xD4fa…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`), AGGon (`0xfF7C…`). No runtime sort.
 */
library IxMedioxConfig {
    address internal constant TIPON         = 0x2Df38cA485D01fC15e4FD85847ed26b7EF871c1c;
    ...
    address internal constant AGGON          = 0xfF7CF16aA2fFc463b996DB2f7B7cf0130336899D;
