...
/**
 * @title IxRegistrumConfig
 * @notice Five-asset Standard pool — OPEN 16% + sUSDS 26% + ixEDEL 16% + svZCHF 26% + ETHPLUS 16%.
 *         Sector: DeFi Ecosystem.
 *
 * @dev Manifest row: `aumm-site/06_miliarium_manifest.md` (slot 15) — "ixRegistrum".
 *
 *      Composition lock: N-D0 build scope on the M-D5 Standard 26/26/16/16/16 template (svZCHF 26% / sUSDS 26% / ixEDEL 16% / ETHPLUS 16% / OPEN 16%).
 *      Token addresses + Rate Providers: `aumm-site/07a_tokens.md` (E-D17 verified mainnet literals).
 *      Name / symbol: E-D18 (lower-camel `ixRegistrum` / uppercase `IXREGISTRUM`).
 *      Sector label: E-D19 ("DeFi Ecosystem" per manifest sector column).
 *      Salt: E-D20 (slot-derived `bytes32(uint256(15))`).
 ...
 *      a 52% numerator (0.52e18); OPEN, ixEDEL, ETHPLUS are the three `TokenType.STANDARD` tokens at 16% each.
 ...
 *      OPEN (`0x323c…`), sUSDS (`0xa393…`), ixEDEL (`0xe4a1…`), svZCHF (`0xE5F1…`), ETHPLUS (`0xE72B…`). No runtime sort.
 */
library IxRegistrumConfig {
    address internal constant OPEN            = 0x323c03c48660fE31186fa82c289b0766d331Ce21;
