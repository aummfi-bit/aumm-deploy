...
contract GaugeEligibility is IGaugeEligibility {
     ...
     * @dev Caller-gated by **G-D22** via `onlyGaugeRegistry` (the wired GaugeRegistry binding from G2.4-post's F-D23 one-shot setter). Per-pool zero-handling precedence per **G-D23 (v)** as REORDERED by **PP-D56 (viii)**: `numeratorSma == 0` skips the pool (E.7a — a zero numerator yields a zero ratio for every pool alike, collapsing the sort onto its address tiebreak and assigning cap tiers by ADDRESS) → `denominatorSma == 0` skips the pool (excluded from ranking, no cap, no revert per **P-D15 (3)**) → cold-start grace (first sighting WITH USABLE DATA writes `firstTournamentEpoch[pool] = newEpoch`, pool is skipped without revert) → warmup window (`newEpoch - firstTournamentEpoch[pool] < SMOOTHING_EPOCHS`, pool is skipped without revert) → ratio compute `(numeratorSma * 1e18) / denominatorSma` (1e18 fixed-point, dimensionless per F-10 price-agnostic). Both zero-input skips precede the cold `SSTORE`, so `firstTournamentEpoch` means the first epoch in which the pool had usable data rather than the first epoch it was seen, and no skipped pool pays that store; once stamped it persists, so a pool that later loses its feed is skipped while keeping its grace epoch and re-ranks without re-warming. Survivors sorted descending by `efficiencyRatio` via in-memory insertion sort with address-ascending tiebreak per **G-D23 (iv)** / **T-T3** stable-sort determinism — no storage write during sort. Cutoff `favoredCount = (nRanked * 15 + 99) / 100` per **G-D3** ceiling arithmetic (favored top cohort); per-pool emission caps assigned to `poolEmissionCapBps` by floor-percentile tiers per **P-D13 (3)** / **P-D15 (1)** — 10 bps (bottom 5%), 50 bps (bottom 10–5%), 100 bps (bottom 15–10%), 0 (top 85%), via `cap{10,50,100}Count = floor(nRanked × {5,10,15} / 100)`. Transition events fire exactly once per pool per epoch per **G-D5** with reshaped ABI per **G-D23 (iii)**: top-to-bottom → `GaugeEfficiencyDropped` (**T-T2**); bottom-to-top → `GaugeEfficiencyRising` (**T-T1**). Per-pool emission caps ARE assigned here to `poolEmissionCapBps` per **P-D13 (3)** / **P-D15** — the F-16 fix supersedes the **G-D15b** Stage-H deferral; the EmissionDistributor consumes them via `IGaugeRegistry` delegation (F16e / F16f). Caller (GaugeRegistry) responsible for passing a deduped `eligiblePools` set.
