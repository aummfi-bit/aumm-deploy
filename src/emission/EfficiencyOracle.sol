...
contract EfficiencyOracle is IEfficiencyOracle {
     ...
     * @param _aumm AuMM token address — passed as the `token` argument to `tvlOracle.quoteSvZCHF` in `recordEmissions` per H-D10 v2. Reverts `ZeroAddress` on zero input. Deploy prerequisite: `tvlOracle.tokenToUnderlying[_aumm]` must be seeded with a constellation venue pricing AuMM in svZCHF, or `recordEmissions` accumulates 0 and `efficiencyInputs` returns `denominatorSma = 0`, which per **PP-D56 (viii)** skips the pool in `GaugeEligibility`'s tournament ahead of the cold-start stamp rather than after the warmup window.
     ...
     * @dev Two-tier retrieval per epoch target: tier 1 — if `_accEpoch[pool] == targetEpoch`, sum the live `_accNumerator[pool]` and `_accDenominator[pool]` (not yet flushed by `_ensureCurrentEpoch`); else tier 2 — read `_history[pool][targetEpoch % 3]` and contribute only when `epoch` matches `targetEpoch` (stale slots contribute 0; no zero-fill loop). Totals divide by `3` always (`sumNumerator / 3`, `sumDenominator / 3`), so dormant-epoch zeros attenuate the SMA. When `denominatorSma == 0` cold path, callers such as `GaugeEligibility` skip the pool per **P-D15 (3)** rather than reverting; this view never reverts.
