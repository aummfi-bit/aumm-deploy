...
contract BodenseeBootstrapChannel is IBodenseeBootstrapChannel {
     ...
     * @dev H-D11 mint-at-distribute lock — mint timing belongs in `distribute()`, never in `accrue()`. H-D14 `onlyGovernance` gate — mint authority is the scarce resource; permissionless `distribute()` would let any caller force-flush at a self-chosen price window. K-D7 wiring — channel mints via `mintRouter.mintFor(address(this), amount)`, reverting `MintRouterNotSet` until the one-shot `setMintRouter` binds the `AuMMMinterRouter`; the router holds AuMM's minter slot via `aumm.setMinter(router)` at K7, superseding the H-D7 `IAuMM.setMinter(address(this))` posture. 3-transient-slot reentrancy + payload pattern mirrors G-D11 with `_PENDING_PAY_TOKEN_SLOT` omitted (AuMM is immutable pay token). Structural template is `src/gauge/SwapAndDepositToBodensee.sol` L331—L360 (`swapAndDeposit`) with substitutions — no `payToken` slot write, `mintRouter.mintFor(address(this), amount)` (guarded by `MintRouterNotSet`) precedes the reentrancy check, `IERC20(address(AuMM)).balanceOf(address(this))` for the residual read (IAuMM lacks the `using SafeERC20 for IERC20` binding so the cast is required), residual check uses the already-declared `HelperBalanceNonZero(residual)` error. The residual check is a delta against a pre-mint snapshot rather than an absolute zero, so a stray AuMM transfer — or any LP's `claim(pool, channel)` — no longer bricks every future `distribute()` permanently.
     */
     ...
     * @dev G-D18 — reads `balancesRaw[idx]` from `_vault.getPoolTokenInfo(BODENSEE_POOL)`. Mirrors `src/gauge/SwapAndDepositToBodensee.sol` L461—L464 `_currentReserve`.
