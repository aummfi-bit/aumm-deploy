...
contract EmissionDistributor is IEmissionDistributor {
    ...
    /// @notice AuMM token — consumed for `blockEmissionRate(block_)` reads in `_lpTrancheEmission` per H-D21. Per K-D7, `claim` no longer calls `AuMM.mint` directly — mints route through `mintRouter.mintFor` (the router holds AuMM's C-D11 minter slot).
    ...
    /// @notice Running sum of raw F-5 scores across all gauged pools per H-D31 — mutated through the H-D19 signed-delta middleware at H5.3e; consumed by the H-D33 Miliarium reshape leg (`effective_Mil = (1e18 - alpha).mulDown(f5Total / _scoredMiliariumCount) + alpha.mulDown(score_F5_new)`, the divisor being the scored-Miliarium count per PP-D57 (xi)).
    ...
    /// @notice Per-pool block cursor for the Stage L Incendiary boost-delivery leg per L-D25 — the highest `block.number` through which purchased boost (`IIncendiaryRegistry.boostIntegral`) has been delivered into `poolAccRewardPerLP[pool]`. Concrete-only slot (no `override`, not declared in `IEmissionDistributor`), mirroring the `incendiaryRegistry` H-D29 precedent — no interface edit, no G16 inheritor sweep. Defaults to 0 (cold-start ⇒ the first settle reads `boostIntegral(pool, 1, block.number)`); advanced to `block.number` at the tail of `_settlePool` whenever `incendiaryRegistry != address(0)`.
    mapping(address => uint256) public poolBoostCursor;
    ...
    /* ---------- Scored-Miliarium count (PP-D57 (xi) / D.8) ---------- */
    /// @notice The number of Miliarium pools whose score currently sits in `totalScore` — the divisor of the H-D33 equal leg per PP-D57 (xi), replacing the fixed 28. `recordScore` moves it one step when a pool's Miliarium membership differs from its `_countedAsMiliarium` flag, and `deregisterScore` decrements it for a counted pool, so it is at least one whenever the Miliarium branch reads it.
    uint256 internal _scoredMiliariumCount;
     ...
     * @dev `onlyGovernance`-gated; reverts `ZeroAddress` on zero input per H-D14 (non-zero recipient is the load-bearing handoff invariant — mirrors the K-D7 `setMintRouter` ZeroAddress guard, which likewise cannot bind mint authority to address(0)); captures `oldGovernance` before overwrite; emits `GovernanceTransferred(oldGovernance, newGovernance)`. Mirrors TVLOracle / EfficiencyOracle / BodenseeBootstrapChannel L159-L164 `setGovernanceContract` precedent.
     ...
     * @dev H-D26 production accrual path — supersedes H-D21's H4 single-block-sample × Δblocks pattern (H-D21 LOCKED for H4 historical posture). Lazy accrual tick invoked at every mutating external entry (`recordScore` at H4.5e, `recordDeposit` / `recordWithdrawal` at H4.6, `claim` at H4.7) BEFORE any state mutation, ensuring `accRewardPerScoreUnit` is fresh against the accrual cursor, which equals `block.number` except while a backlog drains per PP-D47. Two short-circuits in strict order — (1) empty-interval per H-D21 as WIDENED by PP-D47: `block.number <= lastAccrualBlock` returns immediately, no integral read, no write, the non-strict comparison making the cursor monotone so a pre-genesis touch against the PB-D19 future-dated `GENESIS_BLOCK` anchor cannot drag it backwards; (2) empty-`totalScore` per H-D15 as FORKED by PP-D47 (E.6): `totalScore == 0` still prevents the divide-by-zero on the divDown denominator, but now advances `lastAccrualBlock = block.number` ONLY at cold start, detected by `accRewardPerScoreUnit == 0` — no accrual has ever completed against live scores, so the window is the post-genesis maturity gap in which no pool was ever eligible and it is forfeited as unearned — while after any completed accrual the cursor is HELD, leaving the window PENDING for the next accrual with live scores rather than forfeiting it, which is E.6's defect. Otherwise reads `contribution = _lpTrancheIntegral(last + 1, to)` once per call per H-D26, where `to` is `block.number` CLAMPED to `last + MAX_ACCRUAL_SPAN_BLOCKS` (the integral interval is inclusive on both ends — `[last + 1, to]` covers every block strictly after the previous accrual through the released end, so successive accruals tile the block timeline exactly once each, no double-counting and no gaps, the clamp DEFERRING blocks rather than skipping them); the integral internally splits at era boundaries per H-D30 and phase boundaries per H-D27 / H-D28 / H-D29, snapshotting a constant `rate = AuMM.blockEmissionRate(sub_from)` per era sub-interval, so the conservation invariant `LP_integral + Σ Bodensee_apsum + Σ Incendiary_integral = Σ blockEmissionRate × n_sub` holds across the interval per H-D26. Applies FixedPoint divDown: `accRewardPerScoreUnit += contribution × 1e18 / totalScore` (the `.divDown(uint256)` binding from L22's `using FixedPoint for uint256;`); FixedPoint scale convention keeps `accRewardPerScoreUnit` in AuMM_wei-per-score-unit (FixedPoint 18-decimal), matching the `(accRewardPerScoreUnit - poolAccDebt[pool]).mulDown(poolScore[pool])` allocation formula at `_settlePool` per H-D15 / H-D23. `lastAccrualBlock` writes the clamped end AFTER the accumulator math, so a subsequent call in the same block short-circuits at the empty-interval guard when the interval was unclamped and drains the next chunk when it was not. Local caches `last` (lastAccrualBlock SLOAD) and `total` (totalScore SLOAD) eliminate redundant storage reads in the success path. `_lpTrancheEmission(block_)` at L196-L198 is no longer read in this production path per H-D26 — RETAINED as the single-block external view for tests / UI per H-D26 retention note.
     */
    function _accrueGlobal() internal {
        ...
        if (total == 0) {
            if (accRewardPerScoreUnit == 0) lastAccrualBlock = block.number;
            return;
        }
     ...
     * @dev H-D19 totalScore signed-delta middleware + F12 Stage F type-discipline pattern at `src/ccb/CCBMultiplier.sol` L269 + L273 — extracts the inlined CCBMultiplier pattern into reusable form for `recordScore` (H4.5e). `base.toInt256()` reverts on `base > type(int256).max` (impossible for FixedPoint scores at 18-decimal scale under any plausible totalScore). `.toUint256()` reverts on the final int256 being negative (the F12 clamp invariant — recordScore's signed-delta middleware preserves `totalScore >= 0` because the only call site computes `delta = newScore - oldScore` against `totalScore`, where oldScore is a prior write to `poolScore[pool]` summed into `totalScore`, so subtracting it cannot drive the aggregate below zero in well-formed sequences).
    ...
    function recordScore(address pool) external override {
        ...
        // PP-D57 (xi): reconcile the pool's place in the scored-Miliarium count before the equal leg divides by it.
        bool poolIsMiliarium = _miliariumRegistry.isMiliarium(pool);
        if (poolIsMiliarium != _countedAsMiliarium[pool]) {
            _countedAsMiliarium[pool] = poolIsMiliarium;
            if (poolIsMiliarium) ++_scoredMiliariumCount;
            else --_scoredMiliariumCount;
        }
        if (poolIsMiliarium) {
            effective_new = (1e18 - alpha).mulDown(f5Total / _scoredMiliariumCount) + alpha.mulDown(score_F5_new);
        } else {
    ...
    function deregisterScore(address pool) external override {
        ...
        poolScore[pool] = 0;
        if (_countedAsMiliarium[pool]) {
            _countedAsMiliarium[pool] = false;
            --_scoredMiliariumCount;
        }
    ...
    /// @notice Ratchets `user`'s recorded stake in `pool` down to a reference live-BPT balance, forfeiting the
    ///         withdrawn delta's pending (P-D18 forfeit-delta — "you move you lose"). No-op when the recorded
    ///         stake does not exceed `referenceBalance`.
    /// @dev Settles first (so the phantom delta's pre-sync share is distributed then stranded per H-D24), then
    ///      drops `userLP` and `poolTotalLP` in lockstep by the same delta and resets the §viii clock. Reducing
    ///      `userLP` WITHOUT touching `userRewardDebt` forfeits the delta's pending by construction: the live
    ///      form `(acc - userRewardDebt).mulDown(userLP)` simply drops by `(acc - userRewardDebt).mulDown(delta)`,
    ///      never crystallized into `pendingBalance`. Downward-only — the OQ-25 upward leg stays delta-based in
    ///      `recordDeposit`, so market / transferred BPT confers nothing.
    function _syncDown(address pool, address user, uint256 referenceBalance) internal {
        uint256 recorded = userLP[pool][user];
        if (recorded <= referenceBalance) return;
        _accrueGlobal();
        _settlePool(pool);
    ...
    /// @dev F-17 / P-D18 — anyone may sync any holder so honest LPs / keepers de-dilute the emission denominator
    ///      against a phantom position left by an out-of-band BPT move (a plain transfer or a non-trusted-router
    ///      removal that never reached the recorder). Forfeit-delta per `_syncDown`.
    function syncPosition(address pool, address holder) external override {
        _syncDown(pool, holder, IERC20(pool).balanceOf(holder));
    }
