...
**Last update:** 2026-05-14, **Stage G COMPLETE** at `stage-g-complete` (commit `384a40b`, 2026-05-14). Stage G delivered the **gauge state machine + auto-gauge eligibility** (auto-gauge pivot per `docs/STAGE_G_PRECHECK_AUTO_GAUGE.md`) — four contracts under `src/gauge/` plus extended-in-place `src/ccb/IGaugeRegistry.sol` (1 → 8 functions per **G-D16a**, original `isGaugeApproved` ABI preserved for Stage F callers) and new sibling `IEfficiencyOracle.sol` per **G-D23 (ii)**: **`VaultClassRegistry.sol`** (Frankencoin-inspired proposal-and-veto admission of ERC-4626 vault classes counted toward the 52% Quality Gate numerator; three admission fingerprints `ImplementationAddress` / `FactoryAddress` / `BytecodeHash` per **G-D9**; constructor-injected genesis arrays per **G-D20**; 201_600-block veto window per **G-D19**; bond routes via `SwapAndDepositToBodensee.donate`); **`SwapAndDepositToBodensee.sol`** (OQ-G3 shared swap-and-one-sided-deposit primitive per **G-D11** + **G-D12** + **G-D21** — strict-equality `swapAndDeposit` for activation fees (100 svZCHF or 125 sUSDS per **G-D12**) + variable-amount `donate` for governance-allowlisted bonds and future emission-router donations; reserve-delta defensive invariant via Balancer V3 `calculateSlot` + `tload` / `tstore` transient-storage helpers per **G-D14** + **G-D18**); **`GaugeEligibility.sol`** (epoch-boundary evaluator: 52% numerator gate per **G-D8** + OQ-G2 $10K svZCHF TVL floor + F-10 cohort assignment via `computeEpochSnapshot` per **G-D3** + **G-D23** with **T-T3** insertion-sort address-ascending tiebreak + factory whitelist per **G-D6** / **G-D15a** + forbidden-token block per **T-I3**; `onlyGaugeRegistry` one-shot-setter gate per **G-D22** + **F-D23**; reshaped transition events `GaugeEfficiencyDropped` / `GaugeEfficiencyRising` carrying `(numeratorSma, denominatorSma, efficiencyRatio)` per **G-D23 (iii)**); **`GaugeRegistry.sol`** (three activation paths per **G-D1** + **G-D7** — permissionless `activateGauge` with non-reverting `GaugeActivationFailed` per **G-D16c**, governance `registerGaugeFromComposition`, governance `seedFoundingPool` / `seedFoundingPools` per **STAGE_G_PRECHECK_AUTO_GAUGE** C-4; terminal `revokeGauge` per **G-D17**; Stage K handoff via `setGovernanceContract`; 90-day boost deprecated at Stage G per **G-D13**, M_i reset at activation handled in Stage F `CCBMultiplier`). Test suite: `test/unit/VaultClassRegistry.t.sol` 37/37 + `test/unit/GaugeEligibility.t.sol` 46/46 + `test/unit/GaugeRegistry.t.sol` happy / revert / governance / view + `test/fork/StageGIntegration.t.sol` (G4.1–G4.4e + `StageGRegistryStageFRegressionTest`) — all green via D35 split-form (`--threads 1` fork per **D36** / **F-D11**); Stage F **F-D17** boost-gate regression intact via the F-D23 one-shot seal to the real `GaugeRegistry`. OQ resolutions — **OQ-G1** (F-10 sibling-oracle encoding; `numeratorSma + denominatorSma + efficiencyRatio` event ABI per **G-D23**), **OQ-G2** ($10K svZCHF TVL floor + 7-day SMA cadence), **OQ-G3** (100 svZCHF / 125 sUSDS anti-spam fee + Bodensee donation routing per **G-D12** strict-equality), **OQ-G4** (VaultClassRegistry Frankencoin-style admission per **G-D9** + **G-D20**). Stage findings — **G10** (compile-probe typed-domain syntax before §12 NOTES lock — `transient` keyword unavailable in solc 0.8.26, Balancer assembly-helper pattern locked instead), **G11** (`calculateSlot` not constant-expression-eligible — `immutable` required, not `constant`), **G12** (`using StorageSlotExtension for *;` required to chain typed-slot access; `for bytes32;` fails the `tload` / `tstore` leg), **G13** (`vm.expectRevert(bytes4)` strict-match on full revert data + in-fork synthetic FoT via `vm.mockCall` frozen-pre-state pattern), **G14** (solc 0.8.26 rejects `calldata` for constructor parameters — `memory` is the only valid binding location), **G15** (scaffold NatSpec interface-promise drift — close-of-family §8e.1 Must match must enumerate scaffold promises), **G16** (interface-touching §8e.1 fix scope must enumerate ALL inheritors via `grep -rn "is I<InterfaceName>" src/ test/ script/`). Vault bytecode untouched — zero edits to `lib/balancer-v3-monorepo/`. **Next: Stage H — emission distributor (F-0, F-1, F-3, F-7) (Opus extra-high entry); plan to be authored.**

**Branch:** `main` at the G5.3 CLAUDE.md §11 refresh commit (HEAD) following FF merge from `stage-g`; `stage-g-complete` lightweight tag at `main` HEAD~4 = `384a40b` (with G5.2a `177547c` + G5.2b `330cc34` + G5.2c `1b8d5cb` between the tag commit and the G5.3 HEAD). `stage-g` branch preserved on `origin` as a snapshot per the C0 / D0 / E-D8 / F9 convention.

Recent commit stack on `stage-g` through tag target `384a40b` (the G5.2a / G5.2b / G5.2c close-docs commits + the G5.3 §11 refresh on `main` HEAD sit above):

- `384a40b` — G5.1e: docs/STAGE_G_PLAN.md — Completion Log rows 104–160 (G3.0–G4.4e GaugeRegistry + integration test)
- `779c816` — G5.1d: docs/STAGE_G_PLAN.md — Completion Log rows 81–103 (G2.0–G2.7c GaugeEligibility)
- `cbbdf88` — G5.1c: docs/STAGE_G_PLAN.md — Completion Log rows 50–80 (G1.9–G1.16d VaultClassRegistry + G1.6-bis/G1.7-bis helper donate expansion)
- `b77d942` — G5.1b: docs/STAGE_G_PLAN.md — Completion Log rows 21–49 (RB-002/004 backports + G1.4-pre-A through G1.8-post-A)
- `317b425` — G5.1a: docs/STAGE_G_PLAN.md — Completion Log rows 1–20 (Stage G prep + G0 + G1.0–G1.3 + G0.1b/c/0.2/0.4/0.5)
- `bbf70be` — G4.4e: test/fork/StageGIntegration.t.sol — Stage F F-D17 regression (CCBMultiplier via F-D23 handoff to real GaugeRegistry)
- `c2d680a` — G4.4d: test/fork/StageGIntegration.t.sol — setGovernanceContract (Stage K handoff: GovernanceTransferred emit + ZeroAddress + NotGovernance + post-handoff access shift)

Earlier Stage G commits (G0 plan/notes scaffold + G1.0 OQ-G3 Option A α-via-DONATION resolution + G1.x VaultClassRegistry / SwapAndDepositToBodensee with Bodensee `enableDonation` flip + Stage F regression backports per RB-002 / RB-004 + G2.x GaugeEligibility + G3.x GaugeRegistry with G-D24 abstract-keyword schedule + G4.x integration test) live in `git log stage-g ^ba11236` (branch point) up to G5.3 main HEAD. Full Completion Log in `docs/STAGE_G_PLAN.md`.

**Current tag:** `stage-g-complete` (commit `384a40b`, 2026-05-14). Next expected tag: `stage-h-complete` at Stage H close. `stage-f-complete` (`9751378`, 2026-05-04), `stage-e-complete` (`41a7590`, 2026-04-28), `stage-d-complete` (`951c338`, 2026-04-25), `stage-c-complete` (`5342126`, 2026-04-18), `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage G position:** **COMPLETE.** Sub-step roll-up: G0 plan / notes scaffold + G-D11 OQ-G3 Option A α-via-DONATION resolution + G-D12 SwapAndDeposit full spec lock + G-D13 90-day-boost deprecation + `docs/ROBUSTNESS_BACKPORT_REGISTER.md` scaffold (RB-002 `CCBMultiplier.emaSampler` + RB-003 `AureumWeightedPoolFactory._poolVersion` + RB-004 external-call & type-cast safety) → G1 `VaultClassRegistry` + `SwapAndDepositToBodensee` + Bodensee `enableDonation = true` (G1.1) + interfaces + unit tests + G15 close-of-family `is IVaultClassRegistry` + view bridges + G10–G14 mid-stage findings → G2 `GaugeEligibility` + `IGaugeEligibility` + `IEfficiencyOracle` sibling + unit tests (52% numerator + TVL floor + F-10 ranking + cohort assignment + reshaped transition events + `onlyGaugeRegistry` F-D23 seal) → G3 `GaugeRegistry` (three activation paths per G-D7 + G-D24 abstract-keyword schedule + G16 fix-scope finding) → G4 fork integration test (`test/fork/StageGIntegration.t.sol` G4.1–G4.4e + `StageGRegistryStageFRegressionTest` for Stage F F-D17 boost-gate via F-D23 seal) → G5 docs (G5.1a–G5.1e Completion Log catch-up + G5.2a STAGES_OVERVIEW.md refresh + G5.2b / G5.2c NOTES / PLAN status flips + this G5.3 CLAUDE.md §11 refresh) → G9 close (FF merge to `main`, lightweight tag `stage-g-complete` at `384a40b`, push). Per-sub-step detail in `docs/STAGE_G_PLAN.md` Decisions table (G-D1 through G-D10 entry-time + mid-stage G-D11 through G-D26 + planning-mechanics G-DP1 through G-DP12) + `docs/STAGE_G_NOTES.md` G-D-numbered findings (notably **G-D11** OQ-G3 Option A α-via-DONATION, **G-D12** SwapAndDeposit full spec, **G-D13** 90-day-boost deprecation, **G-D14** Balancer assembly-helper transient-storage pattern, **G-D16a–G-D16d** GaugeRegistry typed-domain immutables, **G-D20** VaultClassRegistry constructor-injected genesis arrays, **G-D22** GaugeEligibility F-D23 one-shot setter, **G-D23** efficiency tournament + reshaped event ABI, **G-D24** GaugeRegistry abstract-keyword schedule) and G-numbered findings **G10** through **G16** (rolled up in `docs/STAGE_G_NOTES.md` Findings queue).

**Deferred (post-Stage G carry-forward):**

- **OQ-22 concrete TVL oracle implementation.** `ITVLOracle` interface shape pinned at F0.2 (svZCHF numéraire, RP-aware unwrap + constellation-spot averaging); concrete oracle implementation deferred to pre-Stage-H — Stage G integration tests use `MockTVLOracle` from `test/fork/mocks/CCBMocks.sol` per F-D11 / G-D25c.
- **`IEfficiencyOracle` concrete implementation.** Sibling interface added at Stage G per **G-D23 (ii)** (deterministic on-chain F-10 numerator + denominator SMA per OQ-G1); concrete implementation deferred to Stage H. Stage G integration tests use `MockEfficiencyOracle` from `test/fork/mocks/StageGMocks.sol` per **G-D25c**.
...
- **aumm-site spec edits (user-side).** `04_tokenomics.md` §ix prose amendments per OQ-20 + OQ-21; not a repo edit. Additionally, `aumm-site` §xxi sandbox fast-track row is obsolete per **G-D2** (gauge state machine has no sandbox fast-track) — spec edit deferred.

**Next stage: Stage H — emission distributor.** Halving emission schedule (F-0 piecewise bootstrap + F-1 geometric era halving + F-3 per-pool distribution + F-7 CCB-weighted share via Stage F `CCBShare`); `IEfficiencyOracle` concrete implementation lands as part of Stage H or as a pre-H micro-stage. Plan not yet authored — entry per §13 Stage-level defaults: **Opus extra-high** (halving math + emission accumulator type-discipline per F12, both Opus-extra-high domains).
