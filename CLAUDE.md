...
**Last update:** 2026-05-23, **Stage H IN PROGRESS** on branch `stage-h` (HEAD `9db4187`). H1—H6 implementation families + H6.4 close-of-family sweep complete (per prior CLAUDE.md L413 H6.4f snapshot at `ef4b9be`); **H9 fork integration test family complete** (H9.0a fixture + H9.2 BootstrapPhaseTest accrue() cohort + H9.3 distribute() cohort + H9.4 ContinuousPhaseTest + H9.5 HalvingBoundaryTest + H9.6 CrossStackTest; 10/10 fork green across 4 concrete derived suites at H-D40 canonical invocation `forge test --match-path "test/fork/StageHIntegration.t.sol" --fork-url $MAINNET_RPC_URL --threads 1 -vv`); **H9.7 close-of-family sweep in flight** — H9.7a PLAN Completion Log catch-up `ed5dc38` + H9.7b NOTES Findings queue H10/H11/H12 `dd21f3f` + H9.7c PLAN status + mode refresh + H9.7a hash backfill `9db4187` (HEAD) + H9.7d CLAUDE.md §11 refresh (this commit); H9.7e PLAN Completion Log tail pending. **H7 consolidated into H5.2** per **H-D29** (`src/incendiary/IIncendiaryRegistry.sol` landed at H5.2 commit `c4cfe7f`; the H7 plan-row is preserved as a Stage L surface-refinement slot but ships no new code). **H8 reordered post-H9** per chat-level decision — the 612/612 unit cohort across 19 `.t.sol` files already covers all H1—H6 surfaces, so H8 confirmation pass now runs after H9 fork validation rather than between H6 and H9. Branch carries 169 sub-step commits beyond `stage-g-complete` (`384a40b`). **Concrete H9 deliveries:** `test/fork/StageHIntegration.t.sol` (0 → 414 lines across 7 implementation sub-step commits) — `StageHIntegrationFixture` per amended **H-D37** Option A (TVLOracle + EMASampler + CCBMultiplier + EfficiencyOracle + BodenseeBootstrapChannel + EmissionDistributor + MockMiliariumRegistry wiring; H-D23 `setEmissionsRecorder` + H6.3 `setAuMTContract` post-construction wiring in fixture `setUp`; `aumm.setMinter` deferred to per-derived-test `setUp` override per Option A); 4 concrete derived suites per **H-D38** — `StageHBootstrapPhaseTest` (accrue() cohort 4 tests: Bootstrap A leg + Bootstrap B leg + boundary-spanning split + conservation invariant; `_expectedApSum` helper H-D27 bit-for-bit mirror of `test/unit/BodenseeBootstrapChannel.t.sol:L333-L348`; plus distribute() cohort 3 tests: `ZeroAfterAccrue` + `RealVaultDonationArc` + `RevertWhen_NoPendingAccrual`; H-D14 governance handoff in `setUp` via §12 ambiguity-gate catch on H-D39 `vm.prank` pattern) + `StageHContinuousPhaseTest` (1 test: F-7 phase 1/1 — Strategy A white-box seed; 3-pilot `recordScore`→`recordDeposit`→`claim` arc; 100e18 exact per pilot, bounded conservation diff=0 ≤ N_gauged=3) + `StageHHalvingBoundaryTest` (1 test: single-pilot 20-block straddle of `firstHalvingBlock` mirrors H6.3c unit identity 9 × 1e18 + 11 × 5e17 = 14.5e18 in fork context against real AuMM; Strategy A founding-seed + slot 0 EMA store) + `StageHCrossStackTest` (1 test: real gauge approval `VaultClassRegistry` + `GaugeEligibility` + ANTI_SPAM_FEE `SwapAndDepositToBodensee` side-effect → Strategy B TVL self-map → F-D15 EMASampler cold-start → continuous-phase `recordScore` / `recordDeposit` / `claim` → `assertGt(balanceOf, 0)`). **Test cohort: 612/612 unit green** across 19 unit `.t.sol` files (Stage H 225: 36 TVLOracle + 56 EfficiencyOracle + 29 BodenseeBootstrapChannel + 100 EmissionDistributor + 4 AuMMDistributorIntegration; Stage G/F/E/D/C 387) **+ 10/10 fork green** across 4 concrete derived suites under D35 split-form invocation per H-D40. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **H-D locks status:** **H-D1—H-D6 + H-D8—H-D40 LOCKED** (37 total; **H-D35** amended at H6.0c — in-place extension reframe of Stage G `src/token/IAuMT.sol`; **H-D37** amended at H9.0c — minter-slot wiring deferred to per-derived-test `setUp` Option A); **H-D7** (AuMM.setMinter handoff timing — Stage H deploy vs Stage K governance migration) remains OPEN, locks at H10 alongside deployment script + multisig dry-run evidence. **`docs/STAGE_H_NOTES.md` Findings queue (3 entries landed at H9.7b `dd21f3f`, 284 → 318 lines):** **H10** fork-test governance-wiring `setUp` override pattern (H-D14 + H-D39 `vm.prank` collision resolution via `setUp`); **H11** Strategy A (white-box founding-seed for unit-style controlled identities) vs Strategy B (black-box real-pool TVL self-map for real-gauge integration) TVL seeding criterion; **H12** Solidity 0.8.26 non-ASCII string literal restriction (UTF-8 byte sequences in `revert("…")` / `error` payloads compile under 0.8.26 but trigger lint warnings — H9.5-fix1 inline correction).

**Branch:** `stage-h` at H9.7c commit `9db4187` (HEAD). Branched from `stage-g-complete` (`384a40b`) per the **C0 / D0 / E-D8 / F9 / G9** convention; `main` remains at `stage-g-complete` HEAD until Stage H close-of-stage merge at H12. `stage-h` snapshot push to `origin` deferred to H12.

Recent commit stack on `stage-h` (top of HEAD downward; full Completion Log in `docs/STAGE_H_PLAN.md`):

- `9db4187` — H9.7c: docs/STAGE_H_PLAN.md — status header + mode refresh + H9.7a hash backfill <pending> → ed5dc38 (249 lines unchanged)
- `dd21f3f` — H9.7b: docs/STAGE_H_NOTES.md — Findings queue H10/H11/H12: fork-test governance-wiring setUp override + Strategy A/B TVL seeding criterion + Solidity 0.8.26 non-ASCII string literal restriction (284 → 318 lines)
- `ed5dc38` — H9.7a: docs/STAGE_H_PLAN.md — Completion Log catch-up: H9.0d hash backfill <pending> → e099cb6 + 6 new rows H9.1—H9.6 (H9.1 commit-msg label-collision noted; H9.5-fix1 Unicode char inline; 10/10 fork green across 4 derived suites) + H9.7a self-row (242 → 249 lines)
- `00932cb` — H9.6: test/fork/StageHIntegration.t.sol — StageHCrossStackTest per H-D38(4): real gauge approval (VaultClassRegistry + GaugeEligibility + ANTI_SPAM_FEE SwapAndDepositToBodensee side-effect) → Strategy B TVL self-map → F-D15 EMASampler cold-start → continuous-phase recordScore/recordDeposit/claim → assertGt(balanceOf, 0); H-D40 canonical invocation 10/10 green (366 → 414 lines)
- `505a507` — H9.5: test/fork/StageHIntegration.t.sol — StageHHalvingBoundaryTest is StageHIntegrationFixture per H-D38(3): single-pilot 20-block straddle of firstHalvingBlock mirrors H6.3c unit identity (9 × 1e18 + 11 × 5e17 = 14.5e18) in fork context against real AuMM; Strategy A founding-seed + slot 0 EMA store; H-D40 canonical invocation 9/9 green (327 → 366 lines)
- `a3f5e65` — H9.4: test/fork/StageHIntegration.t.sol — StageHContinuousPhaseTest per H-D38(2) (F-7 phase 1/1: Strategy A white-box seed; 3-pilot recordScore→recordDeposit→claim arc; 100e18 exact per pilot, bounded conservation diff=0 ≤ N_gauged=3; 257 → 327 lines)
- `3c35616` — H9.3: test/fork/StageHIntegration.t.sol — distribute() cohort (3 tests: ZeroAfterAccrue + RealVaultDonationArc + RevertWhen_NoPendingAccrual) + H-D14 governance handoff in setUp (§12 ambiguity-gate catch on H-D39 vm.prank pattern)
- `8fff3cc` — H9.2: test/fork/StageHIntegration.t.sol — StageHBootstrapPhaseTest accrue() cohort per H-D38(1) (4/4 green against real Ankr fork: Bootstrap A leg + Bootstrap B leg + boundary-spanning split + conservation invariant; AureumTime import added; _expectedApSum helper H-D27 bit-for-bit mirror of test/unit/BodenseeBootstrapChannel.t.sol:L333-L348; setUp override: super.setUp() + aumm.setMinter(address(bootstrapChannel)) per H-D37 amended Option A; 101 → 208 lines)
- `a24db97` — H9.0a: test/fork/StageHIntegration.t.sol — StageHIntegrationFixture scaffold per H-D37 (0 → 101 lines; TVLOracle + EMASampler + CCBMultiplier + EfficiencyOracle + BodenseeBootstrapChannel + EmissionDistributor + MockMiliariumRegistry wiring; H-D23 setEmissionsRecorder + H6.3 setAuMTContract post-construction wiring; aumm.setMinter deferred to derived contracts per H-D37 amended at H9.0c Option A; incendiaryRegistry slot default per H-D29 zero-stub; IEMASampler/ICCBMultiplier interface-cast pattern at construction call sites per test/unit/EmissionDistributor.t.sol:204-205 canonical)
- `e099cb6` — H9.0d: docs/STAGE_H_PLAN.md — H-D37 amendment reflection: L3 status header + L5 Last update + L34 H9.1 sub-step (minter deferred to per-derived-test setUp) + L80 H-D37 row LOCKED amended at H9.0c + Completion Log H9.0b hash backfill + H9.0c row + H9.0d self-row (240 → 242 lines)
- `7f9a95f` — H9.0c: docs/STAGE_H_NOTES.md — H-D37 amended (minter-slot wiring deferred to per-derived-test setUp override per Option A; efficiencyOracle.setEmissionsRecorder + emissionDistributor.setAuMTContract in fixture setUp; Anchors expansion C-D11/H-D11/H-D20/H-D23; 284 lines unchanged)
- `f4a93b4` — H9.0b: docs/STAGE_H_PLAN.md — H9 sub-step skeleton expansion (H9.0a—H9.7 family) + H-D37—H-D40 Decisions rows + H6.4e/H6.4f/H9.0a/H9.0b Completion Log catch-up + status header + Last update + Mode refresh (this commit; 232 → 240 lines)
- `eade88f` — H9.0a: docs/STAGE_H_NOTES.md — H-D37—H-D40 LOCKED (H9 fork-test design freeze: mock swap-out matrix + StageHIntegrationFixture inheritance + 4-contract cohort + BodenseeBootstrapChannel.distribute() IVault.unlock arc + D35/D36 invocation belt; 268 → 284 lines)
- `ef4b9be` — H6.4f: CLAUDE.md — §11 resume-anchor refresh for H6.4e closure (HEAD 71a6d29 → 65fbb4c; 153 → 155 commits; H6.4 sweep 'in flight' → 'complete (H6.0—H6.4f)'; recent commit stack prepend H6.4e + H6.4d, drop H5.5b + H5.5a; L438 Stage H position + sub-step roll-up final element refreshed)

Earlier Stage H commits (H0—H6.4e reshape chain) live in `git log stage-h ^stage-g-complete` between branch point `384a40b` and `9db4187` HEAD (169 commits total at H9.7c). Full Completion Log in `docs/STAGE_H_PLAN.md`.
...
**Stage H position:** **H1—H6 implementation families + H6.4 close-of-family sweep complete; H9 fork integration test family complete (H9.0a fixture + H9.2 + H9.3 + H9.4 + H9.5 + H9.6 derived suites; 10/10 fork green at H-D40 canonical invocation); H9.7 close-of-family sweep in flight (a/b/c committed, d this commit, e pending); H7 consolidated into H5.2; H8 reordered post-H9; H10—H12 close pending.** Sub-step roll-up from H6.4f forward (H1—H6 detail in prior CLAUDE.md §11 snapshot at `ef4b9be`): → **H9.0a NOTES H-D37—H-D40 LOCKED** (fork-test design freeze: mock swap-out matrix + StageHIntegrationFixture inheritance + 4-contract cohort + `BodenseeBootstrapChannel.distribute()` `IVault.unlock` arc + D35/D36 invocation belt) → **H9.0b PLAN refresh** → **H9.0c NOTES H-D37 amended** (minter-slot wiring deferred to per-derived-test `setUp` Option A; `efficiencyOracle.setEmissionsRecorder` + `emissionDistributor.setAuMTContract` in fixture `setUp`) → **H9.0d PLAN refresh** → **H9.1 `StageHIntegrationFixture` scaffold** (commit `a24db97` carries the H9.0a label in `git log` per H9.7a noted PLAN-vs-commit label collision; full 7-contract wiring per H-D37 Option A) → **H9.2 `StageHBootstrapPhaseTest` accrue() cohort** (Bootstrap A leg + Bootstrap B leg + boundary-spanning split + conservation invariant; `_expectedApSum` helper H-D27 bit-for-bit mirror of `test/unit/BodenseeBootstrapChannel.t.sol:L333-L348`) → **H9.3 distribute() cohort** (`ZeroAfterAccrue` + `RealVaultDonationArc` + `RevertWhen_NoPendingAccrual`; H-D14 governance handoff in `setUp` via §12 ambiguity-gate catch on H-D39 `vm.prank` pattern) → **H9.4 `StageHContinuousPhaseTest`** (F-7 phase 1/1: Strategy A white-box seed; 3-pilot `recordScore` → `recordDeposit` → `claim` arc; 100e18 exact per pilot, bounded conservation diff=0 ≤ N_gauged=3) → **H9.5 `StageHHalvingBoundaryTest`** (single-pilot 20-block straddle of `firstHalvingBlock` mirrors H6.3c unit identity 9 × 1e18 + 11 × 5e17 = 14.5e18 in fork context against real AuMM; Strategy A founding-seed + slot 0 EMA store; H9.5-fix1 inline non-ASCII char correction per H12) → **H9.6 `StageHCrossStackTest`** (real gauge approval `VaultClassRegistry` + `GaugeEligibility` + ANTI_SPAM_FEE `SwapAndDepositToBodensee` side-effect → Strategy B TVL self-map → F-D15 EMASampler cold-start → continuous-phase `recordScore` / `recordDeposit` / `claim` → `assertGt(balanceOf, 0)`) → **H9.7a PLAN Completion Log catch-up** (H9.0d hash backfill `<pending>` → `e099cb6` + 6 new rows H9.1—H9.6 + H9.7a self-row; 242 → 249 lines) → **H9.7b NOTES Findings queue H10/H11/H12 appended** (fork-test governance-wiring `setUp` override + Strategy A/B TVL seeding criterion + Solidity 0.8.26 non-ASCII string literal restriction; 284 → 318 lines) → **H9.7c PLAN status + mode refresh + H9.7a hash backfill `<pending>` → `ed5dc38`** (249 lines unchanged) → **H9.7d CLAUDE.md §11 refresh — this commit**. Per-sub-step detail in `docs/STAGE_H_PLAN.md` Completion Log + Decisions table (now H-D1—H-D6 entry-time + mid-stage H-D8—H-D14 + H4 entry-time H-D15—H-D23 + H4-implementation H-D24—H-D25 + H5 entry-time H-D26—H-D30 + H5.3 entry-time H-D31—H-D33 + H5.4 H-D34 + H6 entry-time H-D35 (amended at H6.0c) + H-D36 + **H9 entry-time H-D37 (amended at H9.0c) + H-D38—H-D40**) and `docs/STAGE_H_NOTES.md` H-D-numbered bodies (notably **H-D37** `StageHIntegrationFixture` inheritance + Option A minter deferral; **H-D38** four-suite cohort matrix; **H-D39** governance handoff in `setUp` per H-D14; **H-D40** canonical fork-test invocation `forge test --match-path "test/fork/StageHIntegration.t.sol" --fork-url $MAINNET_RPC_URL --threads 1 -vv` paired with D35 split-form + D36 `--threads 1` belt). **H-D7** remains OPEN; locks at H10.

**Deferred (Stage G carry-forward + Stage H in-flight):**

- **Fork integration test for the Stage H stack.** `test/fork/StageHIntegration.t.sol` **landed at H9** per the Stage G precedent — 10/10 fork green across 4 concrete derived suites (StageHBootstrapPhaseTest accrue() + distribute() cohorts + StageHContinuousPhaseTest + StageHHalvingBoundaryTest + StageHCrossStackTest) at H-D40 canonical invocation `forge test --match-path "test/fork/StageHIntegration.t.sol" --fork-url $MAINNET_RPC_URL --threads 1 -vv` against mainnet fork via Ankr RPC. Validates `IVault.unlock` callback path against actual Balancer V3 Vault bytecode end-to-end through `BodenseeBootstrapChannel.distribute()` and `EmissionDistributor.claim()` against real AuMM minting.
...
**Next sub-step: H9.7e — PLAN Completion Log tail.** Append H9.7b / H9.7c / H9.7d rows + H9.7e self-row to `docs/STAGE_H_PLAN.md` Completion Log (also backfill H9.7c `<pending>` → `9db4187` + H9.7d `<pending>` → its commit hash at H9.7e drafting time); H9.7e self-row notes the line count after the tail. Then **H8 — unit-test close-of-family confirmation pass** (reordered post-H9 per chat-level decision; the 612/612 unit cohort across 19 `.t.sol` files already covers all H1—H6 surfaces, so H8 is expected to be a no-commit `forge test` audit cycle confirming no regression introduced during H9 fork integration). Then **H10 — H-D7 lock + deployment script + multisig dry-run evidence** (AuMM.setMinter handoff timing closure: Stage H deploy vs Stage K governance migration; novel design surface). Then **H11 — final Completion Log + cross-doc backfill** (FINDINGS.md cross-refs as needed; STAGE_H_NOTES.md final findings audit). Then **H12 — close-of-stage fast-forward merge + `stage-h-complete` tag + preserved `stage-h` snapshot push to `origin`**. Entry per §13 Stage-level defaults: **Sonnet for H9.7e** (mechanical Completion Log append) and **Sonnet for H8** (verification audit; no code changes expected); **Opus high for H10** (deployment-script + multisig dry-run is novel design surface for H-D7 closure; could elevate to Opus extra-high if multisig + AuMM minter-slot interaction surfaces non-trivial decision space at execution time).
