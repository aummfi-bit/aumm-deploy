...
**Last update:** 2026-04-25, Stage D at close. **D8 complete** — full-stage Slither triage (`slither . --filter-paths "lib/|test/" --exclude-informational`, v0.11.4) returned 19 findings across 5 detector classes on the Stage D audit surface (`src/fee_router/AureumFeeRoutingHook.sol` + `src/vault/AureumProtocolFeeController.sol`); triage outcome **19/19 inline-suppress, 0 genuine fixes**. All findings reduce to four structural patterns: (a) Vault.unlock transient-lock artifacts (`reentrancy-events` F1 + `reentrancy-no-eth` F4/F9/F10/F13/F16/F17/F18); (b) deliberate balance-sweep design per D3.3.4 Q1 / Option X (`unused-return` F2/F5/F6/F11/F12); (c) bounded fee-token loop in `onAfterSwap` (`calls-loop` F3/F7/F8/F14/F15); (d) D-D15 SplitIsImmutable slot-layout for `_globalProtocolSwapFeePercentage` (`immutable-states` F19). D8.2 (`1eb38db`) NOTES `## D8 — Slither triage` 19-row table; D8.3 (`01e8b6e`) `AureumFeeRoutingHook.sol` 13 inline-suppress sites (+26 lines, 542 → 568); D8.4 (`33ad133`) `AureumProtocolFeeController.sol` 1 inline-suppress site (+2 lines, 828 → 830); D8.5 post-suppression Slither = 0 findings; D8.6 `forge build` clean + 160/160 green via Part A `forge test --no-match-path "test/fork/**" -vv` (151/151 unit) + Part B `forge test --match-path "test/fork/**" --fork-url $MAINNET_RPC_URL -vv --threads 1` (9/9 fork). **D36** (`19d9eda`) logs the vm.setEnv race condition between `test/fork/AureumFeeRoutingHook.t.sol` L108 and `test/fork/DeployAureumVault.t.sol` L79 under default Foundry parallelism — both write the shared `FEE_ROUTING_HOOK` env key, occasionally clobbering the controller's immutable to `0xF86D…3a77`; `--threads 1` on Part B is the workaround, permanent env-key namespacing fix deferred to Stage E. D8.7a (`1b76407`) Plan Completion Log entry. **D9 — `stage-d-complete` tag + merge to main — next.** Stage C remains complete at `stage-c-complete` (commit `5342126`).

**Branch:** `stage-d` at `1b76407` on top of the following stack:

- `1b76407` — docs: STAGE_D_PLAN.md — D8 Completion Log entry (Slither triage gate clean, 19 suppressions + D36)
- `19d9eda` — docs: STAGE_D_NOTES.md — D36 vm.setEnv race condition between parallel fork test contracts
- `33ad133` — D8.4: src/vault/AureumProtocolFeeController.sol — slither inline-suppress F19
- `01e8b6e` — D8.3: src/fee_router/AureumFeeRoutingHook.sol — slither inline-suppress F1-F18
- `1eb38db` — docs: STAGE_D_NOTES.md — D8 Slither triage section (19 findings, all suppress)
- `86d32d0` — docs: CLAUDE.md §11 refresh — D7 complete + D8 next (tip b225ba5, D35 Ankr split-form)
- `b225ba5` — docs: STAGE_D_NOTES.md D35 — Ankr RPC rate-limit hang on global --fork-url; split-form pattern
- `50a0c2f` — docs: STAGE_D_PLAN.md — D7 Completion Log entry (fork tests green, 160/160)
- `7a296c1` — docs: STAGE_D_PLAN.md — D4.7 Completion Log entry (D4.7a a1c358a + D4.7b 5f4e1db)
- `5f4e1db` — D4.7b: test/unit/AureumProtocolFeeController.t.sol — tests for β1 swap-leg forward
...
- D0 through D6 — complete; see prior §11 states in git history for sub-step detail.
- **D7 — complete (2026-04-25).** D7.1a–d (`b948a98`) + D7.1e–g (`c62f16c`) + D7.1z (`24d5bfa`) — fork harness + 6 fork tests + `.env.example` Stage D constants. Corrective **D4.7** required between D7.1z save and D7.2 fork run: D4.7a (`a1c358a`) controller-side β1 implementation per D17 L170 (5 new surfaces: `collectSwapAggregateFeesForHook`, `collectAggregateFeesHookSwapForward`, `_receiveAggregateFeesSwapForward`, `SwapLegFeeForwarded` event, `OnlyFeeRoutingHook` error); D4.7b (`5f4e1db`) 6 targeted unit tests, 37/37 green (4 invariants × 256 runs × 32768 calls, 0 reverts); D4.7c narrow Slither clean on D4.7a surface (one informational finding on `_globalProtocolSwapFeePercentage` L98 deferred to D8 — pre-existing from D0.5, not D4.7a-introduced). D7.2 fork run (2026-04-25): on-chain verification of `SV_ZCHF` (`0xE5F130253fF137f9917C0107659A4c5262abf6b0`, `SavingsVault ZCHF`, asset = ZCHF `0xB58E61C3098d85632Df34EecfB899A1Ed80921cB`, decimals 18) + `SUSDS` (`0xa3931d71877C0E7a3148CB7Eb4463524FEc27fbD`, `Savings USDS`, asset = USDS `0xdC035D45d973E3EC169d2276DDab16f1e407384F`, decimals 18); `forge test --match-path "test/fork/AureumFeeRoutingHook.t.sol" --fork-url $MAINNET_RPC_URL -vv` 6/6 green in 7.49s wall. Full suite via split-form: Part A `forge test --no-match-path "test/fork/**" -vv` 151/151 green across 6 unit suites; Part B `forge test --match-path "test/fork/**" --fork-url $MAINNET_RPC_URL -vv` 9/9 green across 3 fork suites; combined ~23s wall for 160/160. Global `--fork-url` without path filter triggers indefinite Ankr rate-limit hang — see **D35**. Completion Log: D4.7 row at L897 (`7a296c1`); D7 row at L898 (`50a0c2f`).
- **D8 — complete (2026-04-25).** Full-stage Slither triage per **C8** / **C15** discipline. Baseline: `slither . --filter-paths "lib/|test/" --exclude-informational` (v0.11.4) returned 19 findings across 5 detector classes on the Stage D audit surface (`src/fee_router/AureumFeeRoutingHook.sol` + `src/vault/AureumProtocolFeeController.sol`); triage outcome **19/19 inline-suppress, 0 genuine fixes**. Four structural patterns: (a) Vault.unlock transient-lock artifacts (`reentrancy-events` × 1, F1; `reentrancy-no-eth` × 7, F4/F9/F10/F13/F16/F17/F18); (b) balance-sweep design per D3.3.4 Q1 / Option X (`unused-return` × 5, F2/F5/F6/F11/F12); (c) bounded fee-token loop in `onAfterSwap` capped by Balancer V3's per-pool token limit (`calls-loop` × 5, F3/F7/F8/F14/F15); (d) D-D15 `SplitIsImmutable` slot-layout for `_globalProtocolSwapFeePercentage` kept as `uint256 private` for symmetry with the governance-mutable yield slot (`immutable-states` × 1, F19). D8.2 (`1eb38db`) `STAGE_D_NOTES.md ## D8 — Slither triage` 19-row table with per-finding (severity, file:line, detector, disposition, rationale) entries; D8.3 (`01e8b6e`) `AureumFeeRoutingHook.sol` 13 inline-suppress sites (+26 lines, 542 → 568) — rationale comments contain `D8 NOTES F<n>` cross-refs for greppability; consolidated multi-detector directives at L329 (F2+F3), L357 (F5+F7), L369 (F6+F8), L407 (F11+F14), L420 (F12+F15). D8.4 (`33ad133`) `AureumProtocolFeeController.sol` 1 inline-suppress site at L98 `_globalProtocolSwapFeePercentage` (+2 lines, 828 → 830). D8.5 post-suppression Slither re-run on the same filter set: 0 findings — all 19 suppressions effective on first attempt. D8.6 `forge build` clean; full split-form test suite 160/160 green via Part A `forge test --no-match-path "test/fork/**" -vv` (151/151 unit, ~14s wall) + Part B `forge test --match-path "test/fork/**" --fork-url $MAINNET_RPC_URL -vv --threads 1` (9/9 fork). The `--threads 1` flag on Part B is the **D36** workaround. D8.7a (`1b76407`) Plan Completion Log entry. Completion Log: D8 row at L899 (`1b76407`).
- **D9 — `stage-d-complete` tag + merge to main — next.** Plan §D9 at STAGE_D_PLAN.md L798 (10 min). D9.1 fast-forward `stage-d` → `main` and tag (`git checkout main && git pull && git merge --ff-only stage-d && git push origin main && git tag stage-d-complete && git push origin stage-d-complete`); D9.2 update cross-doc completion logs (`STAGES_OVERVIEW.md` Stage D row marked complete; `FINDINGS.md` deferred-section confirmation for OQ-20 / OQ-21 / D33). Closes Stage D.

**Deferred (post-Stage D):**

- **OQ-20 / D4.6.** Controller yield-fee entry point (governance-gated `routeYieldFeeToHook` or equivalent) — post-Stage D. D7 tests exercise the hook primitive directly, not the controller entry point. See FINDINGS.md OQ-20.
...
**How to resume (Stage D — D9: `stage-d-complete` tag + merge to main):**

1. Confirm current tip: `git log --oneline -1` on `stage-d` matches `1b76407` (D8.7a Plan Completion Log). After this §11 refresh commit lands, the actual tip will advance one commit; that's expected — §11 references the tip-before-itself per project convention.
2. Confirm clean working tree: `git status -sb` shows `## stage-d` only, no modified or untracked files.
3. Confirm full suite still 160/160 green via split-form: Part A `forge test --no-match-path "test/fork/**" -vv` (151/151 unit) + Part B `forge test --match-path "test/fork/**" --fork-url $MAINNET_RPC_URL -vv --threads 1` (9/9 fork). The `--threads 1` flag on Part B is the **D36** workaround for the vm.setEnv race; without it, parallel-write between `AureumFeeRoutingHook.t.sol` and `DeployAureumVault.t.sol` intermittently fails 4–5 of the 9 fork tests.
4. Read plan §D9 at `STAGE_D_PLAN.md` L798 for the exact merge + tag command sequence.
5. D9.1 — fast-forward `stage-d` → `main` and tag. Claude Code drafts the exact terminal commands; user runs in terminal:
   - `git checkout main && git pull` (confirm `main` at `e5ceb7a`, unchanged since D0)
   - `git merge --ff-only stage-d`
   - `git push origin main`
   - `git tag stage-d-complete && git push origin stage-d-complete`
6. D9.2 — update cross-doc completion logs: `STAGES_OVERVIEW.md` Stage D row marked complete; `FINDINGS.md` deferred-section confirmation for **OQ-20** / **OQ-21** / **D33** carried to post-Stage D; commit as `docs: STAGES_OVERVIEW.md + FINDINGS.md — Stage D close (D9.2)`.
7. After D9 closes, the next stage is **Stage E** per `STAGES_OVERVIEW.md`. Entry mode per §13 stage-level defaults table: Opus high entry, then mostly Sonnet for scaffolding/config.
