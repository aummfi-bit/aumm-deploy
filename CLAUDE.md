...
**Last update:** 2026-06-22 — **Stage M COMPLETE** (`stage-m-complete`, ff-merged to `main`). The Miliarium Sector-2 Majors: five yield-core pools — ixCasper (03), ixBrevis (08), ixAltrix (09), ixMediox (10), ixLongus (11) — deployed via per-pool config libs + `DeployIx*.run()` wrappers (Stage-E framework, E-D21/E-D23), then bound into registry/gauge/distributor by the bind-only `DeployStageM.s.sol` (M-D9: `replaceSlot` + `seedFoundingPool` + `setAuMTContractForPool`, under three fail-fast governance preconditions). M-D7 narrowed the M-D2 9-slot Majors set to these five at the M2 token-availability preflight (gap pools 02/04/06/07 → Stage N); M-D11 swapped the ixCasper waEthwstETH 16% leg to plain Lido wstETH after its Rate Provider `0xe1D9…` `getRate()` reverted on mainnet at the M5 fork stage. Fork-tested via `test/fork/StageMIntegration.t.sol` (7/7 binding-liveness — registry/gauge/distributor/hook bound + QG via `evaluateEligibility` + RP `getRate()` resolve, per M-D10) + `test/fork/DeployStageM.t.sol` (3/3 fail-fast governance reverts); full regression 847/847 unit + 109/109 fork green. Prior (2026-06-18) — **WM whitehat pass closed:** F-08 / F-09 / F-10 all **Fixed** fix-forward on `stage-m`, no re-tag (I13-class). Next: Stage N.
...
**Branch / tag state:** `stage-m-complete` tagged and ff-merged to `main` — `main` == `stage-m` at the tag, no divergence. Stage-M history since the prior `main` (`1f8dfa0`, the F-07 WL record): M0.2 NOTES freeze (`8e3dcb5`) → WM whitehat fixes + ledger (`5cc5f3f` … `e86f63d`) → M0.3 PLAN (`edd66e5`) → M1—M5 pool / orchestration / fork work (`4b15d50` … `e580170`) → M6 close. Tags `stage-a-complete` … `stage-l-complete` + `stage-k-whitehat-complete` + `stage-m-complete` all in place. Whitehat ledger carries zero open findings (F-01…F-10 all Fixed).
...
- **OQ-I10 recorder-binding rollout** — pilots 01/05/14 bound through Stage I (post-M-D3 seed); Majors 03/08/09/10/11 bound at Stage M (M-D9 `setAuMTContractForPool`); remaining slots 02/04/06/07 + 12—28 → Stage N.
- **Stage-N pool restoration (M-D7 / M-D11)** — the four deferred Majors (02 ixAetheron / 04 ixViatica / 06 ixLibertas / 07 ixCambio) re-evaluated once their token / RP / QG composition resolves to verified mainnet literals (E-D17 bar); ixCasper's waEthwstETH 16% leg restored via an Aureum-deployed composite RP (`0x72D07D × stataEthwstETH.convertToAssets`, mirroring the fWSTETH composite `0x8Be2`).
