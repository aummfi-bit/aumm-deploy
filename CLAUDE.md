...
**Last update:** 2026-05-25, **Stage I in progress** on branch `stage-i` (HEAD `6cf219f`). **Stage H complete and tagged** at `stage-h-complete` (commit `2a33649`) — fast-forward merged to `main` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. **Stage I — AuMT (Aureum Market Tessera)** open since 2026-05-23: per-pool LP-receipt token (soulbound ERC-20, 18 decimals, one contract per Miliarium pool — 3 instances at Stage I pilot, 28 at full deployment per Stages M/N). I-D1—I-D10 design decisions LOCKED at I0.0a (`9409d76`) + I0.0b (`1404b2d`) per `docs/STAGE_I_NOTES.md`; PLAN sub-step roadmap I0—I7 (45 sub-steps + I-D1—I-D10 Decisions table mirror + Completion log) seeded at I0.0c (`984456a`). **I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE** across I1.1—I1.8c: I-D9 LOCKED Option A (per-pool mapping replaces single-slot `auMTContract`; `setAuMTContractForPool(pool, addr)` one-shot setter with `ZeroAddress` + `AuMTAlreadyBound` guards; `onlyAuMTContract(pool)` modifier; H-D16 zero-address safety valve removed since each AuMT instance is mandatory-non-zero per I-D9); interface signature updates at I1.1 (`5ff9072`) with I1.1-fix1 (`46a93c7`) for stale L73 NatSpec cross-ref; implementation at I1.2 (`c3147c1`) with I1.2-fix1 (`1f9c74f`) for stale NatSpec at L19/L81/L100/L107/L191; test file refactors at I1.3 EmissionDistributor.t.sol (`887b656`, 8 tests updated + 1 new `test_RevertWhen_SetAuMTContractForPoolAlreadyBound`, 100→101 tests) + I1.4 AuMMDistributorIntegration.t.sol (`a20245a`) + I1.5 StageHIntegration.t.sol (`009e591`, fixture expanded to bind all 3 pilot pools) + I1.6 DeployStageH.t.sol (`401f047`, new `(3b-bis)` assertion `auMTContractByPool(bodenseePool) == address(0)` post-deploy per H-D7 Option C + I-D9 deferred binding); **I1.7 regression 624/624 green** at H-D40 canonical invocation pattern (D35 split-form + D36 `--threads 1` belt) — 613/613 unit across 22 suites + 10/10 fork integration (4 derived StageHIntegrationFixture suites) + 1/1 deploy fork; no commit (regression run only). **I1.8 close-of-family sweep in flight** across I1.8a `44a94bb` (NOTES — I10 Findings entry: multi-file rename refactor NatSpec drift, default rule for rename §8e.1 to enumerate old-symbol grep hits, intra-file complement to G16) + I1.8b `c33ea30` (PLAN — Completion Log catch-up: I0.0c hash backfill + 10 new rows I1.1—I1.8a) + I1.8c `6cf219f` (PLAN — status header + Last update + Mode refresh for I1 family close + I2/I3 ahead) + I1.8d CLAUDE.md §11 refresh (this commit) + I1.8e PLAN Completion Log tail pending. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **I-D locks status:** I-D1—I-D10 LOCKED (full 10-decision lock at I0.0b). **`docs/STAGE_I_NOTES.md` Findings queue (1 entry):** I10 — multi-file rename refactor NatSpec drift (intra-file old-symbol grep rule complement to G16 cross-file inheritor enumeration) landed at I1.8a `44a94bb`. **Stage H complete state:** H-D1—H-D42 LOCKED (39 total); stage tag at `2a33649`; full Completion Log in `docs/STAGE_H_PLAN.md`.

**Branch:** `stage-i` at I1.8c commit `6cf219f` (HEAD). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.
...
- `009e591` — I1.5: test/fork/StageHIntegration.t.sol — `setAuMTContractForPool(pilotPools[0..2], address(this))` per I-D9 — expand single fixture recorder binding to all 3 pilot pools; L32 NatSpec + L100—L102 statement expansion (414 → 416 lines)
...
- `887b656` — I1.3: test/unit/EmissionDistributor.t.sol — update 8 tests + add `AuMTAlreadyBound` test per I-D9 (setUp binds POOL_A; rename governance-gate / constructor-default / bind+emit tests; zero-address test inverts to `ZeroAddress` revert; new `test_RevertWhen_SetAuMTContractForPoolAlreadyBound`; `NotAuMTContract` reverts add POOL_A first arg; 100 → 101 tests; 1482 → 1489 lines)
- `1f9c74f` — I1.2-fix1: src/emission/EmissionDistributor.sol — stale NatSpec cross-refs at L19/L81/L100/L107/L191 (`auMTContract` single-slot → `auMTContractByPool` per I-D9); I10 default rule applied
- `c3147c1` — I1.2: src/emission/EmissionDistributor.sol — per-pool `auMTContractByPool` mapping per I-D9 (storage slot + `onlyAuMTContract(pool)` modifier + `setAuMTContractForPool` one-shot setter with `ZeroAddress` + `AuMTAlreadyBound` guards + `recordDeposit` / `recordWithdrawal` modifier annotation update; 500 lines)
- `46a93c7` — I1.1-fix1: src/emission/IEmissionDistributor.sol — stale L73 NatSpec cross-ref (`AuMTContractSet` → `AuMTContractBound` per I1.1 rename); surfaced I10 default rule
- `5ff9072` — I1.1: src/emission/IEmissionDistributor.sol — interface signature updates per I-D9 (rename `AuMTContractSet` → `AuMTContractBound` event + amend `NotAuMTContract(pool, caller)` error + add `AuMTAlreadyBound(pool)` error + rename `setAuMTContract` → `setAuMTContractForPool` + rename `auMTContract()` → `auMTContractByPool(pool)` view; H-D16 zero-address safety valve removed; 263 lines)
- `984456a` — I0.0c: docs/STAGE_I_PLAN.md — Stage I sub-step roadmap (I0—I7, 45 sub-steps, I-D1—I-D10 Decisions table mirror, Pre-flight checklist, Completion log seeded; 182 lines)
- `1404b2d` — I0.0b: docs/STAGE_I_NOTES.md — I-D9 LOCKED Option A (per-pool mapping in EmissionDistributor) + I-D10 LOCKED (AureumTime extension `QUALIFICATION_PERIOD_BLOCKS` + `ON_RAMP_PERIOD_BLOCKS`)
- `9409d76` — I0.0a: docs/STAGE_I_NOTES.md — Stage I design freeze (I-D1—I-D8 LOCKED + I-D9 OPEN; 114-line file landed)
...
**Stage I position:** **I0 design freeze + PLAN creation complete (I-D1—I-D10 LOCKED); I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE across I1.1—I1.8c (interface + implementation + 4 test files + regression 624/624 green + close-of-family sweep a/b/c committed); I1.8d CLAUDE.md §11 refresh this commit; I1.8e PLAN Completion Log tail pending.** Sub-step roll-up: → I0.0a NOTES design freeze (I-D1—I-D8 LOCKED + I-D9 OPEN) → I0.0b NOTES I-D9 LOCKED Option A + I-D10 LOCKED → I0.0c PLAN creation (sub-step roadmap I0—I7 + Decisions table + Pre-flight + Completion log + Open questions + Anchors) → I1.1 IEmissionDistributor.sol interface signature updates (I-D9 event / error / view / setter renames) → I1.1-fix1 stale L73 NatSpec cross-ref → I1.2 EmissionDistributor.sol per-pool mapping implementation → I1.2-fix1 stale NatSpec at L19/L81/L100/L107/L191 → I1.3 EmissionDistributor.t.sol 8 tests updated + new AuMTAlreadyBound test (100 → 101 tests) → I1.4 AuMMDistributorIntegration.t.sol setUp update (1 line) → I1.5 StageHIntegration.t.sol fixture expansion (3 pilot pools) → I1.6 DeployStageH.t.sol new (3b-bis) assertion → I1.7 regression 624/624 green (613 unit + 10 fork integration + 1 deploy fork) → I1.8a NOTES I10 Findings entry → I1.8b PLAN Completion Log catch-up (I0.0c hash backfill + 10 new rows) → I1.8c PLAN status / Last update / Mode refresh → I1.8d CLAUDE.md §11 refresh — this commit. Per-sub-step detail in `docs/STAGE_I_PLAN.md` Completion Log + Decisions table (I-D1—I-D10 LOCKED — 10 total at I1.8d) and `docs/STAGE_I_NOTES.md` I-D-numbered bodies.
...
**Next sub-step: I1.8e — PLAN Completion Log tail.** Append rows for I1.8b/I1.8c/I1.8d + I1.8e self-row to `docs/STAGE_I_PLAN.md` Completion log table. After I1.8e the I1 close-of-family sweep is fully landed and Stage I I2 entry opens (AureumTime constants extension per I-D10: `QUALIFICATION_PERIOD_BLOCKS = BLOCKS_PER_EPOCH = 100_800` + `ON_RAMP_PERIOD_BLOCKS = 180 * BLOCKS_PER_DAY = 1_296_000`). Entry per §13 Stage-level defaults: **Sonnet for I2** (mechanical lib constant addition + test assertions mirroring existing AureumTime.t.sol patterns); Opus drafts the I2.1 §8e.1 then Sonnet executes I2.2/I2.3. I3 (AuMT.sol concrete implementation — soulbound ERC-20 state machine + I-D7 root-curve `governanceWeight` formula + I-D6 qualification clock + I-D4 onlyLiquidityHook access + I-D1 `NotTransferable` reverts) is the next Opus-high focus block.
