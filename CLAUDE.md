...
**Last update:** 2026-05-28, **Stage I in progress** on branch `stage-i` (HEAD `6671a29`). **Stage H complete and tagged** at `stage-h-complete` (commit `2a33649`) — fast-forward merged to `main` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. **Stage I — AuMT (Aureum Market Tessera)** open since 2026-05-23: per-pool LP-receipt token (soulbound ERC-20, 18 decimals, one contract per Miliarium pool — 3 instances at Stage I pilot, 28 at full deployment per Stages M/N). I-D1—I-D12 design decisions LOCKED across I0.0a/b (`9409d76` / `1404b2d`) + I3.0a (`ca2596e`, I-D11) + I3.5-pre1 (`4a595e8`, I-D12) per `docs/STAGE_I_NOTES.md`; PLAN sub-step roadmap I0—I7 (45 sub-steps + I-D1—I-D12 Decisions table mirror + Completion log) seeded at I0.0c (`984456a`). **I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE** across I1.1—I1.8e (interface signature updates + per-pool mapping + 4 test file refactors + regression 624/624 green + close-of-family sweep). **I2 — AureumTime constants extension (per I-D10) COMPLETE** across I2.1—I2.3e (QUALIFICATION_PERIOD_BLOCKS + ON_RAMP_PERIOD_BLOCKS constants + 4 I-D10 tests + close-of-family sweep). **I3 — AuMT.sol concrete implementation COMPLETE** across I3.1—I3.8c: I3.1 (`0bd199d`) skeleton + I3.2 (`e068d4a`) soulbound overrides per I-D1 + I3.3 (`7163c93`) mint/burn activation per I-D4 (`onlyLiquidityHook` modifier + `NotLiquidityHook` revert) + I3.4 (`a2a11bc`) I-D6 qualification clock + I3.5-pre1/pre2/pre3 (`4a595e8` / `feab19d` / `4178fd9`) I-D12 gaugeRegistry 5th immutable LOCK + propagation + I3.5 (`1daf212`) I-D7 root-curve `governanceWeight` (4th/3rd root via `FixedPoint.powDown` + F-9 era-boundary transition) + I3.6 (`d96e7dd`) H-D35 recorder integration (`recordDeposit` post-mint + `recordWithdrawal` post-burn) + I3.7 (`c285738`) IAuMT.sol L35/L47-L48 dual NatSpec correction + I3.8a (`d11dbb7`) NOTES I12 Findings + I3.8b (`c8360b9`) AuMT contract-level NatSpec refresh + I3.8c (`38ac23b`) PLAN Completion Log catch-up + I3.8d (`6671a29`) PLAN status refresh. **I3.8e CLAUDE.md §11 refresh** (this commit) + **I3.8f PLAN Completion Log tail** pending. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **I-D locks status:** I-D1—I-D12 LOCKED (12 total: I-D1—I-D10 at I0.0a/b; I-D11 at I3.0a; I-D12 at I3.5-pre1). **`docs/STAGE_I_NOTES.md` Findings queue (3 entries):** I10 — multi-file rename refactor NatSpec drift at I1.8a `44a94bb`; I11 — Foundry `forge test --match-test` Rust regex at I2.3a `f143b7a`; I12 — container-shape LOCKs must cross-check prior D* consumers at I3.8a `d11dbb7`. **Stage H complete state:** H-D1—H-D42 LOCKED (39 total); stage tag at `2a33649`; full Completion Log in `docs/STAGE_H_PLAN.md`.

**Branch:** `stage-i` at I3.8d commit `6671a29` (HEAD). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.
...
**Stage I position:** **I0 design freeze + PLAN creation complete (I-D1—I-D12 LOCKED); I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE across I1.1—I1.8e; I2 — AureumTime constants extension (per I-D10) COMPLETE across I2.1—I2.3e; I3 — AuMT.sol concrete implementation (per I-D1 / I-D4 / I-D6 / I-D7 / I-D11 / I-D12) COMPLETE across I3.1—I3.8c (soulbound overrides + onlyLiquidityHook gate + qualification clock + I-D12 gaugeRegistry 5th immutable LOCK + propagation + root-curve governanceWeight + H-D35 recorder integration + IAuMT.sol dual NatSpec correction + AuMT contract-level NatSpec refresh + PLAN catch-up); I3.8d PLAN status refresh landed; I3.8e CLAUDE.md §11 refresh this commit; I3.8f PLAN Completion Log tail pending.** Sub-step roll-up: I0.0a/b/c (design + PLAN seed; I-D1—I-D10 LOCKED) → I1.1—I1.8e (interface + impl + 4 test files + regression 624/624 green + close-of-family sweep) → I2.1—I2.3e (AureumTime constants + 4 tests + close-of-family sweep) → I3.0a (NOTES I-D11 LOCK + I-D4 amendment) → I3.0b (PLAN I-D11 row + 4 wording amendments) → I3.0c (PLAN Completion Log catch-up + Anchors) → I3.0d (PLAN status refresh) → I3.1 (AuMT skeleton: pragma + imports + 6-arg constructor + 4 immutables + IAuMT/ERC20 inheritance) → I3.2 (soulbound overrides per I-D1: NotTransferable revert on transfer/transferFrom/approve) → I3.3 (mint/burn activation per I-D4: NotLiquidityHook + onlyLiquidityHook modifier + _mint/_burn bodies) → I3.4 (I-D6 qualification clock: qualificationBlock + lastDepositBlock mappings + mint sets-if-zero + burn resets-on-any-withdrawal) → I3.5-pre1 (NOTES I-D12 LOCK: gaugeRegistry 5th immutable; Option A direct-binding) → I3.5-pre2 (PLAN I-D12 mirror) → I3.5-pre3 (AuMT.sol IGaugeRegistry import + gaugeRegistry immutable + constructor arg + guard + cast) → I3.5 (governanceWeight per I-D7 root-curve: 4th/3rd root via FixedPoint.powDown + F-9 era-boundary transition + 3-branch ZERO short-circuit) → I3.6 (H-D35 recorder integration: IEmissionDistributor import + recordDeposit post-mint + recordWithdrawal post-burn; CEI ordering) → I3.7 (IAuMT.sol L35 mint + L47-L48 burn NatSpec correction: NotDistributor/distributor-only → NotLiquidityHook/liquidity-hook-only) → I3.8a (NOTES I12 Findings: container-shape LOCK §12 cross-check rule) → I3.8b (AuMT contract-level NatSpec refresh: I-D11+I-D12 5 immutables + gaugeRegistry entry + 7-arg constructor) → I3.8c (PLAN Completion Log catch-up: 14 new rows I3.0d—I3.8b + self-row) → I3.8d (PLAN status header refresh) → I3.8e CLAUDE.md §11 refresh — this commit. Per-sub-step detail in `docs/STAGE_I_PLAN.md` Completion Log + Decisions table (I-D1—I-D12 LOCKED — 12 total at I3.8e) and `docs/STAGE_I_NOTES.md` I-D-numbered bodies + I10/I11/I12 Findings.
...
**Next sub-step: I3.8f — PLAN Completion Log tail.** Append rows for I3.8c/I3.8d/I3.8e + I3.8f self-row to `docs/STAGE_I_PLAN.md` Completion log table. After I3.8f the I3 close-of-family sweep is fully landed and Stage I I4 entry opens (AureumFeeRoutingHook extension per I-D5 — `auMTByPool` storage + `setAuMTForPool` one-shot setter + `onAfterAddLiquidity` mint dispatch + `onAfterRemoveLiquidity` burn dispatch). Entry per §13 Stage-level defaults: **Opus high for I4** (existing AureumFeeRoutingHook extension with new hook callback bodies; Opus drafts I4.x §8e.1 prompts and audits Cursor's saves). I5 (AuMT unit tests including I5 invariant tests) and I6 (fork integration) follow after I4 close-of-family.
