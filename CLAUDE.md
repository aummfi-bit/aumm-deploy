...
**Next sub-step: K5 pre-flight** — the Opus beat that locks **K-D7** (`AuMM.setMinter(EmissionDistributor)` handoff per H-D41). K5 is the minter transition that unblocks `EmissionDistributor.distribute()` / `claim()` from the `NotMinter` guard: AuMM is deployed minterless, and a one-shot `setMinter` points it at the live `EmissionDistributor`. The pre-flight resolves where the call lives (standalone vs folded into the K7 `script/DeployStageK.s.sol` per H-D42), the one-shot-vs-idempotent semantics of `AuMM.setMinter`, and the fork-test coverage asserting the `NotMinter`→minter transition. K5 reads H-D41 / H-D42 (`docs/STAGE_H_NOTES.md`), the `AuMM.setMinter` surface (`src/token/AuMM.sol`), and `EmissionDistributor` (`src/emission/EmissionDistributor.sol`). **Stage K mode: Opus extra-high** per §13 — governance handoff, stay on Opus through the stage; each unit opens with its own Opus pre-flight.
