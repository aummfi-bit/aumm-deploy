...
Dropped as CLOSED: D33 (no custom Router, O-D5 — the Router redeploy/bind is owned by P-bis workstream B); the whitehat back-fill queue (ran at Stage P; see the Stage P line + the ledger); and the pool-restoration set — Majors 04 ixViatica / 07 ixCambio + ixCasper waEthwstETH + the OQ-I10 remainder (PB2.3 / PB-D8: ixCasper BUILT, 04/07 DESCOPED to the Stage-O challenge path, OQ-I10 closed).
