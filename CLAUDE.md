...
**Last update:** 2026-04-25, **Stage D closed.** D9 complete — `git push origin stage-d` (D9.0; `origin/stage-d` was 39 commits behind local `stage-d`, never pushed during D8; 154 objects from `18f74b9` to `951c338`); `git merge --ff-only stage-d` on `main` (D9.1; fast-forward from `d04a43d` to `951c338`, 23 files changed, 4476 insertions, 389 deletions, 10 new files); `git tag stage-d-complete && git push origin stage-d-complete && git push origin main` (D9.1). All five refs — `main`, `origin/main`, `stage-d`, `origin/stage-d`, and the `stage-d-complete` tag — are at `951c338`. The D9.2/D9.3/D9.4 cross-doc bundle commit (`docs/STAGE_D_PLAN.md` Completion Log row + `docs/STAGES_OVERVIEW.md` Stage D row + this CLAUDE.md §11 refresh) lands on `stage-d` on top of `951c338` per the plan's prescribed flow. Stage D delivered: `AureumFeeRoutingHook` (568 lines, `BaseHooks` + `IAureumFeeRoutingHook` + `VaultGuard`) implementing OQ-1 50% swap-leg saturation via `Vault.unlock` + nested swap to svZCHF + one-sided add to der Bodensee with trusted-router recursion guard per D10, plus three governance-callable primitives (`routeYieldFee` / `routeGovernanceDeposit` / `routeIncendiaryDeposit`) per D-D2 Option A; `AureumProtocolFeeController` retrofit (D-D7 B10 retarget to `FEE_ROUTING_HOOK`, D-D9/OQ-2 Bodensee yield-collection guard, D-D8/OQ-11 swap-fee band 0.10%/1.00%/0.75%, D-D15 `SplitIsImmutable` lock, D4.7 β1 controller-side `collectSwapAggregateFeesForHook`); `script/DeployDerBodensee.s.sol` fork-only 40/30/30 WeightedPool (AuMM/sUSDS/svZCHF, D11 Rate Providers); 151 unit + 9 fork = 160/160 green via split-form (Part B `--threads 1` per D35 Ankr rate-limit / D36 vm.setEnv race); Slither D8 triage 19/19 inline-suppress (4 structural patterns: `Vault.unlock` transient-lock, deliberate balance-sweep, bounded fee-token loop, `SplitIsImmutable` layout), 0 genuine fixes, post-suppression re-run 0 findings. **Next: Stage E — Pool-deployment framework + 3 pilot pools (ixHelvetia, ixEdelweiss, ixCambio).**

**Branch:** `stage-d` at `951c338` post-D9.1 merge — identical to `main`, `origin/main`, `origin/stage-d`, and the `stage-d-complete` tag. The D9 cross-doc bundle commit lands on top of `951c338`; after it lands, §11 references `951c338` as the tip-before-itself per project convention.

Recent commit stack (`951c338`-and-prior, all on `main` post-FF):

- `951c338` — docs: CLAUDE.md §11 refresh — D8 complete + D9 next (tip 1b76407, D36 vm.setEnv race)
...
Earlier Stage D commits live in `git log main` from `d04a43d` (Stage D branch point) to `951c338`.

**Current tag:** `stage-d-complete` (commit `951c338`, 2026-04-25). Next expected tag: `stage-e-complete` at Stage E close. `stage-c-complete` (`5342126`, 2026-04-18), `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage D position:** **complete.** Per-sub-step detail (D0 through D9) lives in `docs/STAGE_D_PLAN.md` Completion Log (bottom of file) + `docs/STAGE_D_NOTES.md` D-numbered findings (notably **D17** β-split architecture, **D22** `IVault.unlock` inner-callback decode contract, **D24** Cursor scope-expansion, **D31** §8e.1 two-block split, **D32** Router single-vault binding + Permit2 chain, **D34** D4 controller-side execution gap, **D35** Ankr global `--fork-url` hang, **D36** vm.setEnv race) + git history on `main` from `d04a43d` to `951c338`. The full Stage D summary lives in the Stage D row of `docs/STAGES_OVERVIEW.md`'s master Completion Log.

**Deferred (post-Stage D):**

- **OQ-20 / D4.6.** Controller yield-fee entry point (governance-gated `routeYieldFeeToHook` or equivalent) — post-Stage D. D7 fork tests exercise the hook primitive directly, not the controller entry point. See FINDINGS.md OQ-20.
...
- **D36 permanent fix.** `test/fork/DeployAureumVault.t.sol` and `test/fork/AureumFeeRoutingHook.t.sol` env-key namespacing (e.g. `FEE_ROUTING_HOOK_DEPLOY_TEST`) or eliminating `vm.setEnv` from fork tests entirely. Workaround `--threads 1` on the Part B fork run holds; permanent fix targeted at Stage E entry (the Stage E test harness for 3 pilot pools must solve this) or a dedicated pending-findings stub.
...
**How to resume (Stage E — Pool-deployment framework + 3 pilot pools):**

1. Confirm current tip: `git log --oneline -1` on `main` (or `stage-d`, identical post-FF) returns the D9 cross-doc bundle commit on top of `951c338`. The D9 commit is the §11-refresh-itself; §11 above references `951c338` as the tip-before-itself per project convention.
2. Confirm clean working tree: `git status -sb` shows only the active branch tracking line, no modified or untracked files.
3. Read `docs/STAGES_OVERVIEW.md` `## Stage E — Pool-deployment framework + 3 pilot pools` for stage scope: parameterized `script/pools/deploy-miliarium-pool.s.sol` framework + per-pool config files + 3 pilot pools — **ixHelvetia** (100% ERC-4626, 80% svZCHF + 20% sUSDS, simplest composition, routing anchor), **ixEdelweiss** (46% ixEDEL routing hub, primary connector pool), **ixCambio** (6-token FX hub per OQ-12/OQ-12a stressing `MAX_TOKENS`). Dependencies: hook + Bodensee (D) deployed; AuMM (C) exists.
4. Open a Stage E branch from `main`: `git checkout main && git checkout -b stage-e`. Author `docs/STAGE_E_PLAN.md` and `docs/STAGE_E_NOTES.md` skeletons (Stage A–D pattern: numbered sub-steps with grep commands per §6 / §8e Audit cycle; D-numbered planning decisions and incident log).
5. Pre-flight reads to draft E0 (plan kickoff): `06_miliarium_manifest.md` from project knowledge (slot numbers — ixHelvetia, ixEdelweiss, ixCambio per the manifest); OQ-12 / OQ-12a in `FINDINGS.md` (ixCambio FX-hub composition); `script/DeployDerBodensee.s.sol` as the (β) deployment-pattern reference (D32); `lib/balancer-v3-monorepo/` `WeightedPoolFactory.create` interface and `AureumVault.registerPool(...)` parameter shape including hook attachment and 52% ERC-4626 Quality Gate per stage scope.
6. Resolve pending E-D* decisions before E1 implementation: parameter-file format (JSON vs. `.s.sol` struct); 52% ERC-4626 Quality Gate enforcement point (factory vs. Vault registration); Rate Provider configuration per pilot pool; mainnet-fork pool-init pattern (β-style `Vault.unlock` per D32, no Router, no Permit2); test-harness layout for 3 pilot pools (one fork test contract per pool vs. shared parameterized contract — D36 env-key namespacing must be solved at the harness level, not as a per-test workaround).
7. Entry mode per §13 stage-level defaults table: **Opus high entry**, then mostly **Sonnet** for scaffolding and per-pool config files; back to **Opus high** for the 52% Quality Gate logic and fork-test harness design (D36 permanent fix).
