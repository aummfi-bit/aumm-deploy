...
**Last update:** 2026-06-07, **Stage K open** on branch `stage-k`. K0 entry landed: K0.1 branch (`83ef71e` base), K0.2 NOTES freeze (`e241c22` — K-D1 + K-D2 LOCKED, K-D3—K-D9 forward-declared), K0.3 PLAN roadmap (`597880e`); this commit is **K0.4** (§11 refresh). **Stage K = the governance handoff** (STAGES_OVERVIEW Stage K; "Largest stage"): ship on-chain governance (`AureumGovernance` — three proposal types) + the value-weighted voting reader (`VotingWeight`, I-D15) + the B-strict 12-month time-bomb authorizer (`AureumGovernanceAuthorizer`, OQ-10), and migrate the Vault's authorizer from the Stage B Safe to the governance contract. **K-D1 + K-D2 LOCKED; K-D3—K-D9 forward-declared, each locking at its unit pre-flight. Vault bytecode untouched** — the migration is `setAuthorizer` / `setStaticSwapFeePercentage` CALLS, never edits. **Stage J context:** J-D1—J-D8 LOCKED, Finding J10; tag `stage-j-complete` (`83ef71e`), ff-merged to `main`, pushed to `origin`.

**Branch:** `stage-k` at K0.4 (this `CLAUDE.md` §11 commit; prior `597880e` K0.3, `e241c22` K0.2). Full K0 stack on the branch. Branched from `stage-j-complete` (`83ef71e`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention (and Stages I, J). `main` sits at `stage-j-complete` HEAD until the K-close ff-merge; `stage-k` snapshot push to `origin` lands at K-close.

Recent commit stack — Stage K (full per-sub-step catalog in the `docs/STAGE_K_PLAN.md` Completion log):

- **K0** (entry): K0.1 branch (`83ef71e` base) / K0.2 NOTES freeze (`e241c22`) / K0.3 PLAN roadmap (`597880e`) / K0.4 this §11 refresh.
...
**Next sub-step: K1 pre-flight** — the Opus extra-high beat that locks **K-D3** (`AureumGovernanceAuthorizer` full design per OQ-10: exact emergency-action-ID set from Vault action IDs, `canPerform` routing, immutable layout) and details the K1.x sub-steps (interface check + concrete contract + unit cohort). K1 reads OQ-10 (FINDINGS L722-794) + the Stage B `AureumAuthorizer` migration-target framing + Balancer `IAuthorizer`. **Stage K entry mode: Opus extra-high** per §13 — governance handoff, stay on Opus through the stage; each unit K1—K7 opens with its own Opus pre-flight.
