...
**Last update:** 2026-06-05, **Stage I code phase open** on branch `stage-i` (HEAD at I5.5c, the §11 resume-anchor refresh closing the I5 family). **Stage H complete and tagged** at `stage-h-complete` (`2a33649`). **I9 (AuMT deprecation + IVotingWeight migration), I4 (recorder dispatch + clock), and I5 (recorder-clock unit tests) are all COMPLETE.** Per **OQ-25 / I-D14**, AuMT is the pool's own Balancer V3 **BPT** (branded), **not a separate token**; Stage I is the oracle-free **recorder clock + hook dispatch** only; the value-weighted snapshot-voting view is **deferred to a Stage K `src/governance/VotingWeight.sol` reader** per I-D15. **I4 shipped:** `getHookFlags` bump (I4.1 `409f51a`), one-shot `emissionRecorder` slot + `setEmissionRecorder` two-flag setter (I4.2 `3f8e8d5`), `effectiveQualBlock` clock in `EmissionDistributor` (I4.3 `5fb3c3f`), `onAfterAddLiquidity` → `recordDeposit` with a minimal `IRouterSender` Permit2-free shim (I4.4 `c47e6df` + I4.4a `62f7741`; D32 / Finding I16), `onAfterRemoveLiquidity` → `recordWithdrawal` (I4.5 `2b85263`), regression + post-I4.2 storage-slot-layout fix (I4.6a/c `5e63b3b`/`56bc143`; Finding I17; **620/620 unit + 71/71 fork green**), close-of-family sweep (I4.7a—d). **I5 shipped:** `test/unit/RecorderClock.t.sol` — first-deposit fresh-start (I5.1a/b `766566a`/`198a935`), weighted-average top-up (I5.2 `9db3caa`), partial-withdrawal reset + over-qualification vector (I5.3a/b `370df5b`/`1c9d638`), hook-gate `NotAuMTContract` (I5.4 `739a329`), close-of-family sweep (I5.5a—c); **targeted 5/5 RecorderClock green** (full unit 625, fork 71 — full regression at I6). **Locks:** I-D14 (AuMT = BPT), I-D15 (view deferred), I-D16 (hook one-shot `emissionRecorder` slot, not a ctor immutable), I-D17 (`IVotingWeight` stub + `VaultClassRegistry` rename); I-D1—I-D17 LOCKED (17 total). **I1 (I-D9), I2 (I-D10), I8 (I-D13) COMPLETE pre-reframe; I3 (concrete `AuMT.sol`) TOMBSTONED** (removed at I9.4). **Vault bytecode untouched.** **Findings:** I10—I17 (`docs/STAGE_I_NOTES.md`). **Stage H:** H-D1—H-D42 LOCKED; tag `2a33649`.

**Branch:** `stage-i` at I5.5c (the §11 resume-anchor refresh closing the I5 family; HEAD this commit; prior `cb8c65e` I5.5b closed the PLAN status/mode refresh). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.

Recent commit stack — the full per-sub-step catalog (every commit, hash, diff) now lives in the `docs/STAGE_I_PLAN.md` Completion Log, caught up through I5.5a. §11 keeps only the phase summary:
...
- **I5 (recorder-clock unit tests):** `test/unit/RecorderClock.t.sol` — first-deposit fresh-start + weighted-average top-up + partial-withdrawal reset + over-qualification vector + hook-gate `NotAuMTContract` + sweep (I5.1a `766566a`…I5.5b `cb8c65e`). CODE COMPLETE; targeted 5/5 RecorderClock green (full unit 625, fork 71).
...
**Next sub-step: I6.1 — `StageIIntegrationFixture` in `test/fork/StageIIntegration.t.sol`.** I5 (recorder-clock unit tests) is COMPLETE (I5.1a—I5.5c; `test/unit/RecorderClock.t.sol` — 5 tests, 5/5 RecorderClock green). The code phase continues with the I6 recorder-clock fork integration: **I6.1** `StageIIntegrationFixture` (inherits `StageHIntegrationFixture` per H13) — `setEmissionRecorder(distributor)` on the canonical `AureumFeeRoutingHook` (I-D16) + `setAuMTContractForPool(pilotPool, hook)` ×3 binding the recorder gate to the hook (I-D9 amend); no AuMT deploys → **I6.2** deposit-dispatch — real Balancer V3 add-liquidity to each of the 3 pilot pools routes through `onAfterAddLiquidity` → `recordDeposit`, setting `effectiveQualBlock` + accruing `userLP` → **I6.3** top-up — second add-liquidity blends `effectiveQualBlock` by deposit-weighted average per I-D14 → **I6.4** aging + withdrawal-reset — `vm.roll` past `QUALIFICATION_PERIOD_BLOCKS`; real remove-liquidity → `recordWithdrawal` resets `effectiveQualBlock` to 0 → **I6.5** sweep. Real Balancer V3 add / remove-liquidity through the canonical `AureumFeeRoutingHook` (per H13 fixture inheritance). Entry mode: **Opus high** per §13 for the I6 fork-fixture design (`setEmissionRecorder` + `setAuMTContractForPool` ×3 binding + real liquidity-callback dispatch). Also pending: the **Deferred-from-I4** `setEmissionRecorder` unit-level two-flag-lock tests (see `STAGE_I_PLAN.md` I4 section) — fork-covered at I6.1/I7.2, unit coverage at an I7-adjacent hook-test extension.
