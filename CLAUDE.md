...
**Last update:** 2026-06-04, **Stage I in progress** on branch `stage-i` (HEAD at the I-reframe doc-sweep close). **Stage H complete and tagged** at `stage-h-complete` (`2a33649`). **The I-reframe reckoning reframed Stage I:** per **OQ-25 / I-D14**, AuMT is the pool's own Balancer V3 **BPT** (branded), **not a separate token** — there is no `AuMT.sol`, no soulbound mirror. Stage I is now the oracle-free **recorder clock + hook dispatch** only; the value-weighted snapshot-voting view is **deferred to a Stage K `src/governance/VotingWeight.sol` reader** per I-D15. **New locks I-D14—I-D17:** I-D14 (AuMT = BPT; supersedes I-D1/I-D2/I-D3/I-D4/I-D11/I-D12, amends I-D5—I-D9), I-D15 (view deferred; `AuMT.sol` + `IAuMT.sol` deleted), I-D16 (hook one-shot `emissionRecorder` slot, not a ctor immutable), I-D17 (`IVotingWeight` forward stub + `VaultClassRegistry` full-rename fix-forward — `IAuMT` had a live Stage-G consumer). **I1 (per-pool refactor, I-D9), I2 (AureumTime constants, I-D10), I8 (gauge hook-gate, I-D13) stand unchanged** (all COMPLETE). **I3 (concrete `AuMT.sol`) TOMBSTONED** — built at I3.1—I3.8f, superseded, removed at I9. **I4 reframed** to recorder dispatch + clock; **I9 new family** (AuMT deprecation + IVotingWeight migration). The **doc reframe (I-reframe.2—3c8) is COMPLETE; the code phase is next.** **Vault bytecode untouched.** **I-D locks:** I-D1—I-D17 LOCKED (17 total). **Findings:** I10—I14 (`docs/STAGE_I_NOTES.md`). **Stage H complete state:** H-D1—H-D42 LOCKED; tag `2a33649`.
...
**Stage I position:** The **I-reframe doc sweep is COMPLETE** (I-reframe.2—3c8d); `STAGE_I_PLAN.md`, `STAGE_I_NOTES.md`, and this §11 now describe the AuMT=BPT clock-only architecture. **COMPLETE pre-reframe code, unchanged by the reframe:** I1 (EmissionDistributor per-pool refactor, I-D9; I1.1—I1.8e), I2 (AureumTime constants, I-D10; I2.1—I2.3e), I8 (gauge-eligibility hook-gate, OQ-24 / I-D13; I8.0a—I8.6 — `GaugeEligibility` requires the canonical `AureumFeeRoutingHook`; 49/49 unit + 71/71 fork). **I3 (concrete `AuMT.sol`, I3.1—I3.8f) is TOMBSTONED** — built, superseded by I-D14, removed at I9. **I4.1 `getHookFlags` bump landed** (`409f51a`); the rest of I4 is reframed (recorder dispatch + clock per I-D16) and not yet built. **No code has landed for the reframed design** — the doc sweep settled the plan; the code phase is next. **Decisions:** I-D1—I-D17 LOCKED (I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED, I-D5—I-D9 AMENDED, by I-D14). **Findings:** I10—I14. **Vault bytecode untouched.** Per-sub-step detail in `docs/STAGE_I_PLAN.md` (Completion Log + Decisions + I9 roadmap) and `docs/STAGE_I_NOTES.md` (I-D bodies + I10—I14 Findings).
...
- **`src/governance/VotingWeight.sol` value-weighted voting view (I-D15).** Deferred out of Stage I to Stage K: the `(qualified_AuMT_value × time_in_pool_capped)^(1/4→1/3)` reader (14-day cliff, 6-month on-ramp, F-9 era transition, gauged-only) needs a concrete `ITVLOracle` (OQ-22 / F-D3) + the snapshot-voting layer (Stage K). The `IVotingWeight` forward stub lands at I9.1; its `totalSupply()` veto-denominator semantics under the BPT model (I-D17) also lock at Stage K.

**Next sub-step: I9.1 — `src/governance/IVotingWeight.sol` (the code phase opens).** The reframe's first code action is the AuMT deprecation + IVotingWeight consumer-migration family (I9), which must precede the `git rm` of `AuMT.sol` / `IAuMT.sol`: **I9.1** `src/governance/IVotingWeight.sol` (NEW minimal forward stub — `governanceWeight(address)` + `totalSupply()`, not `is IERC20`, per I-D17) → **I9.2** `VaultClassRegistry.sol` full-rename re-point (Stage-G fix-forward; `auMT`→`votingWeight` etc.) → **I9.3** the two `MockAuMT`→`MockVotingWeight` doubles → **I9.4** `git rm src/token/AuMT.sol src/token/IAuMT.sol` + regression → **I9.5** close-of-family. Then **I4** (recorder dispatch + clock: `emissionRecorder` one-shot slot per I-D16 + `effectiveQualBlock` clock per I-D14 + `onAfterAddLiquidity` / `onAfterRemoveLiquidity` → `recordDeposit` / `recordWithdrawal`) → **I5** (recorder-clock unit tests, `RecorderClock.t.sol`) → **I6** (fork integration, `StageIIntegration.t.sol`) → **I7** (deploy script + close-of-stage). Entry mode: **Opus high** per §13 (live design on the IVotingWeight surface, the hook callback bodies, the EmissionDistributor clock math).
