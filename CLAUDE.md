...
**Last update:** 2026-04-28, **Stage E COMPLETE** at `stage-e-complete` (commit `41a7590`, 2026-04-28). Stage E delivered: `AureumWeightedPoolFactory` (peer-inherited `BasePoolFactory` per E-D11 — external `create()` override bypass prevention; `QualityGateUnsatisfied(uint256,uint256)` at `MIN_ERC4626_WEIGHT = 52e16`); pool-deployment framework — `script/pools/PoolConfig.sol` struct + abstract `MiliariumPoolDeployer` base + per-pilot deploy wrappers (per E-D23); three pilot Miliarium pools — **ixHelvetia** (slot 01, svZCHF 80% / sUSDS 20%), **ixEdelweiss** (slot 05, waEthUSDC 26% / svZCHF 26% / ixEDEL 48%), **ixAurebit** (slot 14, WBTC 16% / Aave Prime GHO 26% / cbBTC 16% / ixEDEL 16% / svZCHF 26%); fork harness `test/fork/PilotPools.t.sol` — `MiliariumPilotPoolBase` + 3 derived pilot test contracts (per E-D24, in-process pattern + `--threads 1` belt); 5 QG revert-path unit tests in `test/unit/AureumWeightedPoolFactory.t.sol` (per E-D13). Test suite: 156 unit + 12 fork = **168/168 green** via D35 split-form. Mid-stage supersessions — OQ-11 (Bodensee swap fee `0.75%` immutable, Miliarium genesis `0.02%`, band `0.01%–0.30%`), E-D6 → E-D24 (env-key suffix superseded by in-process pattern), E10 (forge-std `deal(adjust=true)` Reserve-DTF incompatibility — drop third arg), E11 (Aave Prime GHO naming locked). Vault bytecode untouched — zero edits to `lib/balancer-v3-monorepo/`. **Next: Stage F — CCB engine (Opus extra-high entry).**

**Branch:** `main` at the E9 docs commit (HEAD) following FF merge from `stage-e`; `stage-e-complete` lightweight tag at `main` HEAD~1 = `41a7590`. `stage-e` branch preserved on `origin` as a snapshot per the C0 / D0 / E-D8 convention.

Recent commit stack on `stage-e` through tag target `41a7590` (the E9 docs commit on `main` HEAD sits one above):

- `41a7590` — docs: STAGE_E_NOTES.md E-D26 amendment — NatSpec-hyperlink artifact reclassified as paste-rendering (E5 docs sweep)
- `301d67b` — docs: STAGE_E_PLAN.md — E10 mid-stage supersession (deal(adjust=true) / E-D7 superseded)
- `46858c0` — E5.1: docs/STAGE_E_PLAN.md — Completion Log fill (E0 through ca24e5c, 36 commits)
- `ca24e5c` — docs: CLAUDE.md §11 refresh — Stage E mid-stage (E1+E2+E3+E4 closed, E5 docs / E9 close ahead)
...
Earlier Stage E commits (E0 scaffold, E1.1–E1.2 framework + ixHelvetia, E2 ixEdelweiss configs + IxEdelweissPilotTest, E3.1–E3.2 ixAurebit configs + DeployIxAurebit, OQ-11 supersession landings, E-D24 supersession of E-D6, E10 / E11 NOTES) live in `git log stage-e ^57cd2ce` (branch point) up to `41a7590`. Full Completion Log in `docs/STAGE_E_PLAN.md`.

**Current tag:** `stage-e-complete` (commit `41a7590`, 2026-04-28). Next expected tag: `stage-f-complete` at Stage F close. `stage-d-complete` (`951c338`, 2026-04-25), `stage-c-complete` (`5342126`, 2026-04-18), `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage E position:** **COMPLETE.** Sub-step roll-up: E0 scaffold → E1 framework + ixHelvetia (slot 01) → E2 ixEdelweiss (slot 05, E-D25 vector) → E3 ixAurebit (slot 14, WBTC → svZCHF fee-routing fork test) → E4 QG revert-path unit tests → E5 docs (Completion Log fill, E10 supersession, E-D26 amendment) → E9 close (FF merge to `main`, tag `stage-e-complete`, push). Per-sub-step detail in `docs/STAGE_E_PLAN.md` Decisions table (E-D1 through E-D26 entry-time + mid-stage supersessions OQ-11 / E-D6 → E-D24) + `docs/STAGE_E_NOTES.md` E-D-numbered findings (notably **E-D11** factory peer-inherit pattern, **E-D13** QG unit-test scope, **E-D23** deploy script architecture, **E-D24** in-process harness shape, **E-D26** NatSpec-hyperlink paste-rendering reclassification) and E-numbered findings (**E10** `deal(adjust=true)` Reserve-DTF, **E11** Aave Prime GHO naming).

**Deferred (post-Stage E carry-forward):**

- **OQ-20 / OQ-21.** Controller yield-fee entry point + bi-weekly cadence; per E-D10 not Stage-E-resolved. Per-token `paysYieldFees: true` flags in pilot configs are forward-compatible with the eventual controller entry point and cadence.
...
- **D36 permanent fix.** Env-key namespacing in `test/fork/DeployAureumVault.t.sol` (or eliminating `vm.setEnv` from fork tests entirely); Stage E pilot tests are race-safe via E-D24 in-process pattern + same-value env-write convention; `--threads 1` cross-suite belt remains for legacy Stage D fork tests writing `FEE_ROUTING_HOOK` to conflicting addresses.
- **aumm-site spec edits (user-side).** `04_tokenomics.md` §ix prose amendments per OQ-20 + OQ-21; not a repo edit.

**How to resume (Stage F → F0 entry):**

1. Confirm tip: `git log --oneline -3 main` shows the E9 docs commit at HEAD; `git show stage-e-complete --stat` resolves to `41a7590` at HEAD~1.
