...
**Last update:** 2026-05-23, **Stage H in close-out** on branch `stage-h` (HEAD `2ff10c5`). H1—H6 implementation families + H6.4 close-of-family sweep complete (per prior CLAUDE.md L413 H6.4f snapshot at `ef4b9be`); **H9 fork integration test family complete** (StageHIntegrationFixture + 4 derived suites StageHBootstrapPhaseTest accrue() + distribute() cohorts + StageHContinuousPhaseTest + StageHHalvingBoundaryTest + StageHCrossStackTest; 10/10 fork green at H-D40 canonical invocation `forge test --match-path "test/fork/StageHIntegration.t.sol" --fork-url $MAINNET_RPC_URL --threads 1 -vv`); **H9.7 close-of-family sweep complete** (a PLAN Completion Log catch-up `ed5dc38` + b NOTES Findings queue H10/H11/H12 `dd21f3f` + c PLAN status + mode refresh `9db4187` + d CLAUDE.md §11 refresh `aecb57b` + e PLAN Completion Log tail `2b51d89`); **H10 family complete** — H-D7 LOCKED Option C (AuMM.setMinter handoff deferred to Stage K; no setMinter call at Stage H deploy; H-D41 multi-minter architecture carry-forward to Stage K; H-D42 LOCKED reframing AuMM from Stage H deployment artifact to Stage H input per BodenseeBootstrapChannel constructor `_vault.getPoolTokens(BODENSEE_POOL)` AuMM-index lookup chicken-and-egg surfaced at H10.2 first-attempt revert); `script/DeployStageH.s.sol` (0 → 224 lines across H10.1 `90f9a82` first cut + H10.1-fix1 `c04ee0d` H-D42 revision) consumes `vm.envAddress("AUMM")` per `DeployDerBodensee.s.sol:L39` convention; `test/fork/DeployStageH.t.sol` (0 → 219 lines at H10.2-revised `2112f09`) via `StageGIntegrationFixture` inheritance — 1/1 fork green in 18.44s; **H8 unit-test confirmation pass complete** — 612/612 unit green across 22 suites (19.17s wall) confirms no regression from H9/H10; no code changes (H6.2a-fix `StageGMocks.sol` view→pure at `8da816e` was the sole code-side touch surfaced at H10.2-revised forge build); **H11 close-of-stage in flight** — H11.0a NOTES H13 Findings entry `fb2d9b1` + H11.0b PLAN Completion Log catch-up `2ff10c5` (HEAD) + H11.0c CLAUDE.md §11 refresh (this commit); H12 fast-forward merge + `stage-h-complete` tag pending. **H7 consolidated into H5.2** per H-D29 (`src/incendiary/IIncendiaryRegistry.sol` landed at H5.2 commit `c4cfe7f`; the H7 plan-row is preserved as a Stage L surface-refinement slot but ships no new code). **H8 was reordered post-H9** per chat-level decision — the 612/612 unit cohort across 19 unit `.t.sol` files already covered all H1—H6 surfaces and absorbed H9/H10 changes; H8 ran after H9 fork validation + H10 deploy-script family rather than between H6 and H9. Branch carries 182 sub-step commits beyond `stage-g-complete` (`384a40b`). **Concrete H10 deliveries:** `script/DeployStageH.s.sol` (224 lines) — AuMM consumed via `vm.envAddress("AUMM")` at Step 0; `genesisBlock` derives from `aumm.GENESIS_BLOCK()` (single source of truth, no GENESIS_BLOCK env var); TVLOracle + EfficiencyOracle + BodenseeBootstrapChannel + EmissionDistributor in-script deploy; deployer→GOVERNANCE_MULTISIG handoff per `Ownable2Step` convention; `setMinter` call omitted per H-D7 Option C with `MinterNotZero` invariant reframed as input-precondition check on the AUMM env input; `test/fork/DeployStageH.t.sol` (219 lines) — `is StageGIntegrationFixture` per H13 lesson (the fork test inherits Stage G's real Vault + AuMM + Bodensee + gauge stack rather than stubbing env vars whose receivers make external calls in their constructors); single `test_DeployStageH()` with 11 assertions covering AuMM slot pinning + setMinter zero-precondition + 4 governance handoffs (TVLOracle / EfficiencyOracle / BodenseeBootstrapChannel / EmissionDistributor) + emissionsRecorder wiring + BODENSEE_POOL pinning + GENESIS_BLOCK derivation match + positive `setMinter(address(emissionDistributor))` proof from `_minterAdmin = address(this)` (test contract = fixture's `_minterAdmin`, no `vm.prank` needed); 1/1 fork green in 18.44s at H-D40 canonical invocation. **Test cohort: 612/612 unit green** across 22 suites (Stage H 225: 36 TVLOracle + 56 EfficiencyOracle + 29 BodenseeBootstrapChannel + 100 EmissionDistributor + 4 AuMMDistributorIntegration; Stage G/F/E/D/C 387) **+ 10/10 fork green** for Stage H integration cohort (4 derived suites) **+ 1/1 fork green** for DeployStageH script; combined under D35 split-form invocation per H-D40 + F-D11 `--threads 1` belt. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **H-D locks status:** **H-D1—H-D42 LOCKED (39 total at H11.0c)** — full lock at H10 family close; **H-D7** LOCKED Option C at H10.0a `db75c6b` (defer setMinter to Stage K; no setMinter at Stage H deploy; accrue/recordScore/recordDeposit/recordWithdrawal bookkeeping accumulates from GENESIS_BLOCK; distribute()/claim() revert NotMinter pre-Stage-K); **H-D41** Multi-minter architecture carry-forward to Stage K (deploys EmissionDistributor as initial sole minter then transitions to N-minter slot per future K-D); **H-D42** AuMM as Stage H input LOCKED at H10.0c-fix `2af6c8d` (DeployStageH.s.sol consumes AUMM env per DeployDerBodensee.s.sol:L39 convention; resolves BodenseeBootstrapChannel constructor `_vault.getPoolTokens(BODENSEE_POOL)` chicken-and-egg surfaced at H10.2 first-attempt revert). **`docs/STAGE_H_NOTES.md` Findings queue (4 entries):** H10 + H11 + H12 landed at H9.7b `dd21f3f` (per prior H9.7d snapshot); **H13 — Deploy-script fork tests cannot stub env vars whose receivers make external calls in their constructors; StageGIntegrationFixture inheritance is the resolution** landed at H11.0a `fb2d9b1` (root cause: keccak placeholders work for ZeroAddress-validated immutable slots but fail for any constructor that makes an external call to its argument; resolution: H-D42 + StageGIntegrationFixture inheritance; default rule: audit each consumed contract's constructor body for external calls before drafting a fork-test §8e.1 for any multi-contract deploy script).

**Branch:** `stage-h` at H11.0b commit `2ff10c5` (HEAD). Branched from `stage-g-complete` (`384a40b`) per the **C0 / D0 / E-D8 / F9 / G9** convention; `main` remains at `stage-g-complete` HEAD until Stage H close-of-stage merge at H12. `stage-h` snapshot push to `origin` deferred to H12.

Recent commit stack on `stage-h` (top of HEAD downward; full Completion Log in `docs/STAGE_H_PLAN.md`):

- `2ff10c5` — H11.0b: docs/STAGE_H_PLAN.md — H10.3 hash backfill (<pending> → e19b737) + H8/H11.0a Completion Log rows + H11.0b self-row + status header + Last update + Mode refresh (264 → 267 lines)
- `fb2d9b1` — H11.0a: docs/STAGE_H_NOTES.md — H13 Findings entry: deploy-script fork tests cannot stub env vars whose receivers make external calls in their constructors; StageGIntegrationFixture inheritance is the resolution (H10.2 first-attempt revert + H-D42 chicken-and-egg; 318 → 338 lines)
- `e19b737` — H10.3: docs/STAGE_H_PLAN.md — close-of-family sweep: H10.0d-fix hash backfill (<pending>→11bf56d; 256→260 correction) + H10.1-fix1/H10.2-revised/H6.2a-fix Completion Log rows + status header + Last update + Mode refresh (260 → 264 lines)
- `8da816e` — H6.2a-fix: test/fork/mocks/StageGMocks.sol — MockAuMT distributor()/pool() view → pure (both return address(0) constants; Solc 0.8.26 Warning(2018) cleanup surfaced at H10.2-revised forge build; -2 warnings)
- `2112f09` — H10.2-revised: test/fork/DeployStageH.t.sol — fork-test validates DeployStageH.s.sol (H10.1-fix1) via StageGIntegrationFixture inheritance: real Vault + AuMM + Bodensee + gauge stack; 11 assertions (H-D7 Option C minter slot + Step 7 governance handoffs + Step 6 emissionsRecorder wiring + BODENSEE_POOL + GENESIS_BLOCK derivation) + positive setMinter proof; 1/1 fork green in 18.44s (219 lines)
- `c04ee0d` — H10.1-fix1: script/DeployStageH.s.sol — H-D42 revision: AuMM consumed via AUMM env var (not deployed in-script); genesisBlock derives from aumm.GENESIS_BLOCK(); MinterNotZero invariant reframed as input-precondition check; NatSpec env-list drops GENESIS_BLOCK, adds AUMM (191 → 224 lines)
- `11bf56d` — H10.0d-fix: docs/STAGE_H_PLAN.md — L3 status header + L5 Last update + L7 Mode + L35 H10 skeleton refresh + L50 H-D7 cross-ref to H-D42 + H-D42 Decisions row + H10.0b hash backfill + H10.1/H10.0c-fix/H10.0d-fix Completion Log rows (256 → 260 lines)
- `2af6c8d` — H10.0c-fix: docs/STAGE_H_NOTES.md — H-D42 LOCKED (AuMM as Stage H input per H10.2 first-execution revert at 90f9a82; DeployStageH.s.sol consumes AUMM env per DeployDerBodensee.s.sol:L39 convention; GENESIS_BLOCK derives from aumm.GENESIS_BLOCK(); 322 → 326 lines)
- `90f9a82` — H10.1: script/DeployStageH.s.sol — Stage H deployment script first cut (AuMM + TVLOracle + EfficiencyOracle + BodenseeBootstrapChannel + EmissionDistributor; deployer→GOVERNANCE_MULTISIG handoff; no setMinter per H-D7 Option C; MinterNotZero invariant assertion; 191 lines); subsequently revised at H10.1-fix1 per H-D42
- `c4b747a` — H10.0b: docs/STAGE_H_PLAN.md — H-D7 row OPEN→LOCKED + H-D41 row + H10 sub-step skeleton expansion + status header + Last update + Mode refresh (253 → 256 lines)
- `db75c6b` — H10.0a: docs/STAGE_H_NOTES.md — H-D7 LOCKED Option C (defer setMinter to Stage K; no setMinter at Stage H deploy) + H-D41 Multi-minter architecture carry-forward (318 → 322 lines)
- `2b51d89` — H9.7e: docs/STAGE_H_PLAN.md — Completion Log tail: H9.7b/H9.7c/H9.7d rows + H9.7e self-row (249 → 253 lines)
- `aecb57b` — H9.7d: CLAUDE.md — §11 resume-anchor refresh (H9 fork integration test family complete 10/10; H9.7 sweep in flight; H8 reordered post-H9; 169 commits; H-D1—H-D6+H-D8—H-D40 37 locks; H10/H11/H12 Findings queue)

Earlier Stage H commits (H0—H9.7d chain) live in `git log stage-h ^stage-g-complete` between branch point `384a40b` and `2ff10c5` HEAD (182 commits total at H11.0b). Full Completion Log in `docs/STAGE_H_PLAN.md`.

**Current tag:** `stage-g-complete` remains the latest stage tag (commit `384a40b`, 2026-05-14). Next expected tag: `stage-h-complete` at Stage H close-of-stage at H12. `stage-f-complete`, `stage-e-complete`, `stage-d-complete`, `stage-c-complete`, `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage H position:** **H1—H6 implementation families + H6.4 close-of-family sweep complete; H9 fork integration test family complete (StageHIntegrationFixture + 4 derived suites; 10/10 fork green at H-D40 canonical invocation); H9.7 close-of-family sweep complete (a/b/c/d/e committed); H10 family complete (H-D7 LOCKED Option C; H-D41 carry-forward to Stage K; H-D42 LOCKED AuMM as Stage H input; `script/DeployStageH.s.sol` 224 lines + `test/fork/DeployStageH.t.sol` 219 lines; 1/1 fork green); H8 unit-test confirmation pass complete (612/612 green across 22 suites in 19.17s; no code changes); H7 consolidated into H5.2; H11 close-of-stage in flight (a/b committed, c this commit); H12 fast-forward merge + tag pending.** Sub-step roll-up from H9.7d forward (H1—H9.7d detail in prior CLAUDE.md §11 snapshot at `aecb57b`): → **H9.7e PLAN Completion Log tail** (H9.7b/H9.7c/H9.7d rows + H9.7e self-row; 249 → 253 lines) → **H10.0a NOTES H-D7 LOCKED Option C + H-D41 Multi-minter architecture carry-forward** (defer setMinter to Stage K; no setMinter at Stage H deploy; H-D41 Stage K deploys EmissionDistributor as initial sole minter then transitions to N-minter slot; 318 → 322 lines) → **H10.0b PLAN H-D7 row OPEN→LOCKED + H-D41 row + H10 sub-step skeleton expansion** (253 → 256 lines) → **H10.1 `script/DeployStageH.s.sol` first cut** (191 lines; AuMM + TVLOracle + EfficiencyOracle + BodenseeBootstrapChannel + EmissionDistributor in-script deploy; deployer→GOVERNANCE_MULTISIG handoff; no setMinter per H-D7 Option C; MinterNotZero invariant assertion) → **H10.2-first-attempt `test/fork/DeployStageH.t.sol` REVERTED** on `call to non-contract address 0x6076…DFFb` (VAULT keccak placeholder) — surfaced AuMM-vs-Bodensee chicken-and-egg per BodenseeBootstrapChannel constructor `_vault.getPoolTokens(BODENSEE_POOL)` AuMM-index lookup (H-D12); root cause becomes H13 Findings entry → **H10.0c-fix NOTES H-D42 LOCKED** (AuMM as Stage H input; DeployStageH.s.sol consumes AUMM env per DeployDerBodensee.s.sol:L39 convention; 322 → 326 lines) → **H10.0d-fix PLAN H-D42 row + status/mode refresh + Completion Log catch-up** (256 → 260 lines) → **H10.1-fix1 `script/DeployStageH.s.sol` H-D42 revision** (remove `new AuMM` Step 1; add `AuMM(vm.envAddress("AUMM"))` env-read; derive genesisBlock from `aumm.GENESIS_BLOCK()`; preserve MinterNotZero invariant as input precondition; 191 → 224 lines) → **H10.2-revised `test/fork/DeployStageH.t.sol`** (`is StageGIntegrationFixture` — inherits real Vault + AuMM + Bodensee + gauge stack; 11 assertions covering minter slot + governance handoffs + wiring + pinning + GENESIS_BLOCK derivation; positive setMinter proof; 1/1 fork green in 18.44s; 219 lines) → **H6.2a-fix `test/fork/mocks/StageGMocks.sol`** (MockAuMT distributor/pool view→pure; -2 Solc Warning(2018) warnings; surfaced at H10.2-revised forge build) → **H10.3 PLAN close-of-family sweep** (H10.0d-fix hash backfill + H10.1-fix1/H10.2-revised/H6.2a-fix Completion Log rows + status/mode refresh; 260 → 264 lines) → **H8 unit-test confirmation pass** (no commit; `forge test --no-match-path "test/fork/**"` 612/612 green across 22 suites in 19.17s; confirms no regression from H9/H10 changes) → **H11.0a NOTES H13 Findings entry** (deploy-script fork tests cannot stub env vars whose receivers make external calls in their constructors; StageGIntegrationFixture inheritance is the resolution; 318 → 338 lines) → **H11.0b PLAN Completion Log catch-up** (H10.3 hash backfill `<pending>` → `e19b737` + H8/H11.0a Completion Log rows + H11.0b self-row + status/mode refresh; 264 → 267 lines) → **H11.0c CLAUDE.md §11 refresh — this commit**. Per-sub-step detail in `docs/STAGE_H_PLAN.md` Completion Log + Decisions table (now H-D1—H-D42 LOCKED — 39 total at H11.0c) and `docs/STAGE_H_NOTES.md` H-D-numbered bodies (notably **H-D7** Option C defer setMinter to Stage K; **H-D41** multi-minter architecture carry-forward to Stage K; **H-D42** AuMM as Stage H input). **H-D7 LOCKED at H10.0a.**

**Deferred (Stage G carry-forward + Stage H carry-forward):**

- **H-D41 multi-minter architecture carry-forward to Stage K.** Per H10.0a `db75c6b`: Stage K deploys EmissionDistributor as initial sole minter at K-x then transitions to N-minter slot per future K-D; AuMM.setMinter handoff sequenced at Stage K rather than Stage H per H-D7 Option C; this means `accrue` / `recordScore` / `recordDeposit` / `recordWithdrawal` bookkeeping accumulates from GENESIS_BLOCK pre-Stage-K but `distribute()` / `claim()` revert `NotMinter` until Stage K setMinter call lands.
- **`script/DeployAuMM.s.sol` production deploy chain.** Separate micro-script (~30 lines) for AuMM deployment positioned before `DeployDerBodensee.s.sol` and `DeployStageH.s.sol` in the production deployment sequence; per H-D42 convention (AuMM as Stage H input), AuMM lives upstream of both Bodensee and emission deploys; deferred to later H-x or Stage K opening.
...
- **D36 permanent fix.** Env-key namespacing in `test/fork/DeployAureumVault.t.sol` (or eliminating `vm.setEnv` from fork tests entirely); Stage E + Stage F + Stage G + Stage H fork tests race-safe via E-D24 in-process pattern + same-value env-write convention + `--threads 1` belt per F-D11; permanent namespacing fix carried forward.
...
**Next sub-step: H12 — Stage close.** Fast-forward merge `stage-h` → `main` (no merge commit since `main` is at `stage-g-complete` and `stage-h` is a strict descendant per the **C0 / D0 / E-D8 / F9 / G9** convention); tag `stage-h-complete` at HEAD; push tag + preserved `stage-h` snapshot to `origin`. Then opens entry to **Stage I — AuMT** (per-pool LP-receipt; H-D35 in-place extension reframe of `src/token/IAuMT.sol` was already landed at H6.0c per Stage G's IAuMT skeleton — Stage I implementation builds on that contract). Entry per §13 Stage-level defaults: **Sonnet for H12** (mechanical merge + tag + push; user runs `git merge --ff-only` + `git tag` + `git push` in their own terminal per §8). Stage I entry per §13 defaults: **Opus high entry** (LP-receipt interface design + per-pool accumulator wiring) then mostly Sonnet (test writing after design is set).
