...
**Next sub-step: K7 pre-flight** — the Opus beat that locks **K-D9** (deploy + authorizer migration), the final Stage K unit and the actual governance handoff. K7 ships `script/DeployAuMM.s.sol` (H-D42, ~30 lines; AuMM reframed as a Stage K *input* per the H13 chicken-and-egg resolution) + `script/DeployStageK.s.sol` (deploy the governance stack, the multisig one-shot `Vault.setAuthorizer`, registry + gauge `setGovernanceContract` one-shots, and the K5 minter-router + `AuMM.setMinter` wiring) + `test/fork/DeployStageK.t.sol`. The pre-flight locks the K-D9 migration transaction order per OQ-10 (FINDINGS L788—792), sizes the fork-test fixture against the H13 constructor-external-call rule (deploy-script receivers that call their args in-constructor need real fork state), and folds in the K5 minter-router and K6 `TVLOracle`-roster transition coverage deferred to K7. K7 reads K-D9 (`docs/STAGE_K_NOTES.md`), H-D42 (`docs/STAGE_H_NOTES.md`), OQ-10 (`docs/FINDINGS.md`), and the `DeployDerBodensee.s.sol` / `DeployStageH.s.sol` script precedents. **Stage K mode: Opus extra-high** per §13 — governance handoff, stay on Opus through the stage; K7 opens with its own Opus pre-flight.
