...
**Last update:** 2026-04-23, mid-Stage D. **Gate 3 complete** — STAGE_D_PLAN.md §D7.1 stub is now a full sub-step enumeration (D7.1a `setUp` scaffold + six named test bodies; D7.1b–D7.1g per-test implementations; D7.1z `.env.example` Stage D constants sweep). **D7.1a §8e.1 is the next chat work.** Three commits landed since the §11 stack pin at `fab8fa3`: `8197aaa` (CLAUDE.md §8e.1 text-fence wrapping clause), `43bf8b9` (Gate 3 itself), `5f3b4fc` (cursorrules USER VERIFY hard-stop sentinel). Stage C remains complete at `stage-c-complete` (commit `5342126`). D0 → D6 + D7.0 + Gate 2 + Gate 3 all on `stage-d`.

**Branch:** `stage-d` at `5f3b4fc` on top of the following stack:
...
- **Gate 3 — complete (2026-04-23, `43bf8b9`).** STAGE_D_PLAN.md §D7.1 stub fleshed out into a full sub-step enumeration. **D7.1a** = `setUp` scaffold + six empty test bodies (`test_Fork_WPFBoundToAureumVault`, `test_Fork_BodenseeYieldCollectionReverts`, `test_Fork_WithdrawProtocolFeesRecipientCheck`, `test_Fork_RouteYieldFeePrimitive`, `test_Fork_RecursionGuard`, `test_Fork_SwapRoutesFeeToBodensee`). **D7.1b–D7.1g** = per-test implementations, one §8e.1 per test in the order listed. **D7.1z** = `.env.example` Stage D constants block (six vars per D-D23, omitting `TRADING_POOL_SALT` as test-only). Two commit boundaries per plan: `D7.1: test/fork/AureumFeeRoutingHook.t.sol — setUp scaffold + 6 fork tests` (after D7.1g) and `D7.1z: .env.example — Stage D constants (AUREUM_VAULT, WEIGHTED_POOL_FACTORY, AUMM, SV_ZCHF, SUSDS, BODENSEE_SALT)` (after D7.1z). One open question surfaced and deferred to D7.1g write-time: **OQ-22** candidate, mainnet Balancer V3 Router address — `BALANCER_V3_ROUTER` env vs file-level test constant. Not yet in FINDINGS.md.
- D7.1 — next. **D7.1a §8e.1** is the next chat work — fork-harness `setUp` scaffold per D-D21 pre-compute chain, real AuMM per D-D20, AuMM/svZCHF 50/50 trading pool per D-D22, `test/fork/AureumFeeRoutingHook.t.sol` per D-D19; six empty test bodies. D7.1b–D7.1g + D7.1z to follow, one §8e.1 per sub-step.
- D8, D9 — not started.
...
- **OQ-22 / D7.1g.** Mainnet Balancer V3 Router address — `BALANCER_V3_ROUTER` env vs file-level test constant; pinned at D7.1g write-time per Gate 3 plan. Surfaced during Gate 3 §D7.1g flesh-out; not yet logged in FINDINGS.md.
...
**How to resume (Stage D — D7.1a fork-harness setUp scaffold):**

1. Confirm current tip: `git log --oneline -1` on `stage-d` matches `5f3b4fc` (cursorrules USER VERIFY hard-stop). `git log --oneline -13` shows the full stack from `5f3b4fc` back to `d65a576`.
2. Read: STAGE_D_PLAN.md §D7 + §D7.1 (now full enumeration, no longer stub) + decisions table D-D19 through D-D23; STAGE_D_NOTES.md D29 + D30 + **D31**; FINDINGS.md OQ-20 + OQ-21.
3. Author **D7.1a §8e.1** for the fork-harness `setUp` scaffold + six empty test bodies. File: `test/fork/AureumFeeRoutingHook.t.sol` (new, per D-D19). Scaffold-only this beat — no test bodies; D7.1b through D7.1g implement them one §8e.1 each. Opus-high per §13.
4. After D7.1a commits, author D7.1b §8e.1 (`test_Fork_WPFBoundToAureumVault` — single assertion); then D7.1c through D7.1g in order, each its own §8e.1.
5. After D7.1g commits, author D7.1z §8e.1 for `.env.example` Stage D constants block (six vars per D-D23). Two commit boundaries per Gate 3 plan: scaffold+tests under one message, `.env.example` under another.
6. Loop grep-and-confirm per §6 / §8e Audit cycle; all git mutations run in user's terminal; Cursor is executor-only per §7 + §8e.
