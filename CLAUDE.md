...
**Last update:** 2026-05-26, **Stage I in progress** on branch `stage-i` (HEAD `bac5ff3`). **Stage H complete and tagged** at `stage-h-complete` (commit `2a33649`) — fast-forward merged to `main` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. **Stage I — AuMT (Aureum Market Tessera)** open since 2026-05-23: per-pool LP-receipt token (soulbound ERC-20, 18 decimals, one contract per Miliarium pool — 3 instances at Stage I pilot, 28 at full deployment per Stages M/N). I-D1—I-D10 design decisions LOCKED at I0.0a (`9409d76`) + I0.0b (`1404b2d`) per `docs/STAGE_I_NOTES.md`; PLAN sub-step roadmap I0—I7 (45 sub-steps + I-D1—I-D10 Decisions table mirror + Completion log) seeded at I0.0c (`984456a`). **I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE** across I1.1—I1.8e: I-D9 LOCKED Option A (per-pool mapping replaces single-slot `auMTContract`; `setAuMTContractForPool(pool, addr)` one-shot setter with `ZeroAddress` + `AuMTAlreadyBound` guards; `onlyAuMTContract(pool)` modifier); interface signature updates at I1.1 (`5ff9072`) + I1.1-fix1 (`46a93c7`); implementation at I1.2 (`c3147c1`) + I1.2-fix1 (`1f9c74f`); test file refactors at I1.3—I1.6 (8 tests updated + AuMTAlreadyBound new test 100→101; fixture expansion to 3 pilot pools; deploy fork (3b-bis) assertion); I1.7 regression 624/624 green (613 unit + 10 fork integration + 1 deploy fork); I1.8 close-of-family sweep landed (`44a94bb` NOTES I10 + `c33ea30` PLAN catch-up + `6cf219f` status refresh + `be09f35` CLAUDE.md + `f978e58` PLAN tail). **I2 — AureumTime constants extension (per I-D10) COMPLETE** across I2.1—I2.3c: I2.1 (`783bdf6`) added `QUALIFICATION_PERIOD_BLOCKS = BLOCKS_PER_EPOCH` + `ON_RAMP_PERIOD_BLOCKS = 180 * BLOCKS_PER_DAY` constants (single-source-of-truth named-constant RHS; 83→91 lines); I2.2 (`552300c`) added 4 I-D10 tests (2 value-assertion + 2 SSoT; 178→196 lines; 37/37 AureumTime green via `forge clean && forge test`); I2.3a—I2.3c close-of-family sweep landed (`f143b7a` NOTES I11 Findings + `b651676` PLAN catch-up + `bac5ff3` PLAN status refresh). **I2.3d CLAUDE.md §11 refresh** (this commit) + **I2.3e PLAN Completion Log tail** pending. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **I-D locks status:** I-D1—I-D10 LOCKED (full 10-decision lock at I0.0b). **`docs/STAGE_I_NOTES.md` Findings queue (2 entries):** I10 — multi-file rename refactor NatSpec drift (intra-file old-symbol grep rule) at I1.8a `44a94bb`; I11 — Foundry `forge test --match-test` Rust regex (`\|` literal pipe; bare `|` for alternation; prefer `--match-path`; F14 pairing) at I2.3a `f143b7a`. **Stage H complete state:** H-D1—H-D42 LOCKED (39 total); stage tag at `2a33649`; full Completion Log in `docs/STAGE_H_PLAN.md`.

**Branch:** `stage-i` at I2.3c commit `bac5ff3` (HEAD). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.
...
**Stage I position:** **I0 design freeze + PLAN creation complete (I-D1—I-D10 LOCKED); I1 — EmissionDistributor per-pool refactor (per I-D9; foundation) COMPLETE across I1.1—I1.8e (interface + implementation + 4 test files + regression 624/624 green + close-of-family sweep a/b/c/d/e all committed); I2 — AureumTime constants extension (per I-D10) COMPLETE across I2.1—I2.3c (constants + tests + close-of-family sweep a/b/c committed; 37/37 AureumTime green); I2.3d CLAUDE.md §11 refresh this commit; I2.3e PLAN Completion Log tail pending.** Sub-step roll-up: → I0.0a/b/c (design + PLAN seed; I-D1—I-D10 LOCKED) → I1.1—I1.1-fix1 IEmissionDistributor.sol interface signature updates + stale L73 NatSpec fix → I1.2—I1.2-fix1 EmissionDistributor.sol per-pool mapping + stale NatSpec fixes → I1.3 EmissionDistributor.t.sol 8 tests updated + AuMTAlreadyBound test → I1.4 AuMMDistributorIntegration.t.sol setUp update → I1.5 StageHIntegration.t.sol fixture expansion (3 pilot pools) → I1.6 DeployStageH.t.sol new (3b-bis) assertion → I1.7 regression 624/624 green → I1.8a—I1.8e close-of-family sweep (NOTES I10 + PLAN catch-up + status refresh + CLAUDE.md + PLAN tail) → I2.1 AureumTime.sol `QUALIFICATION_PERIOD_BLOCKS` + `ON_RAMP_PERIOD_BLOCKS` constants → I2.2 AureumTime.t.sol 4 I-D10 tests → I2.3a NOTES I11 Findings entry → I2.3b PLAN Completion Log catch-up → I2.3c PLAN status / Last update / Mode refresh → I2.3d CLAUDE.md §11 refresh — this commit. Per-sub-step detail in `docs/STAGE_I_PLAN.md` Completion Log + Decisions table (I-D1—I-D10 LOCKED — 10 total at I2.3d) and `docs/STAGE_I_NOTES.md` I-D-numbered bodies + I10/I11 Findings.
...
**Next sub-step: I2.3e — PLAN Completion Log tail.** Append rows for I2.3b/I2.3c/I2.3d + I2.3e self-row to `docs/STAGE_I_PLAN.md` Completion log table. After I2.3e the I2 close-of-family sweep is fully landed and Stage I I3 entry opens (AuMT.sol concrete implementation — soulbound ERC-20 state machine + I-D7 root-curve `governanceWeight` formula + I-D6 qualification clock + I-D4 onlyLiquidityHook access + I-D1 `NotTransferable` reverts). Entry per §13 Stage-level defaults: **Opus high for I3** (novel contract authoring; I-D6 qualification clock state machine + I-D7 root-curve formula with first-halving-block F-9 transition; Opus drafts I3.x §8e.1 prompts and audits Cursor's saves). I4 (FeeRoutingHook extension per I-D5) follows after I3 close-of-family.
