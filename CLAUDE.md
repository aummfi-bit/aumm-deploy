...
**Next sub-step: I5.1 — first-deposit clock-set test in `test/unit/RecorderClock.t.sol`.** I4 (recorder dispatch + clock) is COMPLETE (I4.1—I4.7). The code phase continues with the I5 recorder-clock unit tests against the `EmissionDistributor` `effectiveQualBlock` clock (I4.3) + the hook-gate (I-D9): **I5.1** first-deposit clock-set — `recordDeposit(pool, lp, amount)` from the bound recorder sets `effectiveQualBlock[pool][lp]` to `block.number` when previously zero, `userLP` accrues → **I5.2** weighted-average top-up (a second deposit blends `effectiveQualBlock` by the deposit-weighted average per I-D14) → **I5.3** reset-on-any-withdrawal (`recordWithdrawal` of any nonzero amount zeros `effectiveQualBlock` per §viii) → **I5.4** hook-gate access (`recordDeposit`/`recordWithdrawal` revert `NotAuMTContract(pool, caller)` for a non-bound caller; happy-path admits the address bound via `setAuMTContractForPool(pool, hook)` per I-D9) → **I5.5** close-of-family sweep. New test file `test/unit/RecorderClock.t.sol` — pre-flight the `EmissionDistributor` constructor wiring + the `setAuMTContractForPool` bind pattern (mirror the existing `EmissionDistributor.t.sol` harness). Per I11/F14: prefer `--match-path "test/unit/RecorderClock.t.sol"` for the verify and `forge clean &&` if the incremental cache reports 'No files changed'. Entry mode: **Opus high** per §13 for the test-harness design (weighted-average + over-qualification vectors), dropping to **Sonnet** for repetitive test-case bodies. Also pending: the **Deferred-from-I4** `setEmissionRecorder` unit-level two-flag-lock tests (see `STAGE_I_PLAN.md` I4 section) — fork-covered at I6.1/I7.2, unit coverage at an I7-adjacent hook-test extension.
