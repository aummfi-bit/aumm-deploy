...
**Last update:** 2026-06-05, **Stage I code phase open** on branch `stage-i` (HEAD at I7.3d, the §11 resume-anchor refresh closing the I7.3 close-of-family sweep). **Stage H complete and tagged** at `stage-h-complete` (`2a33649`). **I9 (AuMT deprecation + IVotingWeight migration), I4 (recorder dispatch + clock), I5 (recorder-clock unit tests), and I6 (recorder-clock fork integration) are all COMPLETE; I7.1 + I7.2 (DeployStageI deploy script + fork test, 1/1 fork) COMPLETE, I7.3 close-of-family sweep done.** Per **OQ-25 / I-D14**, AuMT is the pool's own Balancer V3 **BPT** (branded), **not a separate token**; Stage I is the oracle-free **recorder clock + hook dispatch** only; the value-weighted snapshot-voting view is **deferred to a Stage K `src/governance/VotingWeight.sol` reader** per I-D15. **I6 shipped:** `test/fork/StageIIntegration.t.sol` — `StageIIntegrationFixture` (Option B own emission stack, recorder gate bound to the canonical `AureumFeeRoutingHook` via `setEmissionRecorder` + `setAuMTContractForPool` ×3) + `StageIWiringTest` (I6.1 `59ba9a3`), deposit-dispatch `onAfterAddLiquidity` → `recordDeposit` (I6.2 `b3dd12c`), cross-block weighted-average top-up blend (I6.3 `22115ed` → I6.3-fix1 `ee71fe7`), aging + withdrawal-reset (I6.4 `8a6394d`); **4/4 fork green**. **I6-DIAG** throwaway bisection disproved a reconstructed fork `block.number` quirk — the recorder clock is correct (Finding I18; I6.5a `3d7f62d`); close-of-family sweep I6.5a—d. **Locks:** I-D14 (AuMT = BPT), I-D15 (view deferred), I-D16 (hook one-shot `emissionRecorder` slot, not a ctor immutable), I-D17 (`IVotingWeight` stub + `VaultClassRegistry` rename), I-D18 (DeployStageI wiring GOVERNANCE_MULTISIG-only); I-D1—I-D18 LOCKED (18 total). **I1 (I-D9), I2 (I-D10), I8 (I-D13) COMPLETE pre-reframe; I3 (concrete `AuMT.sol`) TOMBSTONED** (removed at I9.4). **Vault bytecode untouched.** **Findings:** I10—I19 (`docs/STAGE_I_NOTES.md`). **Stage H:** H-D1—H-D42 LOCKED; tag `2a33649`.

**Branch:** `stage-i` at I7.3d (the §11 resume-anchor refresh closing the I7.3 close-of-family sweep; HEAD this commit; prior `5106aa0` I7.3c closed the PLAN status/mode header refresh, `119e382` I7.3b the PLAN Completion Log + Anchors, `6dab465` I7.3a the NOTES Finding I19). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.

Recent commit stack — the full per-sub-step catalog (every commit, hash, diff) now lives in the `docs/STAGE_I_PLAN.md` Completion Log, caught up through I7.3b. §11 keeps only the phase summary:
...
- **I-reframe.3c3—3c8d (section reframes + close-of-sweep):** I4—I7 roadmap reframe + Interfaces tables + status headers + Anchors + Completion Log catch-up + §11 refresh (`c327d22`…`6140c7f`).
- **I9 (AuMT deprecation + IVotingWeight migration):** `IVotingWeight.sol` forward stub + `VaultClassRegistry.sol` full rename + mock doubles + StageGIntegration consumer migration + `git rm AuMT.sol IAuMT.sol` + sweep (I9.1 `16e1d5c`…I9.5d `66afa4a`). COMPLETE; 619/620 unit + 71/71 fork green.
- **I4 (recorder dispatch + clock):** `getHookFlags` bump + one-shot `emissionRecorder` slot/setter + `effectiveQualBlock` clock + `onAfterAddLiquidity`/`onAfterRemoveLiquidity` → `recordDeposit`/`recordWithdrawal` + `IRouterSender` Permit2-free shim + regression/storage-slot fix + sweep (I4.1 `409f51a`…I4.7c `39fa3c9`). COMPLETE; 620/620 unit + 71/71 fork green; Findings I16/I17.
- **I5 (recorder-clock unit tests):** `test/unit/RecorderClock.t.sol` — first-deposit fresh-start + weighted-average top-up + partial-withdrawal reset + over-qualification vector + hook-gate `NotAuMTContract` + sweep (I5.1a `766566a`…I5.5c `a9afa1c`). COMPLETE; targeted 5/5 RecorderClock green (full unit 625, fork 71).
- **I6 (recorder-clock fork integration):** `test/fork/StageIIntegration.t.sol` — `StageIIntegrationFixture` (Option B own emission stack) + wiring / deposit-dispatch / cross-block top-up blend / aging + withdrawal-reset, real Balancer V3 add / remove-liquidity through the canonical `AureumFeeRoutingHook` → `recordDeposit` / `recordWithdrawal` (I6.1 `59ba9a3`…I6.4 `8a6394d`; I6.3-fix1 `ee71fe7`); I6-DIAG throwaway bisection disproved a fork `block.number` quirk (Finding I18); close-of-family sweep I6.5a—d. COMPLETE; 4/4 fork green.
- **I7 (deploy + close-of-stage):** `script/DeployStageI.s.sol` recorder-path wiring (setEmissionRecorder ×1 + setAuMTContractForPool ×3, GOVERNANCE_MULTISIG-only per I-D18; `fe6f658`) + `test/fork/DeployStageI.t.sol` integration (inherits `StageGIntegrationFixture` + `DeployStageH.deploy()`; 1/1 fork; `08f8ee5`); I7.2-pre §12 catch (Finding I19); close-of-family sweep I7.3a—d. COMPLETE; 1/1 fork green.
...
**Stage I position:** The **I9 (AuMT deprecation), I4 (recorder dispatch + clock), I5 (recorder-clock unit tests), and I6 (recorder-clock fork integration) code families are all COMPLETE**. `STAGE_I_PLAN.md`, `STAGE_I_NOTES.md`, and this §11 describe the AuMT=BPT clock-only architecture. **COMPLETE pre-reframe code, unchanged by the reframe:** I1 (EmissionDistributor per-pool refactor, I-D9; I1.1—I1.8e), I2 (AureumTime constants, I-D10; I2.1—I2.3e), I8 (gauge-eligibility hook-gate, OQ-24 / I-D13; I8.0a—I8.6 — `GaugeEligibility` requires the canonical `AureumFeeRoutingHook`; 49/49 unit + 71/71 fork). **I3 (concrete `AuMT.sol`, I3.1—I3.8f) is TOMBSTONED** — built, superseded by I-D14, removed at I9.4 (`0b5e572`). **I9 COMPLETE** (I9.1—I9.5f): `IVotingWeight.sol` forward stub (`16e1d5c`), `VaultClassRegistry.sol` full rename (`77a9164`), mock doubles + StageGIntegration consumer (`25c4793`/`ad9783e`/`476e77e`), `git rm AuMT.sol IAuMT.sol` (`0b5e572`). **I4 COMPLETE** (I4.1—I4.7): `getHookFlags` bump (`409f51a`) + one-shot `emissionRecorder` slot/setter per I-D16 (`3f8e8d5`) + `effectiveQualBlock` weighted-average clock per I-D14 (`5fb3c3f`) + `onAfterAddLiquidity`/`onAfterRemoveLiquidity` → `recordDeposit`/`recordWithdrawal` dispatch with the `IRouterSender` Permit2-free shim (`c47e6df`/`2b85263`/`62f7741`; Finding I16) + regression / storage-slot-layout fix (`56bc143`; Finding I17); **620/620 unit + 71/71 fork green**. **I5 COMPLETE** (I5.1a—I5.5c): `test/unit/RecorderClock.t.sol` — 5 tests (first-deposit fresh-start / weighted-average top-up / partial-withdrawal reset / over-qualification vector / hook-gate); targeted 5/5 RecorderClock green. **I6 COMPLETE** (I6.1—I6.5d): `test/fork/StageIIntegration.t.sol` — `StageIIntegrationFixture` (Option B own emission stack, recorder gate bound to the canonical `AureumFeeRoutingHook`) + wiring / deposit-dispatch / cross-block top-up blend / aging + withdrawal-reset (I6.1 `59ba9a3`, I6.2 `b3dd12c`, I6.3 `22115ed` → I6.3-fix1 `ee71fe7`, I6.4 `8a6394d`); I6-DIAG bisection disproved a fork `block.number` quirk (Finding I18); **4/4 fork green**. **I7.1 + I7.2 COMPLETE** (I7.1—I7.3d): `script/DeployStageI.s.sol` recorder-path wiring (GOVERNANCE_MULTISIG-only per I-D18; `fe6f658`) + `test/fork/DeployStageI.t.sol` integration (inherits `StageGIntegrationFixture` + `DeployStageH.deploy()`; recorder bind + 3 pilot gates + `NotEmissionRecorderAdmin` one-shot lock; 1/1 fork; `08f8ee5`); I7.2-pre §12 correction (`edd5f8d` → Finding I19); I7.3 close-of-family sweep (I7.3a—d). **I7.4 stage-close (full split-form regression + merge + tag `stage-i-complete`) is next.** **Decisions:** I-D1—I-D18 LOCKED (I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED, I-D5—I-D9 AMENDED, by I-D14). **Findings:** I10—I19. **Vault bytecode untouched.** Per-sub-step detail in `docs/STAGE_I_PLAN.md` (Completion Log + Decisions) and `docs/STAGE_I_NOTES.md` (I-D bodies + I10—I19 Findings).

**Deferred (Stage H carry-forward into Stage I + Stage I onward):**

- **H-D41 multi-minter architecture carry-forward to Stage K.** Per H10.0a `db75c6b`: Stage K deploys EmissionDistributor as initial sole minter at K-x then transitions to N-minter slot per future K-D; AuMM.setMinter handoff sequenced at Stage K rather than Stage H per H-D7 Option C; pre-Stage-K bookkeeping accumulates from GENESIS_BLOCK but `distribute()` / `claim()` revert `NotMinter` until Stage K setMinter call lands.
- **`script/DeployAuMM.s.sol` production deploy chain.** Separate micro-script (~30 lines) for AuMM deployment positioned before `DeployDerBodensee.s.sol` and `DeployStageH.s.sol` in the production deployment sequence per H-D42 convention; deferred to Stage K opening.
- **Direct `ReentrancyGuard.selector` test via `vm.transientStore`.** H3.7e `RevertWhen_Reentrant` covers the gate transitively via inner `NoPendingAccrual` bubble-up; direct slot-pre-seeding deferred to a robustness-backport entry post-Stage-I.
- **OQ-22 2-hop fallback.** H-D9 Phase 1 ships direct venues only; 2-hop fallback through ZCHF / sUSDS per FINDINGS L1115 deferred to future H-x micro-stage or Stage K+.
- **OQ-20 / OQ-21.** Controller yield-fee entry point + bi-weekly cadence per E-D10 not yet resolved.
- **D33 — Aureum's own Router.** Re-entry at Stage K (governance handoff) or Stage O (integration).
- **D36 permanent fix.** Env-key namespacing in `test/fork/DeployAureumVault.t.sol` carried forward; Stage E + Stage F + Stage G + Stage H + Stage I fork tests race-safe via E-D24 in-process pattern + same-value env-write convention + `--threads 1` belt per F-D11.
- **aumm-site spec edits (user-side).** `04_tokenomics.md` §viii / §ix amendments per OQ-20 + OQ-21 + the I-reframe AuMT=BPT reckoning (I-D14—I-D17 — AuMT is the BPT, value-weighted snapshot voting, recorder clock) deferred; `aumm-site` §xxi sandbox fast-track row obsolete per **G-D2** — spec edit deferred.
- **OQ-I10 Stage M/N recorder-binding rollout.** Under I-D14 there are no per-pool AuMT token instances — AuMT is each pool's BPT. Stage I binds the recorder clock + hook dispatch for the 3 Stage E pilot pools (slots 02/03/07); extending `setAuMTContractForPool` + `auMTContractByPool[pool] = hook` to the remaining gauged pools (slots 01/04/05/06/08—28) is deferred per the STAGES_OVERVIEW.md dependency graph (Stage M/N).
- **`src/governance/VotingWeight.sol` value-weighted voting view (I-D15).** Deferred out of Stage I to Stage K: the `(qualified_AuMT_value × time_in_pool_capped)^(1/4→1/3)` reader (14-day cliff, 6-month on-ramp, F-9 era transition, gauged-only) needs a concrete `ITVLOracle` (OQ-22 / F-D3) + the snapshot-voting layer (Stage K). The `IVotingWeight` forward stub **landed at I9.1** (`16e1d5c`); its `totalSupply()` veto-denominator semantics under the BPT model (I-D17) also lock at Stage K.

**Next sub-step: I7.4 — Stage I close-of-stage.** I7.1 + I7.2 (deploy script + 1/1 fork green) and I7.3 close-of-family sweep (I7.3a Finding I19, I7.3b—d PLAN + §11 refresh) are COMPLETE. I7.4 closes the stage: (a) full split-form regression per D35/D36 (`--no-match-path "test/fork/**"` unit + `--match-path "test/fork/**" --fork-url $MAINNET_RPC_URL --threads 1` fork); (b) Pre-flight checklist L87—89 flip backed by actual results; (c) fast-forward merge `stage-i → main` + `stage-i-complete` tag + snapshot push to `origin` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. The `script/DeployAuMM.s.sol` production deploy chain stays **deferred to the Stage K opening** per H-D42 (see Deferred). Entry mode: **Opus high** per §13 for I7.4.
