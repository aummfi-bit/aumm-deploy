...
**Next sub-step: K6 pre-flight** — the Opus beat that locks **K-D8** (`TVLOracle` roster re-wire to the live `MiliariumRegistry` dense enumeration per J-D8, an I13-class fix-forward). K6 re-points the Stage H `TVLOracle` pool roster onto the Stage J `MiliariumRegistry` dense enumeration so vote-weighting and emission scoring read the live roster rather than the original H-D8 constellation slots. The pre-flight resolves the roster-source binding, the re-wire's blast radius across existing `TVLOracle` consumers and tests (I13 sizing), and confirms the change lands fix-forward on `stage-k` (Stage H tag untouched) per the I13 precedent. K6 reads J-D8 (`docs/STAGE_J_NOTES.md`), K-D8 (`docs/STAGE_K_NOTES.md`), the `TVLOracle` roster surface (`src/emission/TVLOracle.sol`), and the Stage J `MiliariumRegistry` enumeration API (`src/registry/`). **Stage K mode: Opus extra-high** per §13 — governance handoff, stay on Opus through the stage; each unit opens with its own Opus pre-flight.
