...
**Last update:** 2026-06-08, **Stage K open** on branch `stage-k`, **K0—K3 complete**. K0 entry (K0.1—K0.4); K1 `AureumGovernanceAuthorizer` (`73ceaae` + 12-green unit `e72104c`, K-D3 LOCKED); K2 TVL-oracle binding (K-D4 reuse-direct, no code surface, `3e8250d` / `29d4b71`); K3 `VotingWeight` value-weighted reader (`50d9f95` + K3.0c `effectiveQualBlock` exposure `888f0bd` + 29-green unit cohort `4d81ffc` / `8e345b5` / `d2f90d1`, K-D5 LOCKED; K3.3 fork test folded into K4 `StageKIntegration` per user decision 2026-06-08). This commit is **K3.4** (§11 refresh). **Stage K = the governance handoff** (STAGES_OVERVIEW Stage K; "Largest stage"): ship on-chain governance (`AureumGovernance` — three proposal types) + the value-weighted voting reader (`VotingWeight`, I-D15) + the B-strict 12-month time-bomb authorizer (`AureumGovernanceAuthorizer`, OQ-10), and migrate the Vault's authorizer from the Stage B Safe to the governance contract. **K-D1—K-D5 LOCKED; K-D6—K-D9 forward-declared, each locking at its unit pre-flight. Vault bytecode untouched** — the migration is `setAuthorizer` / `setStaticSwapFeePercentage` CALLS, never edits. **Stage J context:** J-D1—J-D8 LOCKED, Finding J10; tag `stage-j-complete` (`83ef71e`), ff-merged to `main`, pushed to `origin`.

**Branch:** `stage-k` at K3.4 (this `CLAUDE.md` §11 commit; prior `1ad18ea` K3.3b, `e69652c` K3.3a, `d2f90d1` K3.2c). Full K0—K3 stack on the branch. Branched from `stage-j-complete` (`83ef71e`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention (and Stages I, J). `main` sits at `stage-j-complete` HEAD until the K-close ff-merge; `stage-k` snapshot push to `origin` lands at K-close.
...
- **K3** `VotingWeight` (K-D5 LOCKED): K3.0a NOTES (`d66688b`) / K3.0b PLAN (`0b790c3`) / K3.0c `effectiveQualBlock` exposure (`888f0bd`) / K3.1 contract (`50d9f95`) / K3.2a—c unit 29-green (`4d81ffc` / `8e345b5` / `d2f90d1`) / K3.3a NOTES fold (`e69652c`) / K3.3b PLAN fold (`1ad18ea`) / K3.4 this §11 refresh. K3.3 fork test folded into K4 `StageKIntegration`.
...
**Next sub-step: K4 pre-flight** — the Opus extra-high beat that locks **K-D6** (`AureumGovernance` three proposal types — gauge challenge F-12 / composition challenge / fee proposal — + F-9 dampening + snapshot voting + timelock) with sub-splits **K-D6a—K-D6f** (base proposal machinery, snapshot mechanism, F-9 era root, F-12 elite-tail deposit, per-type quorum/majority/deposit, timelock + execution routing). K4 reads F-9 / F-12 from `11_formulas.md` + OQ-7 / OQ-8 / OQ-9 / OQ-11 / OQ-13 (FINDINGS), and depends on K3 `VotingWeight`, `IGaugeRegistry` (`revokeGauge` / `registerGaugeFromComposition`), `IMiliariumSlotRegistry` (`replaceSlot`), Vault (`setStaticSwapFeePercentage`), and the Stage D fee-routing primitive (proposal-deposit-to-Bodensee). K4 surfaces `src/governance/AureumGovernance.sol` + `test/unit/AureumGovernance.t.sol` + `test/fork/StageKIntegration.t.sol` (the latter absorbs the folded K3.3 fork coverage). **Stage K mode: Opus extra-high** per §13 — governance handoff, stay on Opus through the stage; each unit opens with its own Opus pre-flight.
