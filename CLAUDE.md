...
**Last update:** 2026-06-26 — **Stage N COMPLETE** (`stage-n-complete`, ff-merged to `main`). The Miliarium Sector-3 Equity/thematic tranche plus the two resolvable ex-M Majors: 18 pools (N-D0) — 16 Sector-3 Standard slots plus 02 ixAetheron and 06 ixLibertas — deployed via per-pool config libs + `DeployIx*.run()` wrappers (Stage-E framework), then bound into registry/gauge/distributor by the bind-only `script/DeployStageN.s.sol` (M-D9 shape: `replaceSlot` + `seedFoundingPool` + `setAuMTContractForPool` under three fail-fast governance preconditions; `MILIARIUM_POOL_NN` env-key scheme, D36-collision-safe). New this stage: the protocol's first Aureum-owned `src/rate_provider/` surface — `ERC4626RateProvider` (N-D2, faithful re-impl of the Balancer submodule pattern) + `CompositeRateProvider` (two-hop, N-D9), re-authored under `src/` not as submodule edits (§8c). N-D8 redesigned ixAetheron to native-4626 ETH-staking cores (sfrxETH/wOETH, dropping the N-D1 stata composites); N-D9 swapped the dead-rate sfrxUSD leg (mainnet `previewRedeem` == 0 post-Fraxtal) to ysyBOLD across 06/20/27, rated via `CompositeRateProvider(ysyBOLD, ERC4626RateProvider(yBOLD))`. Fork-tested via `test/fork/StageNIntegration.t.sol` + `test/fork/DeployStageN.t.sol` (10/10 fail-fast governance + binding-liveness); full regression 864/864 unit + 119/119 fork green. Prior — **WN whitehat pass closed:** F-11 (Low — both Aureum-owned RP constructors assigned the ERC-4626 vault immutable with no decimals check) **Fixed** fail-closed on `stage-m`, no re-tag (I13-class). Next: Stage O.
...
**Branch / tag state:** `stage-n-complete` tagged (at the N7-backfill commit) and ff-merged to `main` — no divergence; Stage N work continued on the `stage-m` branch (post-M no-new-branch posture per the Stage-N NOTES). Stage-N history since `stage-m-complete`: N1—N3 (`src/rate_provider/` `ERC4626RateProvider` + `CompositeRateProvider` + unit tests) → N4 (02/06 Majors, N-D8 native-4626) → N5 (`DeployStageN.s.sol` bind-only) → N6 (fork suite + the N-D9 sfrxUSD→ysyBOLD RP swap) → WN whitehat (F-11 Fixed) → N7 close + backfill. Tags `stage-a-complete` … `stage-m-complete` + `stage-n-complete` + `stage-k-whitehat-complete` all in place. Whitehat ledger carries zero open findings (F-01…F-11 all Fixed).
