...
**Last update:** 2026-06-18 — **WM whitehat pass closed:** F-08 (`VaultClassRegistry.vetoProposal` veto-stacking, Medium), F-09 (`AureumFeeRoutingHook` self-router `getSender()` recorder spoof, Medium), F-10 (`EmissionDistributor.recordScore` ungated `tvlEMA`, Medium) all **Fixed** fix-forward on `stage-m`, no re-tag (I13-class — each finding's contract is in a tagged-complete prior stage G/H/I) — ledger (3 rows Fixed) + this §11 + `docs/STAGE_M_NOTES.md` Whitehat design-traces + PoCs (F08 4/4, F09 7/7, F10 7/7); 847/847 unit + 99/99 fork. **§11 reconciliation:** this refresh also records the M0.2 milestone never logged here — **Stage M design freeze (M-D1—M-D6) LOCKED** at `8e3dcb5` (`docs/STAGE_M_NOTES.md`); the WM pass ran at the Stage-M entry, after M0.2, before the M0.3 `STAGE_M_PLAN.md`. Prior (2026-06-17) — **WL whitehat pass closed:** F-07 (`IncendiaryRegistry._maturePrice` EMA-freshness, Low) **Fixed** fix-forward on `main` — ledger + `STAGE_L_NOTES.md` L-D29 + PoC (4/4). Next: Stage M M0.3 (`STAGE_M_PLAN.md`).
...
**Branch / tag state:** active branch `stage-m`, ahead of `main` (`1f8dfa0`, the F-07 WL record = merge-base) by three not-yet-merged groups — M0.2 NOTES freeze (`8e3dcb5`), the WM ledger log (`e86f63d`), and the WM fixes + PoCs (`609335e` … `d9eb31f`); not yet merged to `main`. `main` is ahead of `stage-l-complete` by the F-07 WL fix-forward (`0a21b99`+`b96227c`+`d3ece7f`); `stage-l` fully merged. No Stage-M tag yet (Stage M mid-bootstrap: M0.2 done, M0.3 / code ahead). Tags `stage-a-complete` … `stage-k-complete` + `stage-k-whitehat-complete` + `stage-l-complete` all in place.
...
- **Stage M (Miliarium Sector 2) — IN PROGRESS** (STAGES_OVERVIEW Stage M — ~10 Majors / yield-core pools; config/deployment, Stage-J/E-class registry stage). **M0.2 design freeze COMPLETE** — M-D1—M-D6 LOCKED at `8e3dcb5` (`docs/STAGE_M_NOTES.md`): scope + on-disk `aumm-site`-clone spec source, the 9-slot Majors set (pilots 01/05/14 already deployed; Stage N takes the rest), the `[2,3,7]`→`[1,5,14]` registry-seed correction, the Standard 26/26/16/16/16 template + 52% Quality-Gate, and multisig founding-pool binding (K-D9). **Next: M0.3 `STAGE_M_PLAN.md`** (not yet authored), then M1+ per-pool config libs + `DeployStageM.s.sol`. §13 has no Stage M row; Stage-J posture — Opus-high for pool-class / interface / literal authoring, Sonnet for test / housekeeping. Per OQ-I10, Stages M/N bind the remaining Miliarium slots (01/04/05/06/08—28).
