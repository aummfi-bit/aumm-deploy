...
**Last update:** 2026-04-27, **Stage E mid-stage — E1 + E2 closed (E3 ixAurebit next).** E1 delivered: `AureumWeightedPoolFactory.sol` peer-inherited from `BasePoolFactory` (E-D11) — own external `create(...)` reverting `QualityGateUnsatisfied(uint256 erc4626WeightSum, uint256 minRequired)` at the 52% ERC-4626 gate (`MIN_ERC4626_WEIGHT = 52e16`); peer-inheritance rather than subclassing because upstream `WeightedPoolFactory.create` is `external`, not `virtual`, so a subclass cannot `override` and would inherit upstream's `create()` as a publicly-callable QG bypass. Vault bytecode untouched, zero edits to `lib/balancer-v3-monorepo/`. Other E1 artifacts: `script/pools/PoolConfig.sol` (E-D2 struct shape — `name`, `symbol`, `slot`, `sectorLabel`, `tokens`, `tokenTypes`, `rateProviders`, `paysYieldFees`, `normalizedWeights`, `swapFeePercentage`, `salt`); `script/pools/configs/01_ixHelvetia.s.sol` (IxHelvetiaConfig library, slot 01, svZCHF 80% + sUSDS 20%, 100% ERC-4626 gate margin +48 pp); `script/pools/deploy-miliarium-pool.s.sol` (abstract `MiliariumPoolDeployer` framework with `config()` / `run()` skeleton + factory call + post-deploy registration per E-D23); `script/pools/DeployIxHelvetia.s.sol` (concrete wrapper for slot 01); `test/fork/PilotPools.t.sol` (E-D24 in-process `MiliariumPilotPoolBase` abstract — full deploy chain in `setUp`, env-key cross-suite race race-safe via same-value writes per E-D23, `--threads 1` belt remains); `IxHelvetiaPilotTest` derived contract with single fork test asserting pilot pool deployed + Bodensee BPT supply expansion + `svZchf.balanceOf(hook) == 0`. Mid-stage OQ-11 supersession landed at E1.3a-quater through E1.3a-septies (Bodensee swap fee `0.75%` immutable per E-D22; Miliarium genesis `0.02%` per `MILIARIUM_SWAP_FEE_GENESIS`; band `0.01%–0.30%` per `MILIARIUM_SWAP_FEE_MIN/MAX`; controller constant rename `BODENSEE_SWAP_FEE_GENESIS → BODENSEE_SWAP_FEE`; `BODENSEE_SWAP_FEE_MIN/MAX` removed; Bodensee `swapFeeManager: address(0)` in `DeployDerBodensee.s.sol` + `AureumFeeRoutingHook.t.sol`). E2 delivered: `script/pools/configs/05_ixEdelweiss.s.sol` (IxEdelweissConfig library, slot 05, waEthUSDT 18% / waEthUSDC 18% / ixEDEL 46% / svZCHF 18%, 54% ERC-4626 gate margin +2 pp, address-sorted token literals); `script/pools/DeployIxEdelweiss.s.sol` (concrete wrapper for slot 05); `IxEdelweissPilotTest` derived contract appended to `test/fork/PilotPools.t.sol` (waEthUSDC → svZCHF representative swap vector per E-D25); E10 finding (forge-std `deal(adjust=true)` panics on Reserve-DTF `ixEDEL` because `totalSupply()` is dynamic / fee-decay accrual and forge's adjust heuristic arithmetic-wraps with panic 0x11) + E2.3-fix corrective sub-step at `test/fork/PilotPools.t.sol` L302 (drop `, true` from `_initializePool` mint loop), L383–L386 (per-decimal `_seedAmounts()` literals matched to address-sorted slot order: `1_000e6 / 1_000e6 / 1_000e18 / 1_000e18` for waEthUSDT / waEthUSDC / ixEDEL / svZCHF), and L393 (IxEdelweiss swap vector `1e6` not `1e18` for 6-dec waEthUSDC). Pilot fork tests 2/2 green via D35 split-form `--threads 1`: `IxHelvetiaPilotTest::test_Fork_IxHelvetia_DeploysAndRoutesFee` PASS gas 651489 (501 ms hot fork); `IxEdelweissPilotTest::test_Fork_IxEdelweiss_DeploysAndRoutesFee` PASS gas 770668 (13.45 s cold fork); combined wall ~15 s on Ankr. **Next: E3 — ixAurebit (slot 14, Standard 26/26/16/16/16 — svZCHF 26% + Aave Prime GHO 26% + ixEDEL 16% + WBTC 16% + cbBTC 16%; 52% ERC-4626 gate boundary +0 pp; 8-decimal WBTC/cbBTC will exercise the per-decimal `_seedAmounts()` pattern landed at E2.3-fix per E10).**

**Branch:** `stage-e` ahead of `origin/stage-e` post the §11-refresh commit landing on top of `fd554c1` (E2.3-fix). Branch point `57cd2ce` (per E-D8). No FF to `main` until E9; the §11-refresh commit is the new tip and references `fd554c1` as the tip-before-itself per project convention.

Recent commit stack (`fd554c1`-and-prior, all on `stage-e`):

- `fd554c1` — E2.3-fix: test/fork/PilotPools.t.sol — drop deal(adjust=true) in _initializePool, per-decimal _seedAmounts(), 1e6 swap vector (per E10)
- `393cea0` — docs: STAGE_E_NOTES.md E10 — forge-std deal(adjust=true) vs Reserve-DTF ixEDEL; per-decimal _seedAmounts() for mixed-decimal pilots
- `efb7a10` — E2.3: test/fork/PilotPools.t.sol — IxEdelweissPilotTest derived contract + fee-routing test (waEthUSDC → svZCHF)
- `cdd7a5d` — E2.2: script/pools/DeployIxEdelweiss.s.sol — concrete MiliariumPoolDeployer wrapper for ixEdelweiss (slot 05)
- `3289ade` — E2.1: script/pools/configs/05_ixEdelweiss.s.sol — IxEdelweissConfig per-pool config library (waEthUSDT 18% / waEthUSDC 18% / ixEDEL 46% / svZCHF 18%, slot 05)
- `e079457` — docs: STAGE_E_NOTES.md E-D25 — E2 ixEdelweiss sub-step layout + locked decisions
- `041d517` — E1.6b: test/fork/PilotPools.t.sol — IxHelvetiaPilotTest derived contract + fee-routing test
- `3f47f43` — E1.6a: test/fork/PilotPools.t.sol — MiliariumPilotPoolBase abstract shared base

Earlier Stage E commits (E0 through E1.5, OQ-11 supersession landings, E-D24 supersession of E-D6 env-key suffix) live in `git log stage-e ^main` from branch point `57cd2ce` to `fd554c1`.

**Current tag:** `stage-d-complete` (commit `951c338`, 2026-04-25). Next expected tag: `stage-e-complete` at Stage E close (E9). `stage-c-complete` (`5342126`, 2026-04-18), `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage E position:** **E1 + E2 complete; E3 / E4 / E5 / E9 ahead.** Per-sub-step detail (E0 through E2) lives in `docs/STAGE_E_PLAN.md` Decisions table (E-D1 through E-D10 entry-time) + Mid-stage supersessions (OQ-11 swap fees / E-D6→E-D24 env-key suffix) + `docs/STAGE_E_NOTES.md` E-D-numbered findings (notably **E-D11** factory peer-inherit pattern, **E-D16** AureumWeightedPoolFactory additive, **E-D17–E-D22** ixEdelweiss config locks, **E-D23** deploy script architecture (abstract base + per-pilot wrappers, env-var contract), **E-D24** in-process harness shape + `--threads 1` belt + INIT_SEED parity, **E-D25** ixEdelweiss sub-step layout + locked decisions) and E-numbered findings (notably **E10** forge-std deal(adjust=true) vs Reserve-DTF) + git history on `stage-e` from branch point `57cd2ce` to `fd554c1`. Per-sub-step Completion Log fills in `docs/STAGE_E_PLAN.md` happen at E5 docs sub-step (per E-D9), not per-sub-step.

**Deferred (post-Stage E):**

- **OQ-20 / OQ-21.** Carry from Stage D; per E-D10, **not** Stage-E-resolved. Per-token `paysYieldFees: true` flags in pilot configs are forward-compatible with the eventual OQ-20 controller yield-fee entry point and OQ-21 bi-weekly cadence.
- **D33 — Aureum's own Router.** Carry from Stage D. Re-entry at Stage K (governance handoff) or Stage O (integration). Full options list in `balancer_v3_reference.md` "Open architectural question — Aureum's own Router."
- **D36 permanent fix.** Within Stage E pilot tests the E-D24 in-process pattern + same-value env-write convention render the cross-suite race race-safe (per E-D23 / E-D24); `--threads 1` flag remains as cross-suite belt against legacy Stage D fork tests writing `FEE_ROUTING_HOOK` to conflicting addresses. Permanent fix in Stage D fork tests (env-key namespacing in `test/fork/DeployAureumVault.t.sol`) deferred — not Stage-E-resolved.
- **E-D7 PLAN.md L79 wording stale post-E10.** `docs/STAGE_E_PLAN.md` L79 still reads `deal(token, address(this), amount, true)` per E-D7; E10 amends this de facto in the harness (drop the third argument, `adjust=false` default). Either fold into a Mid-stage supersessions pointer at E5 or accept as known-stale until the E5 docs sweep.
- **aumm-site spec edits (user-side).** Carry from Stage D. `04_tokenomics.md` §ix prose needs amendment per OQ-20 + OQ-21; not a repo edit.

**How to resume (Stage E → E3 ixAurebit):**

1. Confirm current tip: `git log --oneline -1` on `stage-e` returns the §11-refresh commit on top of `fd554c1`. The §11 commit is the §11-refresh-itself; §11 above references `fd554c1` as the tip-before-itself per project convention.
2. Confirm clean working tree: `git status -sb` shows only the active branch tracking line, no modified or untracked files.
3. Read `docs/STAGE_E_PLAN.md` E-D4 ixAurebit composition lock (slot 14: svZCHF 26% + Aave Prime GHO 26% + ixEDEL 16% + WBTC 16% + cbBTC 16%; addresses + RPs verified per OQ-12 / OQ-12a) and the relevant E-D entries for the harness pattern (E-D24 in-process harness; E-D25 INIT_SEED amendment per E10 — `_seedAmounts()` returns per-token `1_000 × 10^decimals(token)` matched to address-sorted slot order).
4. **E3 sub-step structure mirrors E2:** **E3.1** `script/pools/configs/14_ixAurebit.s.sol` IxAurebitConfig library (slot 14, address-sorted token literals + RPs + weights + types + `paysYieldFees` + swap fee `MILIARIUM_SWAP_FEE_GENESIS` + slot-derived `bytes32(uint256(14))` salt); **E3.2** `script/pools/DeployIxAurebit.s.sol` concrete `MiliariumPoolDeployer` wrapper; **E3.3** `IxAurebitPilotTest` derived contract appended to `test/fork/PilotPools.t.sol` overriding `_deployer()` (returns `new DeployIxAurebit()`) + `_seedAmounts()` (returns 5 per-decimal literals matched to address-sorted slot order — the 8-decimal slots for WBTC/cbBTC are the new pattern post-E10) + `test_Fork_IxAurebit_DeploysAndRoutesFee()` with a representative swap vector (e.g. STANDARD-token-input `WBTC → svZCHF` covers the STANDARD-typed `tokenIn` case reserved for E3 per E-D25); **E3.4** single-test fork run `forge test --match-path "test/fork/PilotPools.t.sol" --match-test "test_Fork_IxAurebit" --fork-url $MAINNET_RPC_URL -vv --threads 1`. Final E3 numbering may refine at E3 entry.
5. **Pre-flight reads to draft E3.1:** `aumm-site/06_miliarium_manifest.md` ixAurebit row (slot 14 confirmation, sector label); E-D4 already locks token addresses + RPs + types + weights; OQ-12 / OQ-12a in `FINDINGS.md` for FX-related cross-references; `script/pools/configs/05_ixEdelweiss.s.sol` (E2.1 commit `3289ade`) as the immediate-precedent shape.
6. **Quality Gate boundary check:** ixAurebit at 52% margin +0 pp must clear the QG check exactly. Verify in IxAurebitConfig that `sum(weights[i] where tokenTypes[i] == WITH_RATE && rateProviders[i] != IRateProvider(address(0))) == 52e16` exactly. `AureumWeightedPoolFactory.create(...)` reverts `QualityGateUnsatisfied` when sum < 52e16; the boundary case `== 52e16` must NOT revert (verify `>=` comparator in factory implementation, not `>`).
7. **Decimal mapping for `_seedAmounts()`:** ixEDEL = 18-dec STANDARD (no RP); svZCHF = 18-dec WITH_RATE (RP); Aave Prime GHO = 18-dec WITH_RATE (RP); WBTC = 8-dec STANDARD (no RP); cbBTC = 8-dec STANDARD (no RP). Address-sorted positions and per-decimal seed literals must match — verify against IxAurebitConfig at E3.3 entry. Do not hardcode the slot order in the IxAurebitPilotTest override before IxAurebitConfig is locked.
8. **DTF-as-`tokenIn` swap-path consideration (per E10).** ixAurebit composition has no Reserve DTF as tokenIn under the recommended `WBTC → svZCHF` representative vector. If a future ixAurebit test vector uses ixEDEL as tokenIn, the same `adjust=false` rule from E2.3-fix L302 applies to `_performSwap` L324 — currently `_performSwap` retains `adjust=true` because the deployed E2 vector (waEthUSDC, 6-dec wrapped stablecoin) is not a DTF.
9. Entry mode per §13 stage-level defaults table (Stage E row): **Opus high entry** for E3 design + E3.1 config draft (Quality Gate boundary case requires careful weight + type accounting; address-sort verification); then **mostly Sonnet** for E3.2 / E3.3 mechanical wrappers and base-derived test contract; back to **Opus high** for E3.4 fork-test interpretation if traces need debugging.
