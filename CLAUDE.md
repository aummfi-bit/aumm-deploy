...
**Last update:** 2026-05-29, **Stage I in progress** on branch `stage-i` (HEAD at the I8.6 close-of-family sweep). **Stage H complete and tagged** at `stage-h-complete` (commit `2a33649`) — fast-forward merged to `main` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. **Stage I — AuMT (Aureum Market Tessera)** open since 2026-05-23: per-pool LP-receipt token (soulbound ERC-20, 18 decimals). **I1 (EmissionDistributor per-pool refactor, I-D9), I2 (AureumTime constants, I-D10), and I3 (AuMT.sol concrete implementation, I-D1/I-D4/I-D6/I-D7/I-D11/I-D12) COMPLETE** (I1.1—I1.8e + I2.1—I2.3e + I3.1—I3.8f). **I4 — AureumFeeRoutingHook AuMT extension (I-D5) — PARKED at I4.2**: I4.1 `getHookFlags` bump landed (`409f51a`); I4.2 (`auMTByPool` + `setAuMTForPool`) blocked pending the I-D5/I-D8 AuMT-scope reconciliation (AuMT for all gauged pools, not Miliarium-only) + `setAuMTForPool` binding-authority decision (leaning permissionless per I-D13). **I8 — gauge-eligibility hook-gate add-on (OQ-24 / I-D13) COMPLETE** across I8.0a—I8.6 (`f2d9da4`…`2d6fc8f`): `GaugeEligibility` now requires the canonical `AureumFeeRoutingHook` (9th constructor immutable + `WrongFeeRoutingHook` fail-fast gate in `_checkEligibilityCriteria`); 49/49 GaugeEligibility unit + 71/71 fork green; Stage G tag untouched (fix-forward on `stage-i`). **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **I-D locks:** I-D1—I-D13 LOCKED (13 total; I-D13 at I8.0b `b09605c`). **`docs/STAGE_I_NOTES.md` Findings queue (4):** I10 (multi-file rename NatSpec drift `44a94bb`); I11 (Foundry `--match-test` Rust regex `f143b7a`); I12 (container-shape LOCK cross-check `d11dbb7`); I13 (fail-fast gate test blast-radius `dcca7fe`). **Stage H complete state:** H-D1—H-D42 LOCKED (39 total); stage tag at `2a33649`; full Completion Log in `docs/STAGE_H_PLAN.md`.

**Branch:** `stage-i` at the I8.6 close-of-family sweep (HEAD; I8.6e CLAUDE §11 this commit; prior I8 commits `f2d9da4`…`2d6fc8f`). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.
...
**Stage I position:** **I1 (EmissionDistributor per-pool refactor, I-D9), I2 (AureumTime constants, I-D10), and I3 (AuMT.sol concrete implementation, I-D1/I-D4/I-D6/I-D7/I-D11/I-D12) COMPLETE** (I1.1—I1.8e + I2.1—I2.3e + I3.1—I3.8f). **I8 — gauge-eligibility hook-gate add-on (OQ-24 / I-D13) COMPLETE** across I8.0a—I8.6: `GaugeEligibility` requires the canonical `AureumFeeRoutingHook` (9th constructor immutable `feeRoutingHook` + `WrongFeeRoutingHook` fail-fast first check in `_checkEligibilityCriteria`); all 11 construction sites updated (G16); base setUp `getHooksConfig` mock retrofit + 3 dedicated hook-gate tests (49/49 GaugeEligibility unit); fork `StageGIntegration` carries the real hook (71/71 fork); Stage G tag untouched (fix-forward on `stage-i`). **I4 — AureumFeeRoutingHook AuMT extension (I-D5) — PARKED at I4.2**: I4.1 `getHookFlags` bump landed (`409f51a`, enabling `shouldCallAfterAddLiquidity` / `shouldCallAfterRemoveLiquidity`); I4.2 (`auMTByPool` storage + `setAuMTForPool` setter) blocked pending two chat-level resolutions — (a) the I-D5/I-D8 AuMT-scope reconciliation (AuMT is the per-pool LP-receipt token for ALL gauged pools, not the 28 Miliarium slots alone; the "Miliarium-only" framing is the pilot rollout cadence) and (b) the `setAuMTForPool` binding-authority decision (permissionless vs governance-gated; leaning permissionless since gauging is permissionless + hook-gated per I-D13). Per-sub-step detail in `docs/STAGE_I_PLAN.md` Completion Log + Decisions table (I-D1—I-D13 LOCKED — 13 total) and `docs/STAGE_I_NOTES.md` I-D bodies + I10/I11/I12/I13 Findings.
...
- **aumm-site spec edits (user-side).** `04_tokenomics.md` §ix amendments per OQ-20 + OQ-21 + AuMT design (I-D1—I-D10) deferred; `aumm-site` §xxi sandbox fast-track row obsolete per **G-D2** — spec edit deferred.
- **OQ-I10 Stage M/N AuMT rollout.** Remaining 25 AuMT instances (slots 01/04/05/06/08—28) deferred per STAGES_OVERVIEW.md dependency graph; Stage I ships 3 pilot AuMT + FeeRoutingHook extension per I-D8.

**Next sub-step: resolve the I4.2 design block, then I4.2.** I4.2 (`auMTByPool` + one-shot `setAuMTForPool` + `AuMTBound` event + `AuMTAlreadyBound` error on `AureumFeeRoutingHook`) is PARKED pending: (a) a NOTES amendment reconciling I-D5/I-D8 AuMT scope (AuMT for all gauged pools, not Miliarium-only); (b) a binding-authority LOCK for `setAuMTForPool` (permissionless self-binding with a trust check vs governance-gated; leaning permissionless per the I8/I-D13 hook-gate reasoning). Both are chat-level decisions per §12 — resolve in `docs/STAGE_I_NOTES.md` before drafting the I4.2 §8e.1. After the LOCK: I4.2 → I4.3 (`onAfterAddLiquidity` mint dispatch) → I4.4 (`onAfterRemoveLiquidity` burn dispatch) → I4.5 (Stage D regression — also clears the pre-existing `test_getHookFlags_shouldCallAfterSwapOnly` failure from the I4.1 flag bump) → I4.6 close-of-family; then I5 (AuMT unit tests) / I6 (fork integration) / I7 (deploy + close-of-stage). Entry mode: **Opus high** per §13 (live design decisions on the binding authority + new hook callback bodies).
