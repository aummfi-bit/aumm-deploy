...
**Last update:** 2026-06-04, **Stage I code phase open** on branch `stage-i` (HEAD at I9.5e, the §11 resume-anchor refresh). **Stage H complete and tagged** at `stage-h-complete` (`2a33649`). **The I9 family is COMPLETE** (AuMT deprecation + IVotingWeight migration): `src/governance/IVotingWeight.sol` created (I9.1 `16e1d5c`), `VaultClassRegistry.sol` full-renamed off the deleted `IAuMT` consumer (I9.2 `77a9164`), the mock doubles + StageGIntegration consumer migrated (I9.3a/b/c), and **`AuMT.sol` + `IAuMT.sol` removed** via `git rm` (I9.4 `0b5e572`; 619/620 unit + 71/71 fork green). Per **OQ-25 / I-D14**, AuMT is the pool's own Balancer V3 **BPT** (branded), **not a separate token**; Stage I is the oracle-free **recorder clock + hook dispatch** only; the value-weighted snapshot-voting view is **deferred to a Stage K `src/governance/VotingWeight.sol` reader** per I-D15. **Locks I-D14—I-D17:** I-D14 (AuMT = BPT; supersedes I-D1/I-D2/I-D3/I-D4/I-D11/I-D12, amends I-D5—I-D9), I-D15 (view deferred; `AuMT.sol` + `IAuMT.sol` deleted), I-D16 (hook one-shot `emissionRecorder` slot, not a ctor immutable), I-D17 (`IVotingWeight` forward stub + `VaultClassRegistry` full-rename fix-forward). **I1 (per-pool refactor, I-D9), I2 (AureumTime constants, I-D10), I8 (gauge hook-gate, I-D13) COMPLETE pre-reframe, unchanged.** **I3 (concrete `AuMT.sol`) TOMBSTONED** — built at I3.1—I3.8f, superseded, removed at I9.4. **I4 reframed** to recorder dispatch + clock (I4.1 `getHookFlags` bump landed `409f51a`; I4.2+ next). **The doc reframe (I-reframe.2—3c8d) and the I9 code family are both COMPLETE; I4 is next.** **Vault bytecode untouched.** **I-D locks:** I-D1—I-D17 LOCKED (17 total). **Findings:** I10—I15 (`docs/STAGE_I_NOTES.md`). **Stage H complete state:** H-D1—H-D42 LOCKED; tag `2a33649`.

**Branch:** `stage-i` at I9.5e (the §11 resume-anchor refresh; HEAD this commit; prior `66afa4a` I9.5d closed the PLAN/NOTES sweep tail). Branched from `stage-h-complete` (`2a33649`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention; `main` is at `stage-h-complete` HEAD. `stage-i` snapshot push to `origin` deferred to I7.4 close-of-stage.

Recent commit stack — the full per-sub-step catalog (every commit, hash, diff) now lives in the `docs/STAGE_I_PLAN.md` Completion Log, caught up through I9.5d. §11 keeps only the phase summary:
...
**Stage I position:** The **I-reframe doc sweep (I-reframe.2—3c8d) and the I9 code family are both COMPLETE**. `STAGE_I_PLAN.md`, `STAGE_I_NOTES.md`, and this §11 describe the AuMT=BPT clock-only architecture. **COMPLETE pre-reframe code, unchanged by the reframe:** I1 (EmissionDistributor per-pool refactor, I-D9; I1.1—I1.8e), I2 (AureumTime constants, I-D10; I2.1—I2.3e), I8 (gauge-eligibility hook-gate, OQ-24 / I-D13; I8.0a—I8.6 — `GaugeEligibility` requires the canonical `AureumFeeRoutingHook`; 49/49 unit + 71/71 fork). **I3 (concrete `AuMT.sol`, I3.1—I3.8f) is TOMBSTONED** — built, superseded by I-D14, removed at I9.4 (`0b5e572`). **I9 COMPLETE** (I9.1—I9.5d): `IVotingWeight.sol` forward stub (`16e1d5c`), `VaultClassRegistry.sol` full rename (`77a9164`), mock doubles + StageGIntegration consumer (`25c4793`/`ad9783e`/`476e77e`), `git rm AuMT.sol IAuMT.sol` (`0b5e572`); 619/620 unit + 71/71 fork green. **I4.1 `getHookFlags` bump landed** (`409f51a`); I4.2+ (recorder dispatch + clock per I-D16) is next. **Decisions:** I-D1—I-D17 LOCKED (I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED, I-D5—I-D9 AMENDED, by I-D14). **Findings:** I10—I15. **Vault bytecode untouched.** Per-sub-step detail in `docs/STAGE_I_PLAN.md` (Completion Log + Decisions) and `docs/STAGE_I_NOTES.md` (I-D bodies + I10—I15 Findings).
...
**Next sub-step: I4.2 — `emissionRecorder` slot + `setEmissionRecorder` one-shot setter on `AureumFeeRoutingHook` (I4.3/I4.4 design pre-flight pending).** I9 (AuMT deprecation + IVotingWeight migration) is COMPLETE (I9.1—I9.5d). The code phase continues: **I4.2** `emissionRecorder` slot + `setEmissionRecorder` one-shot setter on `src/fee_router/AureumFeeRoutingHook.sol` (mirrors `setGovernanceModule` at L100—112; NOT a ctor immutable per I-D16 + H13-class ordering) → **I4.3** `onAfterAddLiquidity` → `recordDeposit(pool, lp, bptAmountOut)` → **I4.4** `onAfterRemoveLiquidity` → `recordWithdrawal(pool, lp, bptAmountIn)` — **design pre-flight required before I4.3/I4.4**: check the exact `onAfterAddLiquidity` / `onAfterRemoveLiquidity` callback signature in `AureumFeeRoutingHook.sol` for the address parameter that is the liquidity provider (`sender`? router-derived?) → **I5** (recorder-clock unit tests, `RecorderClock.t.sol`) → **I6** (fork integration, `StageIIntegration.t.sol`) → **I7** (deploy script + close-of-stage). Entry mode: **Opus high** per §13 (live design on hook callback bodies + EmissionDistributor clock math).
