...
**Last update:** 2026-05-21, **Stage H IN PROGRESS** on branch `stage-h` (HEAD `a9e4b06`). H1—H3 families complete; H4 family complete (H4.0 entry-time scaffold + H4.1 IEmissionDistributor interface + H4.1-bis IEfficiencyOracle.recordEmissions push-signature retrofit + H4.2—H4.7 EmissionDistributor implementation family + H4.7c abstract-keyword drop + H4.8 unit test cohort 64/64 green); **H5 family complete (H5.0—H5.4)** — phase-aware emission wiring per **H-D26—H-D30** + `recordScore` reshape per **H-D31—H-D33** + Harness observation surface per **H-D34**; **H5.5 close-of-family sweep in flight** (H5.5a Completion Log 25-row backfill + H5.5b status header refresh landed; H5.5c CLAUDE.md §11 refresh is this commit). Branch carries 141 sub-step commits beyond `stage-g-complete` (`384a40b`). **Concrete deliveries to date — Stage H stack through H5.5b:** `src/emission/TVLOracle.sol` (constructor-injected `tokenToUnderlying` seed loop + governance-extensible constellation roster per **H-D8** + Step 1 single `IVaultExplorer.getPoolData.balancesLiveScaled18[]` read + Step 2 `_constellationRatio` balance-ratio averaging across `underlyingToPools[U]` reverse-map per **H-D9** + `quoteSvZCHF(token, amountScaled18)` push primitive at H2a.10a per **H-D10 v2**; 36/36 unit tests at H2a.10b); `src/ccb/ITVLOracle.sol` retrofit at H2a.10c (`quoteSvZCHF` interface + `override` keyword on `TVLOracle.quoteSvZCHF` + return-0 stubs in all three `MockTVLOracle` inheritors per **G16** inheritor enumeration discipline); `src/emission/EfficiencyOracle.sol` (intra-epoch accumulation per **H-D10** — `recordFees(pool, token, amountScaled18)` + `recordEmissions(pool, aummAmountScaled18)` push entries routed through `tvlOracle.quoteSvZCHF` for same-unit svZCHF accumulation per **G-D23 (i)** + `EpochEntry[3]` ring buffer with epoch-stamp stale detection + O(1) `_ensureCurrentEpoch` boundary flush + two-tier `efficiencyInputs` view + early-epoch underflow guard at `currentEpoch < 3` + divide-by-3 SMA + immutable `tvlOracle` + immutable `AuMM` + mutable `feeRecorder` / `emissionsRecorder` slots; 56/56 unit tests at H2b.6j); `src/ccb/ICCBMultiplier.sol` (RB-005 Stage F backport per OQ-23 at H1.x-bis); `src/emission/IBodenseeBootstrapChannel.sol` + `src/emission/BodenseeBootstrapChannel.sol` (F-0 bootstrap channel — AP piecewise integration + `IVault.unlock` AuMM DONATION callback per **H-D11**—**H-D14**; 29/29 unit tests at H3.7). **NEW since H3.8.4:** `src/emission/IEmissionDistributor.sol` (H4.1 interface — `recordScore(pool)` permissionless per **H-D17** + `recordDeposit` / `recordWithdrawal` AuMT-recorder-gated per **H-D16** + `claim(pool, user, to)` with IAuMM.mint at claim per **H-D20** + `pendingClaim` view per **H-D25** additive form + views and events per **H-D19** absolute-newScore ABI); `src/ccb/IEfficiencyOracle.sol` H4.1-bis in-place retrofit (added `recordEmissions(pool, aummAmountScaled18)` push signature per **H-D23** + `override` keyword on `EfficiencyOracle.recordEmissions` + sweep of `is IEfficiencyOracle` inheritors per **G16**); `src/emission/EmissionDistributor.sol` (H4.2—H4.7c concrete distributor — MasterChef/Synthetix two-tier topology per **H-D15** with global `accRewardPerScoreUnit` + per-pool `poolAccDebt` + per-pool `poolAccRewardPerLP` per **H-D24** + per-user `userLP` / `userRewardDebt` / `pendingBalance` per **H-D16** + **H-D25** crystallized-claim slot under mint-at-claim-only per **H-D20**; lazy accrual tick `_accrueGlobal` + `_settlePool` called on every mutating entry per **H-D21**; permissionless `recordScore` with CCBScore delegation per **H-D17** + AuMT-recorder-gated `recordDeposit` / `recordWithdrawal` per **H-D16** + `claim(pool, user, to)` with IAuMM.mint per **H-D20** + `pendingClaim` view per **H-D25**; F-3/F-7 share computation in-distributor per **H-D18** + absolute newScore external ABI + F12 signed-delta totalScore middleware per **H-D19** + accrual-time `recordEmissions` push at `_settlePool` boundary per **H-D23** + H4 LP-tranche stub returning `IAuMM.blockEmissionRate(block.number)` per **H-D21** pending H5 wiring; H4.7a/H4.7b claim + pendingClaim bundled at `13c1ae5`; abstract keyword dropped at H4.7c (`7ee706f`); 329 lines; 64/64 unit tests at H4.8.1—H4.8.9 cohort across scaffold + 8 sub-cohorts). **NEW since H4.8.10c:** `src/incendiary/IIncendiaryRegistry.sol` (H5.2 Stage L forward-dep interface stub per **H-D29** — `integratedSkim(uint256 from, uint256 to)` external view returning `uint256`; first file under `src/incendiary/`; H7 consolidated into H5.2 per **H-D29** Stage L deployment narrative); `src/emission/EmissionDistributor.sol` H5.1a—H5.1e + H5.3b/d/e + H5.4.0g refit (governance-settable `incendiaryRegistry` slot + `setIncendiaryRegistry(address)` `onlyGovernance` setter per **H-D29** zero-address-acceptable deprecation valve at H5.1a; private `_bootstrapApSum` helper at H5.1b bit-for-bit mirror of `BodenseeBootstrapChannel._apSum` per **H-D27** F-0 bootstrap AP closed-form; private view `_phaseAwareBody` three-phase dispatcher at H5.1c per **H-D27/H-D28/H-D29** — bootstrap A/B AP split at `month6End` with conservation identity `rate × n − _bootstrapApSum` + transition `rate × n` no-subtract + continuous `rate × n − IIncendiaryRegistry.integratedSkim` with `address(0)` zero-stub; internal view `_lpTrancheIntegral(from, to)` era-splitting cursor walk at H5.1d per **H-D26/H-D30** — cursor walks with `AureumTime.eraIndex` + `nthHalvingBlock` split + constant `blockEmissionRate(cursor)` per era sub-interval + `_phaseAwareBody` dispatch for inner phase split; `_accrueGlobal` body rewrite at H5.1e per **H-D26** — `contribution = _lpTrancheIntegral(last+1, block.number)` supersedes **H-D21** H4 single-block sample for production accrual path; `IMiliariumRegistry` 6th typed immutable + `f5Total` / `f5Score` storage slots + 7th constructor param `miliariumRegistry_` + `ZeroAddress` guard at H5.3b per **H-D31** 10-step flow prereq; private view `_alphaF3(uint256 block_)` linear F-3 α blend at H5.3d per **H-D32** — lower clamp `0` at `≤month10EndBlock` + upper clamp `1e18` at `≥year1EndBlock` + 437_999-block denominator; `recordScore` body rewrite at H5.3e per **H-D31** 10-step flow + **H-D33** reshape — Miliarium `(1e18−α).mulDown(f5Total/28) + α.mulDown(F5)` vs non-Miliarium `α.mulDown(F5)` Option A per §xxviii); `src/emission/IEmissionDistributor.sol` H5.4.0g G16 retrofit (added `IncendiaryRegistrySet(address indexed oldRegistry, address indexed newRegistry)` event declaration mirroring `AuMTContractSet` / `GovernanceTransferred` precedent; concrete contract's duplicate declaration removed); `test/unit/harness/EmissionDistributorHarness.sol` H5.4.0d (Harness `is EmissionDistributor` + 8-arg constructor pass-through + 2 public-view wrappers `extLpTrancheIntegral(uint256, uint256)` + `extLpTrancheEmission(uint256)` per **H-D34** test-side observation surface; preserves prod ABI minimality). **Test suite:** 470/470 unit tests green at H5.5b across 16 suites (36 TVLOracle + 56 EfficiencyOracle + 29 BodenseeBootstrapChannel + 100 EmissionDistributor + Stage G/F/E/D/C suites preserved); fork integration deferred to **H9**. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **H-D locks status:** H-D1—H-D6 + H-D8—H-D34 LOCKED; **H-D7** (AuMM.setMinter handoff timing — Stage H deploy vs Stage K governance migration) OPEN, lock at H10 alongside deployment script + multisig dry-run evidence. **No H10+ implementation findings landed yet** — `docs/STAGE_H_NOTES.md` Findings queue is empty at H5.5b close (H5.4.0g G16 strike on `IncendiaryRegistrySet` interface gap caught and closed at audit-time per `grep -rn "is IEmissionDistributor" src/ test/` enumeration discipline — textbook G16 application; not a novel finding).

**Branch:** `stage-h` at H5.5b commit `a9e4b06` (HEAD). Branched from `stage-g-complete` (`384a40b`) per the **C0 / D0 / E-D8 / F9 / G9** convention; `main` remains at `stage-g-complete` HEAD until Stage H close-of-stage merge at H12. `stage-h` snapshot push to `origin` deferred to H12.

Recent commit stack on `stage-h` (top of HEAD downward; full Completion Log in `docs/STAGE_H_PLAN.md`):

- `a9e4b06` — H5.5b: docs/STAGE_H_PLAN.md — status header + Last update + Mode refresh (H5 family complete; 100/100 tests; H5.5c CLAUDE.md §11 next; 215 lines)
- `b7e9cc2` — H5.5a: docs/STAGE_H_PLAN.md — Completion Log backfill 25 rows H5.0b—H5.4.0g (190 → 215 lines)
- `5009a0c` — H5.4.0g: test/unit/EmissionDistributor.t.sol — 4 setIncendiaryRegistry governance setter tests (100/100 green; 1451 → 1482 lines); fix IEmissionDistributor.sol — move IncendiaryRegistrySet event from concrete contract to interface per G16
- `8f1fe3e` — H5.4.0f.5: test/unit/EmissionDistributor.t.sol — 6 H-D26 conservation invariant tests (LP+bootstrap+skim=rate×n property; 96/96 green; 1368 → 1451 lines)
- `620d92d` — H5.4.0f.4: test/unit/EmissionDistributor.t.sol — 5 continuous-phase Incendiary skim tests (DefaultRegistry + NonZeroRegistry/ZeroSkim + PositiveSkim/Subtracts + RevertWhen registry reverts + RegistryRebindToZero deprecation valve; 90/90 green; 1291 → 1368 lines)
- `85a0ff1` — H5.4.0f.3: test/unit/EmissionDistributor.t.sol — 6 phase-boundary nesting tests for extLpTrancheIntegral per H-D27/H-D28/H-D29 (CrossM6 ALeg→BLeg + CrossM10 BLeg→transition + CrossY1 transition→continuous + CrossM6AndM10 + CrossM10AndY1 + FullSweep; 1208 → 1291 lines; 85/85 green)
- `9dbce93` — H5.4.0f.2: test/unit/EmissionDistributor.t.sol — 6 era-boundary cursor walk tests for extLpTrancheIntegral per H-D30 (single-era + straddle-constant + straddle-halved via vm.mockCall + exact-era-end + era1-start + three-era-halving; 1144 → 1208 lines; 79/79 green)
- `fac7e95` — H5.4.0f.1: test/unit/EmissionDistributor.t.sol — 6 AP arithmetic identity tests for extLpTrancheIntegral + `_apSumFixture` closed-form helper mirroring `_bootstrapApSum` per H-D27 + H-D34 (1062 → 1144 lines; 73/73 green)
- `df2af18` — H5.4.0e: test/unit/EmissionDistributor.t.sol — distributor slot retype EmissionDistributor → EmissionDistributorHarness + setUp construction retype + harness import per H-D34 (1061 → 1062 lines; 67/67 green)
- `5b25585` — H5.4.0d: test/unit/harness/EmissionDistributorHarness.sol — Harness scaffold `is EmissionDistributor` + 8-arg constructor pass-through + `extLpTrancheIntegral` + `extLpTrancheEmission` wrappers per H-D34 (37 lines; forge build green)
- `e69d45a` — H5.4.0c: docs/STAGE_H_PLAN.md — status header + Last update + Mode + sub-step skeleton H5 row refresh (HEAD ccc29d9; H-D1—H-D34 LOCKED; 17 commits since H5.0a 841fe24 reflected; 190 lines)
- `ccc29d9` — H5.4.0b: docs/STAGE_H_PLAN.md — H-D31—H-D34 Decisions rows added to table (186 → 190 lines)
- `73c5979` — H5.4.0a: docs/STAGE_H_NOTES.md — H-D34 LOCKED (test-side observation surface for `_lpTrancheIntegral` + `_lpTrancheEmission` via Harness contract; preserves prod ABI minimality; 256 → 260 lines)
- `7b75aad` — H5.3f-fix1+fix2: test/unit/EmissionDistributor.t.sol — rational literal cast `uint256(100e18)/28` (fix1) + 16-test `vm.roll` migration to α=1e18 continuous regime + Accrual `lastAccrualBlock` assertion rebase to year1EndBlock (fix2); 67/67 green; H5.3 family complete (1045 → 1061 lines)

Earlier Stage H commits (H0—H4.8.10 close-of-family + H5.0—H5.3f reshape chain) live in `git log stage-h ^stage-g-complete` between branch point `384a40b` and `a9e4b06` HEAD (141 commits total). Full Completion Log in `docs/STAGE_H_PLAN.md`.
...
**Stage H position:** **H1—H5 families complete; H5.5 close-of-family sweep (a/b landed, c this commit); H6 implementation pending.** Sub-step roll-up: H0 plan / notes scaffold + H-D1—H-D5 entry-time locks → H1 architecture + schedules + H-D6 (1/28 literal supply-deflationary) + H-D8 (constellation roster) + H-D9 (TVLOracle balance-ratio averaging) + H-D10 (EfficiencyOracle ring buffer + H-D10 v2 svZCHF numéraire amendment at H1.9-bis) + H1.x-bis ICCBMultiplier RB-005 retrofit → H2a `TVLOracle` (36/36 tests + `quoteSvZCHF` push primitive at H2a.10a/b + `ITVLOracle` retrofit at H2a.10c per **G16**) → H2b `EfficiencyOracle` (56/56 tests) → H3 `BodenseeBootstrapChannel` (29/29 tests) → H3.8 close-of-family sweep (RB-005 close per RB-004 canonical) → H4.0 entry-time scaffold (H4.0a NOTES freeze H-D15—H-D23 + H4.0b/c PLAN refresh) → H4.1 `IEmissionDistributor` interface + H4.1-bis `IEfficiencyOracle.recordEmissions` retrofit per **G16** → H4.2 EmissionDistributor scaffold + H4.3 constructor + H4.4 governance setters + H4.5 internals (`_accrueGlobal` + `_settlePool`) + H4.6 mutators (`recordScore` + `recordDeposit` + `recordWithdrawal`) + H4.7a/b claim + pendingClaim bundle at `13c1ae5` + H4.7c abstract-keyword drop → H4.8.1—H4.8.9 unit test cohort (64/64 green across scaffold + 8 sub-cohorts) → H4.8.10a—d close-of-family sweep → **H5.0a NOTES H-D26—H-D30 + H5.0b PLAN refresh** → **H5.1 helpers family** (H5.1a `incendiaryRegistry` slot + setter per **H-D29** + H5.1b `_bootstrapApSum` per **H-D27** + H5.1c `_phaseAwareBody` per **H-D27/H-D28/H-D29** + H5.1d `_lpTrancheIntegral` era-splitting per **H-D26/H-D30** + H5.1e `_accrueGlobal` rewrite per **H-D26** superseding **H-D21** single-block sample for production accrual) → **H5.2** `src/incendiary/IIncendiaryRegistry.sol` Stage L forward-dep stub per **H-D29** (first file under `src/incendiary/`; H7 consolidated) → **H5.3 `recordScore` reshape** (H5.3a NOTES H-D31—H-D33 + H5.3b `IMiliariumRegistry` 6th immutable + `f5Total` / `f5Score` storage per **H-D31** + H5.3c G16 inheritor sweep `MockMiliariumRegistry` + H5.3d `_alphaF3` linear F-3 α blend per **H-D32** + H5.3e body rewrite to 10-step flow + **H-D33** reshape + H5.3f / H5.3f-fix1+fix2 test cohort migration to α=1e18 continuous regime) → **H5.4 Harness observation surface** (H5.4.0a NOTES H-D34 + H5.4.0b/c PLAN refresh + H5.4.0d `test/unit/harness/EmissionDistributorHarness.sol` per **H-D34** + H5.4.0e setUp retype + H5.4.0f.1—.5 observation cohort across AP identity + era cursor walk + phase nesting + continuous-phase Incendiary skim + H-D26 conservation invariant + H5.4.0g `incendiaryRegistry` setter cohort + IncendiaryRegistrySet interface fix per **G16**) → **H5.5 close-of-family sweep** (H5.5a Completion Log 25-row backfill + H5.5b status header refresh + H5.5c CLAUDE.md §11 refresh). Per-sub-step detail in `docs/STAGE_H_PLAN.md` Completion Log + Decisions table (H-D1—H-D6 entry-time + mid-stage H-D8—H-D14 + H4 entry-time H-D15—H-D23 + H4-implementation H-D24—H-D25 + **H5 entry-time H-D26—H-D30** + **H5.3 entry-time H-D31—H-D33** + **H5.4 H-D34**) and `docs/STAGE_H_NOTES.md` H-D-numbered bodies (notably **H-D26** `_lpTrancheIntegral` interval integration superseding **H-D21** single-block sample for production accrual; **H-D27** bootstrap leg AP integration mirrors `BodenseeBootstrapChannel._apSum` bit-for-bit; **H-D28** transition leg `rate × n` no subtraction — F-3 α-blend lives in `recordScore` middleware per **H-D18**; **H-D29** continuous-leg F-7 Step 1 Incendiary skim with mutable `incendiaryRegistry` slot + interval-form `integratedSkim` interface + Stage H zero-stub; **H-D30** era-boundary splitting at `nthHalvingBlock(g, k)`; **H-D31** `recordScore` 10-step flow + `IMiliariumRegistry` 6th immutable + `f5Total` / `f5Score` storage; **H-D32** `_alphaF3` linear blend with endpoint clamping; **H-D33** effective score reshape Option A — non-Miliarium joins α-CCB leg during F-3; **H-D34** Harness observation surface preserving prod ABI minimality). **H-D7** remains OPEN; locks at H10.

**Deferred (Stage G carry-forward + Stage H in-flight):**

- **Fork integration test for the Stage H stack.** `test/fork/StageHIntegration.t.sol` (Stage E pilots + Stage F / G stack + BodenseeBootstrapChannel + EmissionDistributor against real Bodensee via mainnet fork) deferred to **H9** per the Stage G precedent. Unit cohorts cover the audit surface; fork exercise validates `IVault.unlock` callback path against actual Balancer V3 Vault bytecode and end-to-end emission flow.
...
**Next sub-step: H6 — AuMM minter plumbing + AuMT claim interface surface for Stage I.** H6 wires (a) `IAuMM.setMinter(address(EmissionDistributor))` one-shot at deployment per **H-D7** (handoff timing locks at H10 alongside deployment script + multisig dry-run evidence; H6 is the implementation/test pre-pass that prepares the EmissionDistributor as the AuMM mint authority and exercises the claim-time mint path under harness setup); (b) AuMT claim-interface surface exposed for Stage I — the consumer-side AuMT receipt token implementation lands at Stage I, but H6 ensures the EmissionDistributor's outward-facing claim signatures (`claim(pool, user, to)` per **H-D20** + `pendingClaim(pool, user)` view per **H-D25** additive form) are stable and unit-tested against the Stage I integration contract surface. Entry per §13 Stage-level defaults: **Opus high** (in-stage continuation — Stage H entry was Opus extra-high at H1—H3 oracles + bootstrap channel + H4 distributor pre-flight, past-entry Opus high; H6 has design surface around H-D7 handoff timing but no novel architectural decisions vs H4—H5 patterns).
