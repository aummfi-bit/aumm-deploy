...
**Last update:** 2026-05-20, **Stage H IN PROGRESS** on branch `stage-h` (HEAD `86e0929`). H1—H3 families complete; H4 family complete (H4.0 entry-time scaffold + H4.1 IEmissionDistributor interface + H4.1-bis IEfficiencyOracle.recordEmissions push-signature retrofit + H4.2—H4.7 EmissionDistributor implementation family + H4.7c abstract-keyword drop + H4.8 unit test cohort 64/64 green); H4.8.10 close-of-family sweep in flight (H4.8.10a/b/c landed; H4.8.10d CLAUDE.md §11 refresh is this commit). Branch carries 112 sub-step commits beyond `stage-g-complete` (`384a40b`). **Concrete deliveries to date — Stage H stack through H4.7c:** `src/emission/TVLOracle.sol` (constructor-injected `tokenToUnderlying` seed loop + governance-extensible constellation roster per **H-D8** + Step 1 single `IVaultExplorer.getPoolData.balancesLiveScaled18[]` read + Step 2 `_constellationRatio` balance-ratio averaging across `underlyingToPools[U]` reverse-map per **H-D9** + `quoteSvZCHF(token, amountScaled18)` push primitive at H2a.10a per **H-D10 v2**; 36/36 unit tests at H2a.10b); `src/ccb/ITVLOracle.sol` retrofit at H2a.10c (`quoteSvZCHF` interface + `override` keyword on `TVLOracle.quoteSvZCHF` + return-0 stubs in all three `MockTVLOracle` inheritors per **G16** inheritor enumeration discipline); `src/emission/EfficiencyOracle.sol` (intra-epoch accumulation per **H-D10** — `recordFees(pool, token, amountScaled18)` + `recordEmissions(pool, aummAmountScaled18)` push entries routed through `tvlOracle.quoteSvZCHF` for same-unit svZCHF accumulation per **G-D23 (i)** + `EpochEntry[3]` ring buffer with epoch-stamp stale detection + O(1) `_ensureCurrentEpoch` boundary flush + two-tier `efficiencyInputs` view + early-epoch underflow guard at `currentEpoch < 3` + divide-by-3 SMA + immutable `tvlOracle` + immutable `AuMM` + mutable `feeRecorder` / `emissionsRecorder` slots; 56/56 unit tests at H2b.6j); `src/ccb/ICCBMultiplier.sol` (RB-005 Stage F backport per OQ-23 at H1.x-bis); `src/emission/IBodenseeBootstrapChannel.sol` + `src/emission/BodenseeBootstrapChannel.sol` (F-0 bootstrap channel — AP piecewise integration + `IVault.unlock` AuMM DONATION callback per **H-D11**—**H-D14**; 29/29 unit tests at H3.7). **NEW since H3.8.4:** `src/emission/IEmissionDistributor.sol` (H4.1 interface — `recordScore(pool)` permissionless per **H-D17** + `recordDeposit` / `recordWithdrawal` AuMT-recorder-gated per **H-D16** + `claim(pool, user, to)` with IAuMM.mint at claim per **H-D20** + `pendingClaim` view per **H-D25** additive form + views and events per **H-D19** absolute-newScore ABI); `src/ccb/IEfficiencyOracle.sol` H4.1-bis in-place retrofit (added `recordEmissions(pool, aummAmountScaled18)` push signature per **H-D23** + `override` keyword on `EfficiencyOracle.recordEmissions` + sweep of `is IEfficiencyOracle` inheritors per **G16**); `src/emission/EmissionDistributor.sol` (H4.2—H4.7c concrete distributor — MasterChef/Synthetix two-tier topology per **H-D15** with global `accRewardPerScoreUnit` + per-pool `poolAccDebt` + per-pool `poolAccRewardPerLP` per **H-D24** + per-user `userLP` / `userRewardDebt` / `pendingBalance` per **H-D16** + **H-D25** crystallized-claim slot under mint-at-claim-only per **H-D20**; lazy accrual tick `_accrueGlobal` + `_settlePool` called on every mutating entry per **H-D21**; permissionless `recordScore` with CCBScore delegation per **H-D17** + AuMT-recorder-gated `recordDeposit` / `recordWithdrawal` per **H-D16** + `claim(pool, user, to)` with IAuMM.mint per **H-D20** + `pendingClaim` view per **H-D25**; F-3/F-7 share computation in-distributor per **H-D18** + absolute newScore external ABI + F12 signed-delta totalScore middleware per **H-D19** + accrual-time `recordEmissions` push at `_settlePool` boundary per **H-D23** + H4 LP-tranche stub returning `IAuMM.blockEmissionRate(block.number)` per **H-D21** pending H5 wiring; H4.7a/H4.7b claim + pendingClaim bundled at `13c1ae5`; abstract keyword dropped at H4.7c (`7ee706f`); 329 lines; 64/64 unit tests at H4.8.1—H4.8.9 cohort across scaffold + 8 sub-cohorts). **Test suite:** 434/434 unit tests green at H4.8.10c across 16 suites (36 TVLOracle + 56 EfficiencyOracle + 29 BodenseeBootstrapChannel + 64 EmissionDistributor + Stage G/F/E/D/C suites preserved); fork integration deferred to **H9**. **Vault bytecode untouched** — zero edits to `lib/balancer-v3-monorepo/`. **H-D locks status:** H-D1—H-D6 + H-D8—H-D25 LOCKED; **H-D7** (AuMM.setMinter handoff timing — Stage H deploy vs Stage K governance migration) OPEN, lock at H10 alongside deployment script + multisig dry-run evidence. **No H10+ implementation findings landed yet** — `docs/STAGE_H_NOTES.md` Findings queue is empty at H4.8.10c close.

**Branch:** `stage-h` at H4.8.10c commit `86e0929` (HEAD). Branched from `stage-g-complete` (`384a40b`) per the **C0 / D0 / E-D8 / F9 / G9** convention; `main` remains at `stage-g-complete` HEAD until Stage H close-of-stage merge at H12. `stage-h` snapshot push to `origin` deferred to H12.

Recent commit stack on `stage-h` (top of HEAD downward; full Completion Log in `docs/STAGE_H_PLAN.md`):

- `86e0929` — H4.8.10c: docs/STAGE_H_PLAN.md — status header refresh (H4 complete + H4.8 unit cohort + H4.8.10a/b landed; H-D1—H-D25 LOCKED) + H-D24/H-D25 Decisions rows + H4.8.10a/b Completion Log self-rows + Mode line tense refresh
- `9a118c2` — H4.8.10b: docs/STAGE_H_PLAN.md — Completion Log rows H4.8.1—H4.8.9 (9 rows: EmissionDistributor unit test cohort family; scaffold + Constructor + governance setters + recordScore + recordDeposit + recordWithdrawal + claim + pendingClaim view + accrual+settle math; 64/64 green)
- `dff5875` — H4.8.10a: docs/STAGE_H_PLAN.md — Completion Log rows H4.0c—H4.7c (22 rows: H4 implementation family backfill; IEmissionDistributor interface + IEfficiencyOracle.recordEmissions retrofit + EmissionDistributor scaffold/constructor/setters/internals/mutators/claim; H4.0d label collision + H4.7a+b bundle noted inline)
- `5cb7b80` — H4.8.9: test/unit/EmissionDistributor.t.sol — accrual + settle math cohort (8 tests; 64/64 green; 840 → 945 lines)
- `eba884d` — H4.8.8: test/unit/EmissionDistributor.t.sol — pendingClaim view cohort (8 tests; 56/56 green)
- `8900996` — H4.8.7: test/unit/EmissionDistributor.t.sol — claim test cohort (8 tests; 48/48 green)
- `02c2c15` — H4.8.6: test/unit/EmissionDistributor.t.sol — recordWithdrawal test cohort (8 tests; 40/40 green)
- `0bc3f86` — H4.8.5: test/unit/EmissionDistributor.t.sol — recordDeposit test cohort (8 tests; 32/32 green)
- `e1873cc` — H4.8.4: test/unit/EmissionDistributor.t.sol — recordScore test cohort (8 tests; 24/24 green)
- `c5b34b6` — H4.8.3: test/unit/EmissionDistributor.t.sol — governance setter test cohort (8 tests; 16/16 green)
- `55ba482` — H4.8.2: test/unit/EmissionDistributor.t.sol — Constructor test cohort (8 tests; 8/8 green)
- `6789fba` — H4.8.1: test/unit/EmissionDistributor.t.sol — scaffold (5 mocks + setUp + helpers; no test functions)
- `7ee706f` — H4.7c: src/emission/EmissionDistributor.sol — drop `abstract` keyword from L17 (all IEmissionDistributor functions implemented after H4.7a/H4.7b bundle; fully concrete; 329 lines)
- `13c1ae5` — H4.7a+b: src/emission/EmissionDistributor.sol — claim(pool, user, to) + pendingClaim(pool, user) bundle per **H-D20** mint-at-claim + **H-D25** additive form (318 → 329 lines; commit message names H4.7b only — H4.7a/H4.7b bundled inline)

Earlier Stage H commits (H0—H3.8 close-of-family sweep + H4.0—H4.6c implementation chain) live in `git log stage-h ^stage-g-complete` between branch point `384a40b` and `86e0929` HEAD (112 commits total). Full Completion Log in `docs/STAGE_H_PLAN.md`.
...
**Next sub-step: H5 — wire F-0 / F-3 / F-7 phase-aware emission subtractions into `_lpTrancheEmission` stub at `src/emission/EmissionDistributor.sol` L174-L176.** Per **H-D21** the stub currently returns `IAuMM(AuMM).blockEmissionRate(block.number)` unmodified; H5 subtracts (a) F-0 Bodensee share for Months 0—10 (closed-form share matching `BodenseeBootstrapChannel.accrue` AP rate at block); (b) F-3 α-blend transition for Months 11—12 per `11_formulas.md` F-3; (c) F-7 Step 1 Incendiary skim for Year 2+ per F-7 standard (Incendiary contract lands at Stage L — H5 wires the subtraction with a configurable recipient slot defaulting to `address(0)` until Stage L crystallization). Entry per §13 Stage-level defaults: **Opus extra-high** (F-0 / F-3 / F-7 phase-boundary arithmetic at block-era boundaries — halving math + signed-delta arithmetic per F12 standard). Drops to **Opus high** after F-0 / F-3 / F-7 wiring lock.
