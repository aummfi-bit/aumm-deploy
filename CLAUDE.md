...
**Last update:** 2026-06-11, **Stage K COMPLETE** on branch `stage-k`, **K0—K7 complete**, full split-form regression 852/852 green (759 unit + 93 fork, 0 failed). K0 entry (K0.1—K0.4); K1 `AureumGovernanceAuthorizer` (`73ceaae` + 12-green unit `e72104c`, K-D3 LOCKED); K2 TVL-oracle binding (K-D4 reuse-direct, no code surface, `3e8250d` / `29d4b71`); K3 `VotingWeight` value-weighted reader (`50d9f95` + 29-green unit cohort, K-D5 LOCKED); K4 `AureumGovernance` (K-D6 LOCKED `4e3b5e6`) — three proposal types + snapshot voting + 2-day timelock, contract `d7cc7b5`—`dc7eb99`, ~600-line unit suite `c2dd343`—`828eb86`, 6/6 `StageKIntegration` fork suite `dcf94f9` / `3f845ea` / `dd37cd1`, findings K10—K12; K5 `AuMMMinterRouter` minter handoff (K-D7 LOCKED — Option B dumb allowlist forwarder, not a bare `setMinter`): interface K5.1 (`47aaddc`), forwarder K5.2 (`104e724`), both consumer fix-forwards K5.3a/b (`99d2f92`—`45335d0`), and the K5.4 test ladder (router 10/10, channel 34/34, distributor 106/106 green), finding K13. K6 `TVLOracle` roster re-wire to the live `MiliariumRegistry` dense enumeration (K-D8 LOCKED): additive `setMiliariumRegistry` self-seal K6.1 (`5a0a809`), `_constellationRatio` two-leg re-wire K6.2 (`e05b52d`), test cohort 42/42 K6.3 (`aef852c`), Completion-Log catch-up K6-log (`f5b85ee`). K7 deploy + authorizer migration (K-D9 LOCKED): `DeployAuMM` K7.1 (`d6bc52c`), `DeployStageK` deploy + wire K7.2 (`7f8cf17` / `04b58b2`), `DeployStageK.t.sol` A/B/C fork assertions K7.3 (`2ee9e85`—`fe660c3`, 5/5 green), K13 `MintRouterNotSet` blast-radius retrofit K7.4 (`AuMMDistributorIntegration` 4/4 + `StageHIntegration` 10/10 via the K5.4 router-wiring pattern, `50b639a` / `68cf6e3` / `6d5ae37`), Completion-Log catch-up K7.4-log (`78a0894`). This commit is **K-§11** (§11 refresh at Stage K close). **Stage K = the governance handoff** (STAGES_OVERVIEW Stage K; "Largest stage"): ship on-chain governance (`AureumGovernance` — three proposal types) + the value-weighted voting reader (`VotingWeight`, I-D15) + the B-strict 12-month time-bomb authorizer (`AureumGovernanceAuthorizer`, OQ-10), and migrate the Vault's authorizer from the Stage B Safe to the governance contract. **K-D1—K-D9 LOCKED. Vault bytecode untouched** — the migration is `setAuthorizer` / `setStaticSwapFeePercentage` CALLS, never edits. **Stage J context:** J-D1—J-D8 LOCKED, Finding J10; tag `stage-j-complete` (`83ef71e`), ff-merged to `main`, pushed to `origin`.

**Branch:** `stage-k` at **K-§11** (this `CLAUDE.md` §11 commit; prior `78a0894` K7.4-log, `6d5ae37` K7.4b-ii, `68cf6e3` K7.4b-i). Full K0—K7 stack on the branch; Stage K functionally complete, regression 852/852 green. Branched from `stage-j-complete` (`83ef71e`) per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention (and Stages I, J). `main` sits at `stage-j-complete` HEAD until the K-close ff-merge; `stage-k` snapshot push to `origin` lands at K-close.

Recent commit stack — Stage K (full per-sub-step catalog in the `docs/STAGE_K_PLAN.md` Completion log):

- **K0** (entry): K0.1 branch (`83ef71e` base) / K0.2 NOTES freeze (`e241c22`) / K0.3 PLAN roadmap (`597880e`) / K0.4 §11 refresh (`5a165a9`).
- **K1** `AureumGovernanceAuthorizer` (K-D3 LOCKED): K1.0a NOTES (`130c39d`) / K1.0b PLAN (`e3b8f57`) / K1.1 contract (`73ceaae`) / K1.2 unit 12-green (`e72104c`).
- **K2** TVL-oracle binding (K-D4 reuse-direct, no code surface): K2.0a NOTES (`3e8250d`) / K2.0b PLAN (`29d4b71`).
- **K3** `VotingWeight` (K-D5 LOCKED): K3.0a NOTES (`d66688b`) / K3.0b PLAN (`0b790c3`) / K3.0c `effectiveQualBlock` exposure (`888f0bd`) / K3.1 contract (`50d9f95`) / K3.2a—c unit 29-green (`4d81ffc` / `8e345b5` / `d2f90d1`) / K3.3a NOTES fold (`e69652c`) / K3.3b PLAN fold (`1ad18ea`) / K3.4 §11 refresh (`632e23d`). K3.3 fork test folded into K4 `StageKIntegration`.
- **K4** `AureumGovernance` (K-D6 LOCKED): K4.0a NOTES (`4e3b5e6`) / K4.0b PLAN (`8ca9d3b`) / K4.1 scaffold (`d7cc7b5`) / K4.2 propose family (`e88a8f8`) / K4.2-post-a K10 NOTES (`1625ec4`) / K4.2-post-b `IVotingWeight.poke` + mock stubs (`81d246d`) / K4.3 castVote (`dd0dbf2`) / K4.4 state, tally, getProposal (`5523e71`) / K4.5 queue, execute (`dc7eb99`) / K4.6a—e unit suite (`c2dd343` / `a5f615c` / `01c3db0` / `a6b18c4` / `828eb86`) / K4.6b-post K11 NOTES (`44fb7ec`) / K4.7.0a fork design (`594eb5b`) / K4.7a fixture (`dcf94f9`) / K4.7b-fix1 R1 (`e599b0e`) / K4.7b poke tests (`3f845ea`) / K4.7c composition lifecycle (`dd37cd1`) / K4.7c-notes K12 (`f68c978`) / K4-log Completion catch-up (`bc682a0`). 6/6 `StageKIntegration` fork suite green; findings K10—K12.
- **K5** `AuMMMinterRouter` minter handoff (K-D7 LOCKED — Option B dumb allowlist forwarder): K5.0a NOTES K-D7 lock (`5058f8f`) / K5.0b PLAN (`056e08c`) / K5.1 `IAuMMMinterRouter` (`47aaddc`) / K5.2-pre event-surface §12 gate (`e3e60a8`) / K5.2 forwarder (`104e724`) / K5.3a-i—ii channel fix-forward (`99d2f92` / `e866682`) / K5.3b-i—iii distributor fix-forward (`4950059` / `fcdccac` / `45335d0`) / K5.4-pre-a NOTES K13 (`f1cc54a`) / K5.4-pre-b PLAN (`7d97a0b`) / K5.4a router suite 10/10 (`36de306`) / K5.4b-i—ii channel retrofit+cohort 34/34 (`cea9ad9` / `8a63db6`) / K5.4c-i—ii distributor retrofit+cohort 106/106 (`03c528c` / `ad7566e`) / K5.4-log Completion catch-up (`81793f3`). Finding K13 (`MintRouterNotSet` blast radius, I13-class). Fork transition coverage folds into K7 per K-D9.
- **K6** `TVLOracle` roster re-wire to the live `MiliariumRegistry` dense enumeration (K-D8 LOCKED): K6.0a NOTES (`bd28e72`) / K6.0b PLAN (`7fc8a85`) / K6.1 additive `setMiliariumRegistry` self-seal (`5a0a809`) / K6.2 `_constellationRatio` two-leg re-wire (`e05b52d`) / K6.3 test cohort 42/42 (`aef852c`) / K6-log Completion catch-up (`f5b85ee`). J-D8 carry-forward, I13-class fix-forward on the tagged Stage H contract; fork transition coverage folds into K7 per K-D9.
- **K7** deploy + authorizer migration (K-D9 LOCKED): K7.0a NOTES (`9502025`) / K7.0b PLAN (`6c2d5cf`) / K7.1 `script/DeployAuMM.s.sol` H-D42 (`d6bc52c`) / K7.2.0a—b K14 NOTES+PLAN (`ea9f2c0` / `0feeb9b`) / K7.2a deploy triad (`7f8cf17`) / K7.2b 8-call wiring (`04b58b2`) / K7.3.0a—b K15 NOTES+PLAN (`2274eca` / `9958fe9`) / K7.3a fixture (`2ee9e85`) / K7.3b B+C assertions (`3d46d1d`) / K7.3c A assertion 5/5 (`fe660c3`) / K7.3.1a—b K16 NOTES+PLAN (`d0692e0` / `204f3e1`) / K7.4.0a—b K17 NOTES+PLAN (`678a81b` / `8cbf9eb`) / K7.4a unit retrofit 4/4 (`50b639a`) / K7.4b-i fork unification 10/10 (`68cf6e3`) / K7.4b-ii I10 NatSpec sweep (`6d5ae37`) / K7.4-log Completion catch-up (`78a0894`). 5/5 `DeployStageK` fork green; full regression 852/852; findings K14—K17.

**Current tag:** `stage-j-complete` (commit `83ef71e`, 2026-06-07 close) is the latest stage tag; `stage-k-complete` is cut at K-close. `stage-i-complete`, `stage-h-complete`, `stage-g-complete`, `stage-f-complete`, `stage-e-complete`, `stage-d-complete`, `stage-c-complete`, `stage-b-complete`, and `stage-a-complete` remain in place upstream.

**Stage K — what it builds (7 dependency-ordered units per K-D2):** **K1** `AureumGovernanceAuthorizer` — the B-strict authorizer per OQ-10, immutables `GOVERNANCE_CONTRACT` / `EMERGENCY_MULTISIG` / `EMERGENCY_WINDOW_END_BLOCK`, `EMERGENCY_WINDOW_BLOCKS = 2_628_000` (12 protocol months), `canPerform` routing (governance = all authorizer-gated actions; multisig = emergency-action-set ∧ `block.number < EMERGENCY_WINDOW_END_BLOCK`). **K2** TVL-oracle binding for vote-weighting (K-D4: reuse `src/emission/TVLOracle.sol`, recommended). **K3** `VotingWeight` implementing the I9.1 `IVotingWeight` stub — `(qualified_AuMT_value × time_in_pool_capped)^(1/4 → 1/3)` over the EmissionDistributor recorder clock, 14-day cliff, 6-month on-ramp, F-9 era root, gauged-only (I-D15); `totalSupply()` veto-denominator semantics lock here (I-D17). **K4** `AureumGovernance` — three proposal types (gauge challenge F-12 / composition challenge / fee proposal) + F-9 dampening + snapshot voting + timelock (the largest unit; sub-splits K-D6a—K-D6f at K4 pre-flight). **K5** `AuMM.setMinter(EmissionDistributor)` handoff (H-D41) — deploy-script call, unblocks `distribute()` / `claim()` from `NotMinter`. **K6** `TVLOracle` roster re-wire to the live `MiliariumRegistry` dense enumeration (J-D8, I13-class fix-forward). **K7** deploy + authorizer migration — `script/DeployAuMM.s.sol` (H-D42) + `script/DeployStageK.s.sol` + `test/fork/DeployStageK.t.sol`, multisig one-shot `Vault.setAuthorizer`. Detail in `docs/STAGE_K_PLAN.md` (roadmap + Completion log + Decisions mirror) and `docs/STAGE_K_NOTES.md` (K-D1—K-D9 + bodies). Scope tight per K-D1: **D33 Router → Stage O; OQ-20 / OQ-21 yield-fee leg stays deferred** (user scope decision 2026-06-07).

**Deferred carry-forward.** Items Stage K now consumes: **H-D41** `AuMM.setMinter` handoff → **K5**; **H-D42** `script/DeployAuMM.s.sol` (~30 lines, before `DeployDerBodensee.s.sol` / `DeployStageH.s.sol`) → **K7**; **I-D15** `VotingWeight` value-weighted view + **I-D17** `totalSupply()` semantics → **K3**; **J-D8** `TVLOracle` H-D8 roster re-wire → **K6**. Items still deferred beyond Stage K:

- **D33 — Aureum's own Router.** Re-entry firmly at **Stage O** (integration) per K-D1; the mainnet Balancer Router is Vault-bound (D32), so Aureum pools need their own.
- **OQ-20 / OQ-21.** Controller-initiated `routeYieldFee` entry point + `BLOCKS_PER_EPOCH` cadence throttle per E-D10 — open design question, stays deferred (user scope decision 2026-06-07).
- **Direct `ReentrancyGuard.selector` test via `vm.transientStore`.** H3.7e `RevertWhen_Reentrant` covers the gate transitively; direct slot-pre-seeding deferred to a robustness-backport entry.
- **OQ-22 2-hop fallback.** H-D9 Phase 1 ships direct venues only; 2-hop through ZCHF / sUSDS per FINDINGS L1115 deferred.
- **D36 permanent fix.** Env-key namespacing in `test/fork/DeployAureumVault.t.sol` carried forward; fork tests race-safe via the E-D24 in-process pattern + `--threads 1` belt per F-D11.
- **aumm-site spec edits (user-side).** `04_tokenomics.md` §viii / §ix amendments per OQ-20 + OQ-21 + the I-reframe AuMT=BPT reckoning (I-D14—I-D17); `aumm-site` §xxi sandbox fast-track row obsolete per **G-D2** — spec edit deferred.
- **OQ-I10 Stage M/N recorder-binding rollout.** Stage I bound the recorder clock + hook dispatch for the 3 pilot pools (slots 02/03/07); extending `setAuMTContractForPool` to slots 01/04/05/06/08—28 is deferred to Stage M/N.

**Next sub-step: Stage K close** — the git mutations that finalize the stage (user's terminal): `stage-k-complete` tag on the branch tip, ff-merge `stage-k` → `main`, and push `main` + tag + `stage-k` to `origin`. After the close, **Stage L (Incendiary)** opens per STAGES_OVERVIEW — branched from `stage-k-complete`, Opus-high entry per §13; the Stage L entry beat does the terminal §11 refresh (Stage K→L), mirroring K0.4. **Stage K held its scope ceiling per K-D1: D33 Router → Stage O; OQ-20 / OQ-21 yield-fee leg deferred.**
