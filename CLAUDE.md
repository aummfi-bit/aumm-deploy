...
7. **Decimal mapping for `_seedAmounts()`:** ixEDEL = 18-dec STANDARD (no RP); svZCHF = 18-dec WITH_RATE (RP); GHO = 18-dec WITH_RATE (RP); WBTC = 8-dec STANDARD (no RP); cbBTC = 8-dec STANDARD (no RP). Address-sorted positions and per-decimal seed literals must match — verify against IxAurebitConfig at E3.3 entry. Do not hardcode the slot order in the IxAurebitPilotTest override before IxAurebitConfig is locked.
