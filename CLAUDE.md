...
**Next sub-step: H4.1 — `src/emission/IEmissionDistributor.sol` interface authoring per H-D15—H-D20.** Consumer-facing surface: `recordScore(pool)` permissionless per **H-D17** + `recordDeposit(pool, user, amount)` / `recordWithdrawal(pool, user, amount)` AuMT-recorder-gated per **H-D16** + `claim(pool, user)` with IAuMM.mint at claim per **H-D20** + views (`pendingReward(pool, user)` + `userLP(pool, user)` + `totalScore(pool)` + `accRewardPerScoreUnit` + `lastAccrualBlock` + `poolAccDebt(pool)`) + events (`ScoreRecorded(pool, oldScore, newScore)` per **H-D19** absolute-newScore ABI + `DepositRecorded` / `WithdrawalRecorded` / `RewardClaimed`). Sibling sub-step **H4.1.x-bis** — in-place extension of `src/gauge/IEfficiencyOracle.sol` to add `recordEmissions(address pool, uint256 aummAmountScaled18)` push signature per **H-D23** + `override` keyword on `EfficiencyOracle.recordEmissions` + sweep of all `is IEfficiencyOracle` inheritors per **G16** inheritor enumeration discipline (mirrors H2a.10c ITVLOracle.quoteSvZCHF retrofit). Entry per §13 Stage-level defaults: **Opus extra-high** (halving math + F12 type-discipline domain — signed-delta accumulator arithmetic per F12 standard at the totalScore middleware). Drops to **Opus high** after H4 core distributor lock.
