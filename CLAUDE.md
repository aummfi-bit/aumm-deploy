...
**Next sub-step: I7.1 — Stage I deploy + close-of-stage (pre-flight required).** I6 (recorder-clock fork integration) is COMPLETE (I6.1—I6.5d; `test/fork/StageIIntegration.t.sol` — `StageIWiringTest` / `StageIDepositTest` / `StageIAgingWithdrawalTest`, 4/4 fork green). The remaining Stage I work is the **I7 close-of-stage family**: (a) the **Deferred-from-I4** `setEmissionRecorder` unit-level two-flag-lock tests (one-shot recorder slot + the two boolean guards per I-D16; fork-covered at I6.1, unit coverage pending) as an I7-adjacent hook-test extension; (b) full split-form regression per D35/D36 (`--no-match-path "test/fork/**"` unit + `--match-path "test/fork/**" --fork-url $MAINNET_RPC_URL --threads 1` fork); (c) the `stage-i-complete` close-of-stage tag at I7.4 with fast-forward merge to `main` per the **C0 / D0 / E-D8 / F9 / G9 / H12** convention. The `script/DeployAuMM.s.sol` production deploy chain stays **deferred to the Stage K opening** per H-D42 (see Deferred). Entry mode: **Opus high** per §13 for the I7.1 pre-flight (decompose the deferred unit tests + close-of-stage reckoning), dropping to Sonnet for assertion-body and Completion-Log housekeeping.
