...
**Last update:** 2026-04-28, **Stage E mid-stage — E1 + E2 + E3 + E4 closed (E5 docs / E9 close ahead).** E1 delivered `AureumWeightedPoolFactory.sol` peer-inherited from `BasePoolFactory` (E-D11) — own external `create(...)` reverting `QualityGateUnsatisfied(uint256 erc4626WeightSum, uint256 minRequired)` at the 52% ERC-4626 gate (`MIN_ERC4626_WEIGHT = 52e16`); peer-inheritance — not subclassing — because upstream `WeightedPoolFactory.create` is `external`, not `virtual`, so a subclass cannot `override` and would inherit upstream's `create()` as a publicly-callable QG bypass. Vault bytecode untouched — zero edits to `lib/balancer-v3-monorepo/`. E1 artifacts include `script/pools/PoolConfig.sol` (E-D2 struct shape — `name`, `symbol`, `slot`, `sectorLabel`, `tokens`, `tokenTypes`, `rateProviders`, `paysYieldFees`, `normalizedWeights`, `swapFeePercentage`, `salt`); `script/pools/configs/01_ixHelvetia.s.sol`; `script/pools/deploy-miliarium-pool.s.sol` (abstract `MiliariumPoolDeployer` — `config()` / `run()` skeleton + factory call + post-deploy registration per E-D23); `DeployIxHelvetia.s.sol`; `test/fork/PilotPools.t.sol` (`MiliariumPilotPoolBase` — E-D24 in-process harness, `--threads 1`). Mid-stage OQ-11 supersession — Bodensee swap fee `0.75%` immutable per E-D22; Miliarium genesis `0.02%` per `MILIARIUM_SWAP_FEE_GENESIS`; band `0.01%–0.30%` per `MILIARIUM_SWAP_FEE_MIN/MAX`; controller rename `BODENSEE_SWAP_FEE_GENESIS → BODENSEE_SWAP_FEE`. E2 — ixEdelweiss (slot 05) configs + `IxEdelweissPilotTest` — waEthUSDC → svZCHF vector per E-D25; E10 — forge-std `deal(adjust=true)` vs Reserve-DTF `ixEDEL` — harness drops `, true` in `_initializePool` mint loop, per-decimal `_seedAmounts()`. E3 — ixAurebit (slot 14): `script/pools/configs/14_ixAurebit.s.sol` — IxAurebitConfig (WBTC 16% / Aave Prime GHO 26% / cbBTC 16% / ixEDEL 16% / svZCHF 26%, address-sorted literals, 52% ERC-4626 gate boundary); `DeployIxAurebit.s.sol`; `IxAurebitPilotTest` — `WBTC → svZCHF` fee-routing fork test (Standard `tokenIn` — per E-D25). E4 — `test/unit/AureumWeightedPoolFactory.t.sol` — QG revert-path unit tests (five methods, mock vault, per E-D13). Docs — E11 locks **Aave Prime GHO** naming; sweep renames GHO → Aave Prime GHO across `docs/STAGE_E_NOTES.md`, `docs/STAGE_E_PLAN.md`, `docs/FINDINGS.md`, `docs/STAGES_OVERVIEW.md`, §11; E-D26 — E4 unit-test scope, E-D9 closure track, Cursor NatSpec backtick→Markdown-hyperlink artifact. Pilot fork suite — ixHelvetia, ixEdelweiss, ixAurebit — D35 split-form `--threads 1`. Test counts post-E4: **156 unit + 12 fork = 168 total, 0 failures.** **Next: E5 — Docs (Completion Log fill in `docs/STAGE_E_PLAN.md` per E-D9; E-D7 L79 stale wording amendment per E10 — drop the third `, true` argument from `deal()`).**

**Branch:** `stage-e` ahead of `origin/stage-e` post `cf9fc37` (E-D26 NOTES). Branch point `57cd2ce` (per E-D8). No FF to `main` until E9 — §11 references tip-before-itself `cf9fc37` (E-D26 NOTES) per project convention.

Recent commit stack (`cf9fc37`-and-prior, all on `stage-e`):

- `cf9fc37` — docs: STAGE_E_NOTES.md E-D26 — E4 unit-test scope, E-D9 closure, Cursor NatSpec-hyperlink artifact
- `3d35244` — E4.1: test/unit/AureumWeightedPoolFactory.t.sol — QG revert-path unit tests (5 methods, mock vault, per E-D13)
- `5dec629` — docs: E11.2–E11.6 rename sweep — GHO → Aave Prime GHO across STAGE_E_NOTES / STAGE_E_PLAN / FINDINGS / STAGES_OVERVIEW / CLAUDE.md §11
- `6a92d3c` — docs: STAGE_E_NOTES.md E11 — Aave Prime GHO naming convention locked; GHO misnaming corrected
- `752c032` — docs: CLAUDE.md §11 refresh — Stage E mid-stage (E1 + E2 closed, E3 ixAurebit next)
- `4a5fcb6` — E3.3: test/fork/PilotPools.t.sol — IxAurebitPilotTest derived contract + fee-routing test (WBTC → svZCHF)
- `9a3bb18` — E3.2: script/pools/DeployIxAurebit.s.sol — concrete MiliariumPoolDeployer wrapper for ixAurebit (slot 14)
- `34ceacb` — E3.1: script/pools/configs/14_ixAurebit.s.sol — IxAurebitConfig per-pool config library (WBTC 16% / Aave Prime GHO 26% / cbBTC 16% / ixEDEL 16% / svZCHF 26%, slot 14)

Earlier Stage E commits (E0 through E2.3-fix, OQ-11 supersession landings, E-D24 supersession of E-D6 env-key suffix, E10 NOTES) live in `git log stage-e ^main` from branch point `57cd2ce` to `fd554c1`.
...
**Stage E position:** **E1 + E2 + E3 + E4 complete; E5 / E9 ahead.** Per-sub-step detail lives in `docs/STAGE_E_PLAN.md` Decisions table (E-D1 through E-D10 entry-time) + Mid-stage supersessions (OQ-11 swap fees / E-D6→E-D24 env-key suffix) + `docs/STAGE_E_NOTES.md` E-D-numbered findings (notably **E-D11** factory peer-inherit pattern, **E-D13** QG unit-test scope, **E-D16** AureumWeightedPoolFactory additive, **E-D17–E-D22** ixEdelweiss config locks, **E-D23** deploy script architecture — abstract base + per-pilot wrappers, env-var contract — **E-D24** in-process harness shape + `--threads 1` belt + INIT_SEED parity, **E-D25** ixEdelweiss / ixAurebit harness vectors, **E-D26** E4 closure + E-D9 + NatSpec-hyperlink artifact) and E-numbered findings (notably **E10** forge-std `deal()` vs Reserve-DTF, **E11** Aave Prime GHO naming) + git history on `stage-e` from branch point `57cd2ce` to `cf9fc37`. Per-sub-step Completion Log fills in `docs/STAGE_E_PLAN.md` happen at E5 docs sub-step (per E-D9), not per-sub-step.
...
- **E-D7 PLAN.md L79 wording stale post-E10.** `docs/STAGE_E_PLAN.md` L79 still shows `deal(token, address(this), amount, true)` per E-D7 — resolution deferred to E5 docs sub-step (amend per E10 — drop the third `, true` argument from `deal()`).
- **NEW — Cursor NatSpec backtick→Markdown-hyperlink artifact per E-D26.** Cosmetic-only — optional E5 revert; not load-bearing for protocol code.
...
**How to resume (Stage E → E5 Docs):**

1. Confirm current tip: `git log --oneline -1` on `stage-e` returns `cf9fc37` (E-D26 NOTES) or later — §11 anchors tip-before-itself at `cf9fc37` until the next refresh.
2. Confirm clean working tree: `git status -sb` shows only the active branch tracking line, no modified or untracked files.
3. Read `docs/STAGE_E_PLAN.md` Completion Log section — which sub-steps are recorded, column shape, gaps vs `git log` since branch point `57cd2ce`.
4. **E5 sub-step structure (proposed):** **E5.1** — fill Completion Log rows in `docs/STAGE_E_PLAN.md` per E-D9 (paste dates — hashes — subjects from `git log`, no design interpretation); **E5.2** — amend E-D7 `docs/STAGE_E_PLAN.md` L79 `deal()` example per E10 — remove the third `, true` argument; **E5.3** (optional) — revert or neutralize Cursor NatSpec-hyperlink artifact per E-D26 if cosmetic cleanup is worth the churn.
5. **Pre-flight reads:** Completion Log table headers and prior rows in `docs/STAGE_E_PLAN.md`; full Stage E commit list `git log stage-e ^57cd2ce` (or `^main` as needed); `docs/STAGE_D_PLAN.md` Completion Log precedent — mechanical transcription discipline.
6. **Scope guard — E5.1:** Completion Log work is paste-and-summarize from authoritative git output — not protocol reinterpretation; ambiguous rows — stop — escalate to Claude Code.
7. Entry mode per §13: **mostly Sonnet** for E5.1 mechanical Completion Log fill; **Opus high** for E5.2 PLAN wording amendment; **Sonnet** for optional E5.3 NatSpec-hyperlink revert; **Opus high** for E9 stage close — tag, final audit, §11 terminal refresh.
