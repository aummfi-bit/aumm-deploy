...
**Last update:** 2026-04-21, mid-Stage D, post-D3. Stage C remains complete at `stage-c-complete` (commit `5342126`). D0 → D3 landed on `stage-d`; D4 next.

**Branch:** `stage-d` is the working branch, at `2f5cf68` (D3 Completion Log commit on top of D3.4d commit `ed4ec75`). `main` is at `e5ceb7a` (C9 log-completion commit on top of the Stage C tag); unchanged since `stage-d` branched from it at D0. Merge of `stage-d` → `main` is deferred to D9 per D-D14.
...
* D3 — complete (`ed4ec75`, Completion Log `2f5cf68`). `src/fee_router/AureumFeeRoutingHook.sol` (531 lines) + `src/fee_router/IAureumProtocolFeeControllerHookExtension.sol` (62 lines) added; `src/fee_router/IAureumFeeRoutingHook.sol` grew to 199 lines via D3.4c's `ModuleNotSet()` addition. D3.3.3 `a341e44` landed the 373-line initial scaffold (constructor + immutables + caller-gate storage + `BaseHooks` / `IAureumFeeRoutingHook` / `VaultGuard` inheritance + `onAfterSwap` trusted-router early-return per D10 + `_swapFeeAndDeposit` phase-1 body); D3.3.1 `bc920a0` the hook-extension interface; D3.3.4 phase-2 one-sided-add helper folded into D3.4a `3802003` per D21. D3.4 external primitives landed one pair per commit: D3.4a `3802003`, D3.4b `7b91a1a` (`routeYieldFee`), D3.4c `478cbf7` (`routeGovernanceDeposit` + `ModuleNotSet`), D3.4d `ed4ec75` (`routeIncendiaryDeposit` + drop `RoutingNotYetImplemented`). Each inner unlock callback returns `uint256` directly per D22. D3.5 `forge lint src/fee_router/` clean. New NOTES findings: D17 (β1 custody + fast-path-only `swapPool == address(0)`), D18 (authoritative reads on-branch, not in worktree), D20 (Router → direct Vault for the phase-2 one-sided-add helper), D21 (WT-vs-HEAD drift at prompt-drafting time), D22 (`IVault.unlock` inner callback return type). `forge build` green; no tests at D3 (unit tests land at D5). See the D3 row at `docs/STAGE_D_PLAN.md` L748 for the sub-step-by-sub-step breakdown.
* D4 — next. `src/vault/AureumProtocolFeeController.sol` modifications: **B10 retarget** (per **D-D7**) — rename the immutable enforcement target to point at the fee-routing hook address; **OQ-11 band relaxation** (per **D-D8**) — replace any pinned single-value Bodensee swap-fee constant with `BODENSEE_SWAP_FEE_MIN` / `BODENSEE_SWAP_FEE_MAX` / `BODENSEE_SWAP_FEE_GENESIS` band constants; **OQ-2 Bodensee yield-leg guard** (per **D-D9**) — `collectAggregateFees(DER_BODENSEE_POOL)` reverts `BodenseeYieldCollectionDisabled()`. Plan sub-steps D4.1 through D4.5 at `docs/STAGE_D_PLAN.md` L415-481. Existing Stage B / D0.5 tests must still pass after the retarget rename propagates through test code.
* D5 through D9 — not started. See `docs/STAGE_D_PLAN.md` Completion Log for the full sub-step list.

**Open items flagged before D4 execution:** none outstanding. D4's surface is entirely within `src/vault/AureumProtocolFeeController.sol`; D-D7 / D-D8 / D-D9 are settled planning decisions applied at D4.2 / D4.3 / D4.4. D3's unit tests are deferred to D5 per plan — D4.5 re-runs the existing Stage B + D0.5 suite against the retarget rename.

**How to resume (Stage D — D4 implementation):**

1. Read `docs/STAGE_D_PLAN.md` L415-481 (D4.1 through D4.5) in full. Cross-read FINDINGS OQ-2 / OQ-11, NOTES D12 (D0.5 retrofit context for the fee controller), D15 (Cursor-interaction fix-forward for multi-blank files), D18 / D21 (authoritative reads on-branch for §8e.1). D4's Solidity surface is `src/vault/AureumProtocolFeeController.sol` plus the test extension in `test/unit/AureumProtocolFeeController.t.sol`.
...
3. Open a Claude Code session pointed at this repo. Claude Code reads this file, `docs/STAGE_D_PLAN.md`, `docs/STAGE_D_NOTES.md`, `docs/FINDINGS.md` OQ-2 / OQ-11, and the current state of `src/vault/AureumProtocolFeeController.sol`.
4. Claude Code authors the D4.1 sub-step prompt (one baby step) for Cursor, paired with the terminal audit commands.
...
6. Claude Code audits, verdicts ✅ or ❌, and either authors D4.2's prompt or a fix prompt for D4.1. Loop.
