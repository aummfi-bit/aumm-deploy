...
**Last update:** 2026-07-09 — **Stage P IN PROGRESS** on branch `stage-p` (HEAD `ba21bee`, cut from `main` @ f50b4cd = `stage-o-complete`). Two workstreams per P-D0: (A) white-hat back-fill sweep opens the stage; (B) full-system deployment follows. Workstream-A status: P0 (freeze + plan, `docs/STAGE_P_NOTES.md` + `docs/STAGE_P_PLAN.md`) complete; P1 (WH-D pass, `AureumFeeRoutingHook` + `AureumProtocolFeeController`) complete — F-13 (S5 Low) Accepted-risk, deferred to Stage Q; P2 (WH-G pass, `GaugeRegistry`/`GaugeEligibility`/`VaultClassRegistry`/`SwapAndDepositToBodensee`) complete — F-14 (S5 High, svZCHF-membership settlement liveness) **Fixed**, F-15 (S9 Low, `VaultClassRegistry` veto live-denominator) Accepted-risk deferred to Stage Q. **F-16 fully closed** (Medium — the WH-G.2 dormant-tournament observation escalated 2026-07-02 per P-D13—P-D17, F16a through F16h): wires the canonical F-10 efficiency-tournament emission caps Stage G deferred wholesale (G-D15b) — `GaugeRegistry` gains on-chain gauge enumeration (`EnumerableSet _activeGauges`) + a permissionless `advanceTournament()` gated at `year1EndBlock(GENESIS_BLOCK) + 1`; `GaugeEligibility.computeEpochSnapshot` assigns floor-percentile cap tiers (bottom 5%/10-5%/15-10% → 10/50/100 bps, `floor(nRanked × {5,10,15}/100)`) with a post-warmup zero-denominator SKIP (not revert); `IGaugeRegistry.poolEmissionCapBps` delegates through; `EmissionDistributor.recordScore` clamps the reshaped effective score to `capT.divDown(1e18-capT).mulDown(totalScore-oldEffective)` post-month-13 — closing the exploit where a cold-start/flash-manipulable TVL-EMA seed earned an uncapped linear emission share once α=1. Coverage: `GaugeEligibility.t.sol` 21/21 (tier assignment) + `EmissionDistributor.t.sol` 115/115 (clamp) + standalone real-stack end-to-end PoC `test/whitehat/F16_emissionCapTournament.t.sol` 2/2 (real `GaugeRegistry`+`GaugeEligibility` via the `seedFoundingPools` eligibility bypass, no mainnet fork — a `1e30`-seed-spiked fee-less pool's emission share holds ≤ 0.1%). Ledger backfilled (`AUREUM_WHITEHAT_OUTPUT.md` F-16 row + WH-G forward-pointer, `0ce5003`) and `STAGE_P_PLAN.md`'s P2 completion-log line closed (`fef4815`, `38b3631`). Deploy wiring for `advanceTournament` + `poolEmissionCapBps` deferred to P9. **P3 (WH-H pass)** complete (`EmissionDistributor` + `BodenseeBootstrapChannel` + `AuMMMinterRouter` + the Stage-F CCB engine `EMASampler`/`EfficiencyOracle`/`TVLOracle`/`CCBMultiplier`/`CCBScore`/`CCBShare`) — all three read-lens steps **NO FINDING**: WH-H.1 S1 supply/mint conservation (cap-gated sole `_mint` + one-shot minter, exactly 2 `mintFor` sites through the router, the LP-tranche vs Bodensee-bootstrap AP-sum partition byte-identical so `LP(b)+bodensee_share(b)=rate(b)`, halving-era tiling no off-by-one), WH-H.2 S7 TVL-EMA (the F-04/F-05/F-10/F-16 gate lineage re-verified — every emission/governance `tvlEMA` consumer maturity+freshness-gated; the lone ungated reader `CCBMultiplier` is bounded — anti-cyclical, sign-only, clamped [0.75,1.25], multiplied by the gated EMA in its sole consumer), WH-H.3 S3 rounding (every emission-engine mul/div rounds down protocol-favorable; the F-16 clamp rounds down so the cap is never exceeded; no div-before-mul). F-16 (already Fixed at P2) remains the sole pass finding. Adjacent obs → **P6** dead-surface/doc sweep: the `EmissionDistributor` L263 B-leg-anchor param-doc nit (benign — code + channel agree on `m6`) + `CCBShare` orphaned (zero src consumers; MasterChef superseded the F-6 normalizer, an O-D4 `activateBoost` sibling). Ledger roll-up CLOSED (rows `9ef5471`/`e9bb218`/`08616d7`); STAGE_P_PLAN P3 completion-log line `fd31f9f`. Deferred to the P6.5 INV harness: S3 precision-fuzz + INV-5 round-trip; to the Stage-P/Q fork sim: the WK.18 thin-venue `_constellationRatio` pump economics. **P4 (WH-I pass)** complete (the AuMT governance spine — per I-D14 no `AuMT.sol`: `EmissionDistributor` recorder + `AureumFeeRoutingHook` dispatch + `VotingWeight`; S9 attribution + S8 recorder-gate lens, host-contract S1/S2 covered by P3/P1) — **F-17 (S9 High, recorder↔BPT desync) Fixed**. I-D14's transferable BPT + F-09's symmetric `trustedRouter` allowlist (skips `recordWithdrawal` on non-trusted removal) leaves a phantom `userLP > balanceOf` (via non-trusted-router removal or plain ERC-20 BPT transfer) monetizing on four seams — capital-free emission (`claim` mints off stale `userLP`), honest-LP dilution (inflated `poolTotalLP`), skin-less governance (`VotingWeight` reads the recorder), recipient canonical-exit revert (`recordWithdrawal` underflow); latent until P9 `setTrustedRouter`. P-D18 forfeit-delta downward reconciliation: `_syncDown` settles-then-ratchets `userLP`/`poolTotalLP` down to a per-site live-`balanceOf` reference WITHOUT crystallizing (forfeits the moved delta's pending, stranded per H-D24), wired at `recordDeposit` (pre-mint ref) / `recordWithdrawal` (pre-burn ref + `debit=min(amount,userLP)` liveness clamp) / `claim` + permissionless `syncPosition`; `VotingWeight._positionPower` numerator read-cap `min(userLP, balanceOf)` (denominator heals via `syncPosition`). S8 recorder gate ✓ (F-09 `onlyAuMTContract` + allowlist holds fail-closed). Commits `392e036` (core+interface+retrofit) → `a9aafa9` (VotingWeight cap + full-suite repair) → `5f539a4` (PoC flip) → `0598a2f`+`5d0a41d` (fork restore + faithful real-BPT witness); PoC `test/whitehat/F17_recorderBptDesync.t.sol` 3/3 + fork `StageKIntegration::test_F17_realBptHolder_weightTracksLiveBalance`. 885 unit/whitehat + 126 fork green; ledger F-17 row + WH-I roll-up CLOSED. Process lesson (STAGE_P_NOTES P-D18): the F17c `balanceOf` read broke 9 test files (caught only at the F17e full-suite run, not F17d's narrow `EmissionDistributor.t.sol`-only verify), restored via a `balanceOf→1e30` fixture mock — an external call added to a widely-consumed contract needs a full-suite verify. **P5 (WH-J + WH-L) COMPLETE** — two read-lens NO-FINDING passes, coverage-roll-up-only (no findings-table rows, no PoC/fix commits): WH-J (`MiliariumRegistry`, 2 read-lens steps — S8 slot-mutation access + S9 roster-gaming, both clean; the O-D2 composition gate confirmed firing ahead of every `replaceSlot` reachable through governance) and WH-L (`IncendiaryRegistry` + the `EmissionDistributor` L-D25 boost-delivery leg, 3 read-lens steps — S9 economic/S4/S6 purchase-and-placement gaming clean with the F-07 EMA-maturity/freshness gate re-verified holding, S1+cross-seam-(a) conservation confirmed skim-funded not minted via the L-D23 `Σ_pools boostIntegral ≤ integratedSkim` bound, S3/S7/S8 sweep clean); this is the formal WL pass, superseding the P-D6 SPOT-only F-07 review. Commits `4f35e66` (WH-J row) + `dc0103b` (WH-L row). One adjacent observation routed to P6 (not a finding): a boost bought for a then-gauged pool still delivers if the gauge is later revoked (`boostIntegral` not re-gated in `_settlePool`) — non-refundable purchase honored, stays skim-funded/cap-bounded, no conservation break. **P6 COMPLETE** (cross-seam synthesis + INV-2 invariant harness + O-D4/P-D22 `activateBoost` removal, commits `015f280`…`2c0bd31`, ledger row `docs/STAGE_P_PLAN.md` Completion Log) — the four P-D6 named seams: **F-18** (channel↔distributor F-0 bootstrap-rate basis, S1 Low) **Fixed** (`c162261`, P-D19 lock), seams (b) hook↔recorder / (c) governance↔registry / (d) roster↔TVLOracle dedup all clean; `test/invariant/EmissionConservation.t.sol` + `ReferenceEmission.sol` deliver the INV-1/2/6 harness green, INV-2 calibrated to the P-D21 bootstrap-dust bound and deep-gated at `runs≥5000` (640k calls); `CCBMultiplier.activateBoost` + the orphaned gauge-registry apparatus removed in full (Option B / P-D22, constructor 3→2-arg, `getMultiplier` 4→3 regimes). Checkpoint satisfied: build ✓, 871/871 unit ✓, 122/122 fork ✓ (D35/D36 split-form). **P7 hevm symbolic pass DEFERRED to Stage Q** (P-D9, user-adjudicated 2026-07-06 — optional non-gating; bundles with the Q-scoped Act formal-methods). **P9 COMPLETE** (Workstream-B deploy-script authoring + P9.6 lightweight validation; P-D33-revised scope — detail in `docs/STAGE_P_PLAN.md` Completion Log P8/P9 rows) — P9.1 no-periphery-redeploy (P-D27) → P9.2 `DeployFeeRoutingHook` → P9.3 `DeployStageF`+`DeployStageH` split → P9.4 `DeployStageG` (P9.4b the P-D31 three-class authority split, fixing the composition bug where every-setter-=-deployer breaks under a unified downstream governor) → P9.5 `DeployStageP` thin orchestrator (P-D32 lock — testable `deploy()`, the base-layer-governor sentinel, the 12-step chain-order `_orchestrate`, 4 post-conditions) → P9.6 lightweight validation (P-D33 — split-form regression 869 unit/123 fork 0 failed; the S11 `AureumVaultFactory` differential attestation, zero divergence beyond the documented Option-F2 diff; the S10 deployment-surface roll-up, zero new findings). P9 checkpoint (P-D33-revised) SATISFIED: script compiles + regresses clean + S11/S10 attestations clean. **P10.1** (`StagePIntegrationFixture`, `docs/STAGE_P_PLAN.md` completion row `1056ae4`) instantiates `DeployStageP` FIRST, seeds `GOVERNANCE_MULTISIG` to `address(orchestrator)`, deploys ONLY the base layer, then calls `orchestrator.deploy()` — the H13 two-layer pattern, one abstract fixture shared by a wiring-assertion test and the behavioral e2e. **P10.2** (`StagePWiringTest`, same completion row) is the H13 wiring-layer witness — 5 tests, all 15 public handles non-zero, `GENESIS_BLOCK` uniform, all 26 roster pools gauged + recorder-bound, all 14 Stage F/G/H/I/K wire-by-wire handoffs, at-rest module/trustedRouter state — unblocked by the P10.2-fix A-lite fix (`8766329`). **P10.3a Leg A COMPLETE** (`StagePEndToEndTest.test_legA_claimMintsThroughFullRecorderChain`, the recorder-claim-mint chain leg of the P-D36 7-leg roster) — the FIRST execution surfaced **F-19** (S7 Low, `TVLOracle._venueRatio` uninitialized-venue liveness: K-D8 Leg 2 feeds every registry pool unconditionally into a `getPoolData` read gated by the Vault's `withInitializedPool` modifier, so ONE registered-but-uninitialized Miliarium pool bricks `tvl()`/`quoteSvZCHF` protocol-wide — the M/N deploy-uninitialized posture the orchestrator is the FIRST fixture to pair with a full 26-pool registry), now **Fixed** (P-D37 — the `isPoolInitialized` guard as `_venueRatio`'s first statement) with PoC `test/whitehat/F19_uninitializedVenueLiveness.t.sol` (4/4). The second execution attempt then surfaced **P-D38** (a test-tooling lesson, not a protocol finding): `vm.prank(lp); orchestrator.emissionDistributor().claim(...)` silently minted 0 AuMM because the chained public auto-getter is itself a STATICCALL that consumes the single-shot prank, leaving `claim` to run as the test contract (zero stake → the H-D25 `amount==0` early return) — fixed by caching the contract handle into a local before the prank (`EmissionDistributor d = orchestrator.emissionDistributor();` then `vm.prank(lp); d.claim(...)`); `vm.startPrank`/`stopPrank` is immune. The rule applies directly to Legs B–G, which chain `orchestrator.X()` getters heavily. 4-commit train: `2181cbf` (F-19 fix: guard + `MockVaultExplorer` init semantics + PoC) → `42cadda` (ledger F-19 row hash backfill) → `062b522` (Leg A fork witness + the P-D38 handle-cache fix, fork-verified 2/2) → `0144e35` (P-D38 lock in `STAGE_P_NOTES.md`). Full split-form regression green: 873/873 unit, 130/130 fork. **P10.3b COMPLETE** (Legs B + D, commits `0b903f2` + `ba21bee`) — **Leg B** (`test_legB_swapFeeSweepGrowsBodensee`) the real-swap fee-sweep witness (`_performSwap`/`_performSwapCallback` transcribed from `PilotPools.t.sol` L325-L355, two-arg `deal` per E10 + plain `transfer` no SafeERC20; sUSDS→svZCHF on pilot 01 through the D-D21 hook↔controller bake dynamic, proving der Bodensee BPT supply grows + zero hook residue) and **Leg D** (`test_legD_ccbMonthWalkEvolvesMultiplier`) the CCB month-walk over `CCBMultiplier.updateMultiplier`'s `BLOCKS_PER_EPOCH` cadence gate + F-8 anti-cyclical `M_i` step-down (permissionless, no prank; the `TooEarly` cadence-gate revert full-encoded per the `CCBMultiplier.t.sol` L174 arg-carrying-error precedent + the intra-channel `INITIAL_MULTIPLIER` → 0.95 → 0.90 step-down, both clearing the 0.75 floor), each fork-verified 1/1 (full split-form re-run deferred to P10-close). **Next: P10.3c** (Legs C1/C2/C3 — the three `AureumGovernance` proposal types: composition challenge / gauge challenge / fee change, sharing `_matureStack`; the C3 fee-change is the dynamic authorizer-migration witness) → P10.3d (Leg E, halving boundary) → P10-close (split-form regression + `forge coverage` + `slither .`) → P11 (two-commit PENDING_P + tag `stage-p-complete`). P8 testnet-stub workstream stays DEFERRED to P-bis per P-D23.
...
- **Whitehat WK-R pass COMPLETE** (rolling post-Stage-K re-audit, fix-forward on `stage-l`, no re-tag) — surfaced F-05 (Low — `VotingWeight` EMA freshness gate) and F-06 (Medium — post-`endBlock` quorum-denominator inflation), both now **Fixed** with PoC suites (`test/whitehat/F05_emaFreshnessGate.t.sol` 4/4, `test/whitehat/F06_postEndBlockDenominatorFreeze.t.sol` 2/2). The F-06 Governor-snapshot voting fix superseded F-01's `e6d9b3d` live-read denominator. WK-R carries **zero** open findings forward — F-01…F-06 all **Fixed**. Ledger `docs/white_hat/AUREUM_WHITEHAT_OUTPUT.md`.
- **Whitehat WL pass — F-07 Fixed** (scheduled Stage-L spot review, fix-forward on `main`, no re-tag) — surfaced F-07 (Low — `IncendiaryRegistry._maturePrice` enforced the F-04 seed-AGE maturity gate but not sample FRESHNESS; the IncendiaryRegistry sibling of F-05's `VotingWeight` gap), now **Fixed** via `EMA_STALENESS_BLOCKS` + the `EMAStale` gate, with PoC `test/whitehat/F07_emaStalePricing.t.sol` (4/4). WL carries **zero** open findings — F-01…F-07 all **Fixed**. Ledger `docs/white_hat/AUREUM_WHITEHAT_OUTPUT.md`; design-trace `STAGE_L_NOTES.md` L-D29; commits `0a21b99` + `b96227c` + `d3ece7f`.
- **Whitehat WM pass COMPLETE** (scheduled Stage-M-entry spot review — run on `stage-m` after the M0.2 design freeze, before M0.3, fix-forward, no re-tag) — surfaced F-08 (Medium — `VaultClassRegistry.vetoProposal` had no per-voter dedup; one address could re-call to stack its weight across the 10% `VETO_THRESHOLD_BPS` bar), F-09 (Medium — `AureumFeeRoutingHook` liquidity-callback recorder dispatch trusted any router's `getSender()`, so a self-router could spoof LP identity into the emission/qualification clock), and F-10 (Medium — `EmissionDistributor.recordScore` read `tvlEMA` with no maturity/freshness gate, the undamped emission-side sibling of F-05/F-07), all now **Fixed** with PoC suites (`F08_vetoStacking.t.sol` 4/4, `F09_routerSpoofRecorderAttribution.t.sol` 7/7, `F10_emaScoreGate.t.sol` 7/7). Fixes: F-08 `hasVetoed`+`AlreadyVetoed` dedup; F-09 governance `trustedRouter` allowlist (fail-closed) + `StageIIntegration` fixture retrofit; F-10 `_gatedTvlEMA` consumer gate (mirrors F-04/F-05). Each finding's contract is in a tagged-complete prior stage (G/H/I) — I13-class fix-forward on `stage-m`, no Stage-G/H/I re-tag. WM carries **zero** open findings — F-01…F-10 all **Fixed**. Ledger `docs/white_hat/AUREUM_WHITEHAT_OUTPUT.md`; design-trace `docs/STAGE_M_NOTES.md` (Whitehat design-traces); commits `5cc5f3f`+`a67d35d` (F-08), `fc023ee`+`d9eb31f` (F-09), `609335e`+`72d921c` (F-10), logged `e86f63d`.
- **Whitehat WN pass COMPLETE** (scheduled Stage-N spot review — run on `stage-m` over the new `src/rate_provider/` surface, N-D6, fix-forward, no re-tag) — surfaced F-11 (Low — both Aureum-owned RP constructors (`ERC4626RateProvider` + `CompositeRateProvider`) assigned the ERC-4626 vault immutable with no decimals check; `getRate()`'s `previewRedeem(1e18)` is silently wrong-scale for any non-18/18 share/asset pairing that still clears the deploy-time `getRate() > 0` liveness gate — the S4 every-external-4626-read surface), now **Fixed** via a fail-closed constructor guard (`InvalidShareDecimals`/`InvalidAssetDecimals`; both contracts require 18-decimal shares AND an 18-decimal underlying asset), with PoC `test/whitehat/F11_decimalsGuard.t.sol` (6/6). All five current consumers are 18/18; the guard is forward-looking defense-in-depth. WN carries **zero** open findings — F-01…F-11 all **Fixed**. Ledger `docs/white_hat/AUREUM_WHITEHAT_OUTPUT.md`; design-trace `docs/STAGE_N_NOTES.md`; commits `ca088eb` (WN.2a) + `58581b9` (WN.2b) + `5a3a40d` (WN.1) + `addad54` (WN.3), logged `2fbe4bd` (WN.5).

**Branch / tag state:** `stage-o-complete` tagged (at the O7.2-backfill commit) and ff-merged to `main` — no divergence; Stage O ran on a fresh `stage-o` branch off `main` @ ef8cf54 (O-D8). Stage-O history since `stage-n-complete`: O0 (freeze + roadmap) → O1 (`meetsCompositionQualityGate` on `GaugeEligibility`) → O2 (`IGaugeRegistry` delegation, G16 inheritor sweep) → O3 (`AureumGovernance` gate wiring, I13) → O4 / O5 (unit + `StageOIntegration` fork tests) → O6 (stale-doc strikes) → WO whitehat (F-12 Fixed) → O7.2 close + backfill. Tags `stage-a-complete` … `stage-n-complete` + `stage-o-complete` + `stage-k-whitehat-complete` all in place. Whitehat ledger carries zero open findings (F-01…F-12 all Fixed).

- **Stage L COMPLETE** (`stage-l-complete`, ff-merged to `main`) — the Incendiary Boost layer. `IncendiaryRegistry` sells directed emission boosts via `buyBoost` (5 gates + rate-scaled-deposit valuation off a 60-day der-Bodensee price EMA — F-04 maturity gate, 5% anti-gaming haircut), placed FCFS walk-forward into per-(epoch,pool) buckets under the L-D6 15%-of-epoch aggregate cap; the deposit tail donates to der Bodensee (L-D2). `EmissionDistributor` delivers the boost stream per-pool via the L-D25 `poolBoostCursor` leg (I13 fix-forward — Stage G/H/I/K tags untouched), with L-D23 conservation (`boostIntegral` ≤ `integratedSkim`). `script/DeployStageL.s.sol` deploys + dual-gate-wires the registry (L-D28). 824/824 unit + 99/99 fork green. Detail: `docs/STAGE_L_PLAN.md` (Completion log L0.1—L9.3) + `docs/STAGE_L_NOTES.md` (L-D1—L-D28).
- **Stage M COMPLETE** (`stage-m-complete`, ff-merged to `main`) — the Miliarium Sector-2 Majors. Five yield-core pools deployed + bound + fork-tested: ixCasper (03), ixBrevis (08), ixAltrix (09), ixMediox (10), ixLongus (11). Per-pool config libs (`script/pools/configs/`) + `DeployIx*.run()` wrappers (Stage-E framework, E-D21/E-D23); bind-only `script/DeployStageM.s.sol` does `replaceSlot` + `seedFoundingPool` + `setAuMTContractForPool(pool, hook)` per (slot, pool) under three fail-fast governance preconditions (M-D9). M-D7 narrowed the M-D2 9-slot Majors set to five at the M2 preflight (gap pools 02 ixAetheron / 04 ixViatica / 06 ixLibertas / 07 ixCambio → Stage N); M-D5 Standard 26/26/16/16/16 template (ixLongus non-Standard, single 32% theme); M-D10 fork-test scope = binding-liveness (not β-init emission accrual); M-D11 ixCasper waEthwstETH→plain wstETH (RP `getRate()` revert). Detail: `docs/STAGE_M_PLAN.md` (Completion log M0.2—M6) + `docs/STAGE_M_NOTES.md` (M-D1—M-D11).
- **Stage N COMPLETE** (`stage-n-complete`, ff-merged to `main`) — the Miliarium Sector-3 tranche + the two resolvable ex-M Majors. 18 pools (N-D0): 16 Sector-3 Standard slots + 02 ixAetheron (N-D8 native-4626: sfrxETH 28 / wOETH 28 / ixEDEL 16 / rETH 14 / weETH 14, QG 56%) + 06 ixLibertas (scrvUSD / ysyBOLD / sUSDS / GHO / USDC / USDT / PYUSD, QG 57%), deployed via per-pool config libs + `DeployIx*.run()` wrappers (Stage-E framework) and bound by the bind-only `script/DeployStageN.s.sol` (M-D9 shape, three fail-fast governance preconditions, `MILIARIUM_POOL_NN` env scheme). First Aureum-owned `src/rate_provider/` surface lands: `ERC4626RateProvider` (N-D2, faithful re-impl of the Balancer submodule pattern) + `CompositeRateProvider` (N-D9 two-hop, ysyBOLD→yBOLD→BOLD), both with the F-11 fail-closed 18/18-decimals guard (WN). N-D9 swapped the dead-rate sfrxUSD leg to ysyBOLD across 06/20/27 (sfrxUSD mainnet `previewRedeem` == 0 post-Fraxtal). 04 ixViatica / 07 ixCambio remain deferred (token/RP/QG unresolved). 864/864 unit + 119/119 fork green. Detail: `docs/STAGE_N_PLAN.md` (Completion log N0.1—N7) + `docs/STAGE_N_NOTES.md` (N-D0—N-D9).
- **Stage O COMPLETE** (`stage-o-complete`, ff-merged to `main`) — the OQ-7 composition-fitness gate. K shipped the atomic deprecate-and-replace skeleton (K-D6a—K-D6e); Stage O adds the programmatic §xxvii check K-D6e omitted: `GaugeEligibility.meetsCompositionQualityGate(pool)` (≥52% admitted-4626 + canonical hook (O-D2a) + Aureum-factory provenance; reverts `ForbiddenToken` / `WrongFeeRoutingHook` / `PoolTypeNotWhitelisted`; supersedes K-D6e per O-D2 / O-D7, I13-class fix-forward — Stage-K tag untouched), delegated through `IGaugeRegistry`/`GaugeRegistry` (K constructor untouched) and gating `AureumGovernance.proposeCompositionChallenge` (pre-deposit) + `_executeProposal` (pre-mutation) via `CompositionQualityGateFailed`. Stale 90-day-boost docs struck (O-D3); orphaned `CCBMultiplier.activateBoost` flagged for the Stage-P sweep (O-D4); no Aureum Router (O-D5, D33 closed); no pool restoration (O-D6). Whitehat WO: F-12 Fixed (factory-provenance spoof). 872 unit + 124 fork green. Detail: `docs/STAGE_O_PLAN.md` (Completion log O0.1—O7.2) + `docs/STAGE_O_NOTES.md` (O-D0—O-D9 + O-D2a).

**Deferred carry-forward** (live obligations beyond Stage K):
- **D33 — Aureum's own Router** — CLOSED at Stage O (O-D5): no custom Router built. Mainnet Balancer's Router is bound to mainnet Vault (D32), so Aureum needs the canonical Balancer V3 Router redeployed against the Aureum Vault — a Stage-P deployment-wiring concern, not a contract to author. The F-09 `trustedRouter` allowlist already holds the hook-side seat for it.
...
- **OQ-I10 recorder-binding rollout** — pilots 01/05/14 bound through Stage I (post-M-D3 seed); Majors 03/08/09/10/11 bound at Stage M; 16 Sector-3 slots + Majors 02/06 bound at Stage N (M-D9 `setAuMTContractForPool`); remaining Majors 04/07 deferred → Stage O+.
- **Pool restoration carry-forward (M-D7 / M-D11 / N-D8)** — 02 ixAetheron resolved at Stage N (N-D8 native-4626 redesign: sfrxETH/wOETH, dropping the N-D1 stata composites) and 06 ixLibertas resolved (scrvUSD + N-D9 ysyBOLD); the two still-deferred Majors 04 ixViatica / 07 ixCambio await verified mainnet token / RP / QG literals (E-D17 bar) → Stage O+. Still open: ixCasper's waEthwstETH 16% leg restored via an Aureum-deployed composite RP (`0x72D07D × stataEthwstETH.convertToAssets`, mirroring the fWSTETH composite `0x8Be2`) — now buildable on the new `src/rate_provider/CompositeRateProvider`.
- **Whitehat back-fill queue** — WH-D / WH-G / WH-H / WH-J per-stage passes + INV-2 harness + O-D4 orphaned `CCBMultiplier.activateBoost` (zero production callers since the auto-gauge pivot removed the activation caller; `stage-f-complete`-tagged, externally reachable, `isGaugeApproved`-gated — confirm deprecate-and-remove vs re-wire) → Stage P sweep.
- **Multisig-dissolution gap (constitution §xxix ↔ K-D9 retained slice)** — §xxix (10_constitution.md L203) mandates "no admin keys, no multisig" post-migration except the OQ-10 12-month emergency window ("the multisig has no authority thereafter, ever"; the window itself is code-enforced — `AureumGovernanceAuthorizer` strict-`<` immutable time-bomb). But the K-D9 handoff was gauge + Miliarium ONLY, and the retained slice — emission-layer governance (`EmissionDistributor` / `BodenseeBootstrapChannel` / `TVLOracle` / `EfficiencyOracle`), `VaultClassRegistry` governance, `SwapAndDepositToBodensee` donate authority, Miliarium pool `pauseManager`/`swapFeeManager` role accounts — sits on `GOVERNANCE_MULTISIG` indefinitely: K-D9 deferred it "to L/M/N/O," yet L/M/N/O all shipped without migrating it, and no committed script or stage step fires the (re-settable, mechanically ready) `setGovernanceContract` rotations. Not a Stage-P blocker (the P9.5 unified-governor fork model is unaffected; production identity is P-bis). Needs user adjudication: a final multisig-dissolution wiring step (P-bis/Q) vs an §xxix amendment acknowledging a permanent operational slice. Surfaced 2026-07-07 during the P9.5 authority-lifetime review.
...
* **H13 — Deploy-script fork tests cannot stub env vars whose receivers make external calls in their constructors; fixture inheritance is the resolution (H10.2 first-attempt revert + H-D42 chicken-and-egg)** (2026-05-23, `2af6c8d` NOTES H-D42 LOCKED + `c04ee0d` H10.1-fix1 script revision + `2112f09` H10.2-revised test rewrite + `fb2d9b1` H11.0a NOTES H13 entry; surfaced at H10.2 first-attempt revert `call to non-contract address 0x6076…DFFb`): `BodenseeBootstrapChannel`'s constructor calls `_vault.getPoolTokens(BODENSEE_POOL)` to resolve `_aummIndex` (per H-D12); when the deploy-script test stubs `vm.setEnv("VAULT", VAULT_PLACEHOLDER)` with a keccak-derived placeholder, the constructor's external call hits a non-contract address on the mainnet fork and reverts the entire `setUp()`. Root cause: keccak placeholders work for ZeroAddress-validated immutable slots (the receiver only checks `addr != address(0)`); they fail for any constructor that makes an external call to its argument. Default rule: before drafting a fork-test §8e.1 for a multi-contract deploy script, the Opus drafter MUST audit each consumed contract's constructor body for external calls — `grep -A 30 "constructor(" src/<each-deployed-contract>.sol` and look for any `_arg.someFunction(...)` call site. ZeroAddress-only construction = keccak placeholder OK; any external call = real fork state required, resolved by either (a) the deploy-script test inherits a fixture that already provides real state (H10.2-revised: `is StageGIntegrationFixture`), or (b) the constructor argument is reframed as a Stage input rather than a Stage output (H-D42: AuMM reframed from Stage H deployment artifact to Stage H input, mirroring `DeployDerBodensee.s.sol:L39` convention). See **H13** in `docs/STAGE_H_NOTES.md`.

* **I10 — Multi-file rename refactor §8e.1 Must-match must enumerate NatSpec cross-references to the OLD symbol, not just the renamed declaration site (I1.1-fix1 + I1.2-fix1 lessons)** (2026-05-25, `44a94bb` NOTES + I1.1-fix1 `46a93c7` + I1.2-fix1 `1f9c74f`; surfaced across I1.1 + I1.2 audit gaps): When a §8e.1 renames a function / event / error / storage slot — e.g., `setAuMTContract` → `setAuMTContractForPool`, `AuMTContractSet` → `AuMTContractBound` per I-D9 — the original Must-match enumerates new symbols at their declaration sites but misses NatSpec `@notice` / `@dev` paragraphs elsewhere in the same file that cross-reference the OLD symbol by name. Default rule for any rename §8e.1 in the Opus drafting beat: run `grep -n "<old-symbol>" <target-file>` first, enumerate every occurrence, add a Must-match bullet asserting `Zero hits anywhere in the file for the standalone `<old-symbol>\b` token`. Each surviving NatSpec hit must be resolved per-line (update with new symbol + new anchor, or remove the cross-reference). Intra-file NatSpec scope; complements G16's cross-file interface-inheritor enumeration. See **I10** in `docs/STAGE_I_NOTES.md`.

* **I11 — Foundry `forge test --match-test` uses Rust regex; `\|` is literal pipe, not alternation (I2.2 USER VERIFY lesson)** (2026-05-26, `f143b7a` NOTES + I2.3a corrective): `forge test --match-test "<regex>"` evaluates `<regex>` as a Rust regex (the `regex` crate), not POSIX BRE/ERE or PCRE — `\|` is the literal pipe character, not an alternation operator. I2.2 USER VERIFY's `--match-test "qualificationPeriodBlocks\|onRampPeriodBlocks"` matched zero tests (Foundry reported "No tests found in project!") because no test name contains a literal `|`. Default rule for §8e.1 USER VERIFY blocks that filter by test name: use the bare pipe `|` (unescaped) for alternation — e.g., `--match-test "(testA|testB)"`. When the §8e.1 only adds tests to a single test file, prefer `--match-path "test/unit/<File>.t.sol"` alone over `--match-test` (path filter is unambiguous, avoids regex pitfalls); pair with `forge clean &&` if a prior `forge build` reported "No files changed, compilation skipped" per F14 incremental-cache behavior. See **I11** in `docs/STAGE_I_NOTES.md`.

* **I12 — Container-shape design decisions must cross-check prior LOCKs that consume their members (I3.5-pre1/2/3 §12 ambiguity-gate)** (2026-05-27, `d11dbb7` NOTES + I3.5-pre1 `4a595e8` + I3.5-pre2 `feab19d` + I3.5-pre3 `4178fd9` corrective): I-D11 LOCKed a 4-immutable / 6-arg AuMT constructor tuple (`pool`, `distributor`, `liquidityHook`, `GENESIS_BLOCK` + `name_`/`symbol_`) without cross-checking I-D7's `governanceWeight` ZERO branch, which required a `gaugeRegistry` slot for `!gaugeRegistry.isGaugeApproved(pool())`. §12 ambiguity-gate fired at I3.5 pre-flight when the formula scaffold tried to bind a non-existent member; resolved via I-D12 LOCK as 5th immutable per Option A direct-binding (mirrors `EmissionDistributor.sol:L31` precedent), not Option B distributor-chained or Option C interface widening. Default rule for any container-shape LOCK (immutable set, struct, tuple, enum, mapping K/V): before locking, grep all prior D-numbered bodies for member-name references in consumer formulas — the cost of one grep beats a post-LOCK reshape cascade through PLAN + NOTES + source + tests. See **I12** in `docs/STAGE_I_NOTES.md`.

* **I13 — A fail-fast validation gate added to a shared evaluator has a test blast radius; size the retrofit at pre-flight (I8 gauge-eligibility hook-gate)** (2026-05-29, `f2d9da4`…`2d6fc8f`; surfaced at I8.3 pre-flight): I-D13 added a `feeRoutingHook` check as the fail-fast FIRST statement of `GaugeEligibility._checkEligibilityCriteria`, so it intercepted every existing `evaluateEligibility` test — ~15 unit tests against the `makeAddr` mock vault (which mocked `getPoolTokens` but not the new `getHooksConfig`) plus the fork `StageGIntegration` suite (hookless der Bodensee's `ForbiddenToken` test would now revert `WrongFeeRoutingHook` first). The I8.0c roadmap scoped I8.3 as "construction-site updates" only; the blast radius surfaced at I8.3 pre-flight (Opus beat reading the test bodies) before any prompt was drafted, and I8.3 was decomposed into I8.3a (ctor args) / I8.3b (one selector-only `getHooksConfig` setUp mock neutralizing the gate for all unit eligibility tests, Foundry longest-match precedence) / I8.3c (fork ctor arg + Bodensee mock past the gate to preserve T-I3 coverage). Default rule for the Opus beat designing a new validation gate inside an existing shared evaluator (especially a fail-fast first check): before drafting the implementation sub-step, grep every test exercising the gated path plus the existing mock surface and size the retrofit — a fail-fast first check intercepts the whole path, so its revert reason supersedes every downstream check; existing tests asserting a specific downstream revert must be neutralized via a shared setUp mock or re-pointed. Sizing at pre-flight turns a coordinated change into a clean sub-step decomposition; discovering it at regression means reactive per-test patching. Corollary: a new gate added to a tagged-complete stage's contract lands fix-forward on the current branch (Stage G tag untouched), recorded as a Stage-I add-on (OQ-24 / I-D13), not a re-tag. See **I13** in `docs/STAGE_I_NOTES.md`.
