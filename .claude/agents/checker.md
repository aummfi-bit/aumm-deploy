...
4. For a fork run the prompt hands you the RB-023 sentinel `env | grep -cE "^(STUB_|SUSDS=|SV_ZCHF=)"` in the same command as the forge invocation. Run them together in one Bash call, never in two, so the sentinel measures the shell forge will actually run in, and report its number first.
