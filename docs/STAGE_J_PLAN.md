...
- TVLOracle H-D8 roster re-wire. `TVLOracle.sol` documents its constellation roster as `{IMiliariumRegistry roster} ∪ {BODENSEE_POOL} ∪ {governanceAddedPools}` (H-D8) but currently does not enumerate the registry (placeholder-collapsed per F-D9). Does Stage J wire `TVLOracle` to consume the live dense enumeration, or defer? Resolve at J4 pre-flight; does not block J1—J3. Mirror of STAGE_J_NOTES.md Open questions.
