...
Mirror of STAGE_M_NOTES.md Decisions table (LOCKED at M0.2 from a pre-flight Opus beat). Future M-D8 onward attach to sub-step-specific locks during M1—M6 execution and are recorded as new entries in both this table and the NOTES file; M-D7 (below) is the first such — the M2 preflight scope-narrowing.
