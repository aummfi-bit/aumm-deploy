...
> **Status:** K7 design LOCKED (K-D9) on `stage-k` — K0—K6 done + K7 pre-flight closed (`AureumGovernanceAuthorizer` K1, TVL-oracle binding K2 doc-only, `VotingWeight` K3, `AureumGovernance` K4, `AuMMMinterRouter` minter handoff K5, `TVLOracle` roster re-wire K6; full unit + fork coverage green). K-D9 LOCKED — deploy + authorizer migration = two scripts (`DeployAuMM` H-D42 + `DeployStageK`) + a load-bearing wiring chain, handoff set = gauge + registry only (K-D4 reconciled). K7.1 `script/DeployAuMM.s.sol` `d6bc52c`; K7.2 `script/DeployStageK.s.sol` deploy + wire green (`7f8cf17` / `04b58b2`); K7.3.0a K15 fixture design `2274eca`; this commit lands K7.3.0b (K15 PLAN mirror). Next: K7.3a `test/fork/DeployStageK.t.sol` setUp (base `StageIIntegrationFixture` + authorizer bridge + fresh registries). **K-D1—K-D9 LOCKED.** Companion to STAGE_K_NOTES.md.
>
> **Last update:** 2026-06-10 — K7.3.0b PLAN (this commit): K15 PLAN mirror — the K7.3 bullet rewritten from the "exact fixture base fixed at pre-flight" placeholder to the locked design (base = `StageIIntegrationFixture`, single-governor `address(this)` + throwaway-authorizer bridge for wire (8), fresh `MiliariumRegistry` + `VaultClassRegistry`, A/B/C assertions) + K7.3a (setUp) / K7.3b (assertions) decomposition + Completion Log back-fill (K7.2.0b `0feeb9b` / K7.2a `7f8cf17` / K7.2b `04b58b2` / K7.3.0a `2274eca`) + Status/Last-update banner refresh. Next: K7.3a `test/fork/DeployStageK.t.sol` setUp.
>
...
- **K7.3** `test/fork/DeployStageK.t.sol` (NEW, ~150 lines) — the fork integration test materializing the governance handoff. Base = `StageIIntegrationFixture` (NOT `StageKIntegrationFixture`, which pre-hands gauge + registry to `gov` at L61—L62 and would foreclose wires (6)—(7)). Single-governor caller model `governor = address(this)` per K15 / Option 1, with a one-shot throwaway authorizer bridge — `new AureumAuthorizer(address(this))` installed via `vm.prank(GOVERNANCE_MULTISIG); vault.setAuthorizer(...)` — so the script's wire (8) `vault.setAuthorizer(authorizer)` succeeds under `address(this)` (`AureumAuthorizer.canPerform` is `GOVERNANCE_MULTISIG`-strict). Fresh `MiliariumRegistry(address(this), [1, 5, 14], pilots)` (dual-interface, no pre-handoff) + fresh `VaultClassRegistry(svZchf, swapAndDeposit, address(this), address(this), genesisTokens, genesisTypes)` (the inherited VCR is one-shot-sealed at StageG L217, so wire (1) needs an unsealed `votingWeightSetter`). Full design + rejected Option 2 in K15 (`docs/STAGE_K_NOTES.md`). Decomposed K7.3a / K7.3b:
    ...
    - **K7.3b** the A / B / C assertion tests: (A) `MintRouterNotSet`→live — post-wiring `bodenseeChannel.distribute()` / `distributor.claim()` mint succeed (wires (3) / (4) / (5)); (B) gauge + registry handoff — `gaugeRegistry.governanceContract()` / `miliariumRegistry.governanceContract()` `== address(gov)` (wires (6) / (7)) + `vaultClassRegistry.votingWeight() == address(votingWeight)` (wire (1)); (C) OQ-10 migration — `vault.getAuthorizer() == address(authorizer)` (wire (8)) + `AureumGovernanceAuthorizer.canPerform` routing (governance broad / emergency multisig in-window-gated / non-emergency multisig denied); Cursor §8e.1.
...
| K7.3.0b | (this commit) | docs/STAGE_K_PLAN.md — K15 PLAN mirror: K7.3 bullet → fixture base + bridge pattern + K7.3a/b decomposition + Completion Log back-fill (K7.2.0b `0feeb9b` / K7.2a `7f8cf17` / K7.2b `04b58b2` / K7.3.0a `2274eca`) + Status/Last-update banner refresh |
