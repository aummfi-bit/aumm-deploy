...
> **Status:** K7 design LOCKED (K-D9) on `stage-k` — K0—K6 done + K7 pre-flight closed (`AureumGovernanceAuthorizer` K1, TVL-oracle binding K2 doc-only, `VotingWeight` K3, `AureumGovernance` K4, `AuMMMinterRouter` minter handoff K5, `TVLOracle` roster re-wire K6; full unit + fork coverage green). K-D9 LOCKED — deploy + authorizer migration = two scripts (`DeployAuMM` H-D42 + `DeployStageK`) + a load-bearing wiring chain, handoff set = gauge + registry only (K-D4 reconciled). K7.0b PLAN lock `6c2d5cf`; K7.1 `script/DeployAuMM.s.sol` `d6bc52c`; K7.2.0a K14 wiring inventory `ea9f2c0`; this commit lands K7.2.0b (K14 PLAN mirror — K7.2 ladder rewritten to 4 deploys + 8 wiring calls + K7.2a/b decomposition + Completion Log back-fill). Next: K7.2a `script/DeployStageK.s.sol` deploys. **K-D1—K-D9 LOCKED.** Companion to STAGE_K_NOTES.md.
>
> **Last update:** 2026-06-10 — K7.2.0b PLAN (this commit): K14 PLAN mirror — the K7.2 ladder line rewritten from the old five-step sketch to the materialized surface (4 deploys + 8 wiring calls; two distinct Bodensee channels, one dual-interface `MiliariumRegistry`, the `addAuthorizedDonator` wiring step, single-governor caller model) + K7.2a (deploys) / K7.2b (wiring) decomposition + Completion Log back-fill (K7.0b `6c2d5cf` / K7.1 `d6bc52c` / K7.2.0a `ea9f2c0`) + Status/Last-update banner refresh. Next: K7.2a `script/DeployStageK.s.sol` deploys (the `run()` / `deploy(governor)` / `_deploy()` triad).
>
...
| K7.2.0b | (this commit) | docs/STAGE_K_PLAN.md — K14 PLAN mirror: K7.2 ladder → 4 deploys + 8 wiring calls + K7.2a/b decomposition + Completion Log back-fill (K7.0b `6c2d5cf` / K7.1 `d6bc52c` / K7.2.0a `ea9f2c0`) + Status/Last-update banner refresh |
