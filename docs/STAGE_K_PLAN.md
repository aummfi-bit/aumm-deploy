...
> **Status:** K7 design LOCKED (K-D9) on `stage-k` — K0—K6 done + K7 pre-flight closed (`AureumGovernanceAuthorizer` K1, TVL-oracle binding K2 doc-only, `VotingWeight` K3, `AureumGovernance` K4, `AuMMMinterRouter` minter handoff K5, `TVLOracle` roster re-wire K6; full unit + fork coverage green). K-D9 LOCKED — deploy + authorizer migration = two scripts (`DeployAuMM` H-D42 + `DeployStageK`) + a load-bearing wiring chain, handoff set = gauge + registry only (K-D4 reconciled). K7.1 `script/DeployAuMM.s.sol` `d6bc52c`; K7.2 `script/DeployStageK.s.sol` deploy + wire green (`7f8cf17` / `04b58b2`); K7.3.0a K15 fixture design `2274eca`; K7.3.0b K15 PLAN mirror `9958fe9`; K7.3a/b/c `test/fork/DeployStageK.t.sol` setUp + A/B/C assertions green (`2ee9e85` / `3d46d1d` / `fe660c3`; 5/5 fork); K7.3.1a/b K16 NOTES + PLAN mirror (`d0692e0` / `204f3e1`). K-close regression gate measured 6 red — the K13 `MintRouterNotSet` blast radius reaching the two consumer test files K5.4 did not enumerate (`AuMMDistributorIntegration.t.sol` 2-red / `StageHIntegration.t.sol` 4-red; everything Stage K built is green); retrofit = K7.4a/b (the proven K5.4b/c router-wiring pattern). K7.4.0a K17 NOTES `678a81b`; this commit lands K7.4.0b (K17 PLAN mirror). Next: K7.4a/b retrofit → re-run regression → K-close (CLAUDE.md §11 refresh + ff-merge to `main` + `stage-k-complete` tag + origin push). **K-D1—K-D9 LOCKED.** Companion to STAGE_K_NOTES.md.
>
> **Last update:** 2026-06-10 — K7.4.0b PLAN (this commit): K17 PLAN mirror — the K-close regression gate measured 6 red (the K13 `MintRouterNotSet` blast radius reaching `AuMMDistributorIntegration.t.sol` 2-red unit + `StageHIntegration.t.sol` 4-red fork; everything Stage K built green), retrofit decomposed K7.4a/b (the proven K5.4b/c router-wiring pattern) + new K7.4 sub-step ladder + Surfaces table +2 consumer-test rows + Completion Log catch-up (K7.3.1b `204f3e1` back-fill / K7.4.0a `678a81b`) + Status/Last-update banner refresh. Next: K7.4a/b retrofit → re-run regression → K-close (CLAUDE.md §11 refresh + ff-merge to `main` + `stage-k-complete` tag + origin push).
>
...
| `test/unit/AuMMDistributorIntegration.t.sol` | EDIT (K17 retrofit) | K7.4a | +router setUp wiring |
| `test/fork/StageHIntegration.t.sol` | EDIT (K17 retrofit) | K7.4b | +fixture router unification |
...
- **K7.2** `script/DeployStageK.s.sol` (NEW, ~120 lines) — the `DeployStageJ` `run()` / `deploy(governor)` / `_deploy()` triad materializing the governance handoff: **4 deploys + 8 wiring calls** under a single-governor caller model. Two distinct Bodensee channels (`SwapAndDepositToBodensee` proposal-deposit vs `BodenseeBootstrapChannel` emission), one dual-interface `MiliariumRegistry` (`IMiliariumRegistry` + `IMiliariumSlotRegistry`, J-D2). Full constructor + wiring-call signatures live in the K14 inventory (`docs/STAGE_K_NOTES.md`). Decomposed K7.2a / K7.2b per K14:
    - **K7.2a** imports + NatSpec + the `run()` / `deploy(governor)` / `_deploy()` triad — `_deploy()` runs the four `new` calls (`VotingWeight` (genesis = `aumm.GENESIS_BLOCK()`) → `AureumGovernance` → `AureumGovernanceAuthorizer` → `AuMMMinterRouter`) and returns the four handles; compiles, no wiring yet; Cursor §8e.1.
    - **K7.2b** adds `_wire(...)` — the eight calls in load-bearing order: (1) `vaultClassRegistry.setVotingWeight`; (2) `swapAndDeposit.addAuthorizedDonator(gov)`; (3) `bodenseeChannel.setMintRouter` + (4) `distributor.setMintRouter` (before the handoff, per K-D7); (5) `aumm.setMinter(router)` (C-D11 one-shot); (6) `gaugeRegistry.setGovernanceContract(gov)` + (7) `miliariumRegistry.setGovernanceContract(gov)` (the handoff); (8) multisig `vault.setAuthorizer(authorizer)` (OQ-10 migration, LAST) — `deploy()` calls `_wire(...)` after `_deploy()`; Cursor §8e.1.
- **K7.3** `test/fork/DeployStageK.t.sol` (NEW, ~150 lines) — the fork integration test materializing the governance handoff. Base = `StageIIntegrationFixture` (NOT `StageKIntegrationFixture`, which pre-hands gauge + registry to `gov` at L61—L62 and would foreclose wires (6)—(7)). Single-governor caller model `governor = address(this)` per K15 / Option 1, with a one-shot throwaway authorizer bridge — `new AureumAuthorizer(address(this))` installed via `vm.prank(GOVERNANCE_MULTISIG); vault.setAuthorizer(...)` — so the script's wire (8) `vault.setAuthorizer(authorizer)` succeeds under `address(this)` (`AureumAuthorizer.canPerform` is `GOVERNANCE_MULTISIG`-strict). Fresh `MiliariumRegistry(address(this), [1, 5, 14], pilots)` (dual-interface, no pre-handoff) + fresh `VaultClassRegistry(svZchf, swapAndDeposit, address(this), address(this), genesisTokens, genesisTypes)` (the inherited VCR is one-shot-sealed at StageG L217, so wire (1) needs an unsealed `votingWeightSetter`). Full design + rejected Option 2 in K15 (`docs/STAGE_K_NOTES.md`). Decomposed K7.3a / K7.3b / K7.3c:
    - **K7.3a** imports + `DeployStageKForkTest is StageIIntegrationFixture` + `setUp()` — fresh `MiliariumRegistry` + `VaultClassRegistry` + authorizer bridge + 13 deploy-path env vars + `deploy(address(this))` storing the four handles (`votingWeight` / `governance` / `authorizer` / `router`); compiles + `setUp` green under a trivial sanity assertion; Cursor §8e.1.
    - **K7.3b** assertions B + C: (B) gauge + registry handoff — `gaugeRegistry.governanceContract()` / `miliariumRegistry.governanceContract()` `== address(governance)` (wires (6) / (7)) + `freshVCR.votingWeight() == address(votingWeight)` (wire (1)); (C) OQ-10 migration — `vault.getAuthorizer() == address(authorizer)` (wire (8)) + `AureumGovernanceAuthorizer.canPerform` routing (governance broad / emergency multisig in-window-gated / non-emergency + arbitrary-account denied); both `public view`; Cursor §8e.1.
    - **K7.3c** assertion A — `MintRouterNotSet`→live, the faithful both-leg mint-path arc through the router post-wiring (wires (3) / (4) / (5)): (A1) `bootstrapChannel.accrue()` + `distribute()` mints via the router (`totalDistributed() > 0`); (A2) the full F-7 continuous-phase claim arc — gauge-seed pranked as the wire-(6) handed-off `governance`, `vm.store` EMA seed, roll past `AureumTime.year1EndBlock`, `recordScore`, `_depositOneSided` hook deposit, roll, `claim` mints AuMM (`aumm.balanceOf(user) > 0`); faithful both-leg arc per K16 (user decision 2026-06-10); Cursor §8e.1.

### K7.4 — K13 blast-radius retrofit (per K17; K-close regression gate)
The K-close split-form regression (D35/D36 form) measured 6 red — all the K13 `MintRouterNotSet` blast radius reaching the two consumer test files K5.4 did not enumerate (`AuMMDistributorIntegration.t.sol` 2-red unit + `StageHIntegration.t.sol` 4-red fork); retrofit = the proven K5.4b/c router-wiring pattern. Everything Stage K built is green (`DeployStageK` 5/5, `StageKIntegration` 6/6, all K1—K6 unit cohorts) — the 6 red are stale consumers, not K7 regressions. Decomposed K7.4a / K7.4b per K17 (`docs/STAGE_K_NOTES.md`).
- **K7.4.0a** (DONE — `678a81b`) `docs/STAGE_K_NOTES.md` — K17 finding (K13 `MintRouterNotSet` blast radius reaches `AuMMDistributorIntegration.t.sol` 2-red + `StageHIntegration.t.sol` 4-red; K7.4a/b retrofit design); Cursor §8e.1.
- **K7.4.0b** (THIS COMMIT) `docs/STAGE_K_PLAN.md` — K17 PLAN mirror + K7.4 sub-step ladder + Surfaces table +2 consumer-test rows + Completion Log catch-up (K7.3.1b `204f3e1` back-fill / K7.4.0a `678a81b`) + Status/Last-update banner refresh; Cursor §8e.1.
- **K7.4a** `test/unit/AuMMDistributorIntegration.t.sol` (EDIT) — K5.4c retrofit: `AuMMMinterRouter` import + `router` state var + dummy-channel placeholder + `aumm.setMinter(address(router))` + `vm.prank(GOV); distributor.setMintRouter(address(router))` in `setUp` (replacing the L61 `aumm.setMinter(address(distributor))`); the 2 claim tests green; `test_SetMinterHandoff` / `test_RevertWhen_SetMinterCalledTwice` untouched (target-agnostic AuMM `setMinter` mechanics); Cursor §8e.1.
- **K7.4b** `test/fork/StageHIntegration.t.sol` (EDIT) — fixture-level router unification: ONE `AuMMMinterRouter(IAuMM(address(aumm)), address(bootstrapChannel), address(emissionDistributor))` in `StageHIntegrationFixture.setUp` + `aumm.setMinter(address(minterRouter))` + both `setMintRouter` calls while channel/distributor governance is still `address(this)`, then DELETE the four derived `aumm.setMinter(...)` lines (the consumed one-shot would revert `NotMinterAdmin`); `StageHBootstrapPhaseTest`'s `setGovernanceContract(GOVERNANCE_MULTISIG)` handoff stays after the fixture wiring (K-D7 wire-before-handoff order); the 4 phase tests green; I10 NatSpec sweep of stale deferred-`setMinter` lines; Cursor §8e.1.
...
| K7.0b | `6c2d5cf` | docs/STAGE_K_PLAN.md — K-D9 mirror flip to LOCKED + K7 sub-step ladder + Completion Log catch-up (K6-log `f5b85ee` back-fill / K6-§11 / K7.0a) + Decisions preamble + Status/Last-update banner refresh |
| K7.1 | `d6bc52c` | script/DeployAuMM.s.sol — H-D42 micro-script: `new AuMM(genesisBlock, minterAdmin)` production-chain prerequisite before DeployStageH (storage-only ctor, H13-safe) |
| K7.2.0a | `ea9f2c0` | docs/STAGE_K_NOTES.md — K14 DeployStageK wiring inventory (4 deploys + 8 wiring calls, K7.2a/b decomp); commit subject `K7.2-pre-a` |
| K7.2.0b | `0feeb9b` | docs/STAGE_K_PLAN.md — K14 PLAN mirror: K7.2 ladder → 4 deploys + 8 wiring calls + K7.2a/b decomposition + Completion Log back-fill (K7.0b `6c2d5cf` / K7.1 `d6bc52c` / K7.2.0a `ea9f2c0`) + Status/Last-update banner refresh |
| K7.2a | `7f8cf17` | script/DeployStageK.s.sol — deploy triad (`run()` / `deploy(governor)` / `_deploy()`; 4 `new` calls VotingWeight → AureumGovernance → AureumGovernanceAuthorizer → AuMMMinterRouter; no wiring, H13-safe) |
| K7.2b | `04b58b2` | script/DeployStageK.s.sol — `_wire` (8-call wiring chain in load-bearing order + deploy-then-wire in `run()` / `deploy()`) |
| K7.3.0a | `2274eca` | docs/STAGE_K_NOTES.md — K15 finding (DeployStageK.t.sol fixture design, Option 1 LOCKED: address(this) governor + throwaway-authorizer bridge for wire 8, StageI base, fresh MiliariumRegistry + VaultClassRegistry, A/B/C assertions, K7.3a/b decomp) |
| K7.3.0b | `9958fe9` | docs/STAGE_K_PLAN.md — K15 PLAN mirror: K7.3 bullet → fixture base + bridge pattern + K7.3a/b decomposition + Completion Log back-fill (K7.2.0b `0feeb9b` / K7.2a `7f8cf17` / K7.2b `04b58b2` / K7.3.0a `2274eca`) + Status/Last-update banner refresh |
| K7.3a | `2ee9e85` | test/fork/DeployStageK.t.sol — setUp: `DeployStageKForkTest is StageIIntegrationFixture` + throwaway-authorizer bridge + fresh `MiliariumRegistry` + `VaultClassRegistry` + 13 deploy-path env vars + `deploy(address(this))` storing the four handles + sanity assertion (1/1 green) |
| K7.3b | `3d46d1d` | test/fork/DeployStageK.t.sol — assertion B (gauge + registry governance handoff, wires 1/6/7) + assertion C (OQ-10 authorizer migration + `canPerform` routing, wire 8); both `public view` (3/3 green) |
| K7.3c | `fe660c3` | test/fork/DeployStageK.t.sol — assertion A (A1 `distribute()` light-mint + A2 full F-7 claim arc; faithful both-leg router mint-path post-wiring, wires 3/4/5) (5/5 green) |
| K7.3.1a | `d0692e0` | docs/STAGE_K_NOTES.md — K16 finding (3-way K7.3a/b/c decomposition + faithful-arc user decision 2026-06-10 + 5/5 green) |
| K7.3.1b | `204f3e1` | docs/STAGE_K_PLAN.md — K16 PLAN mirror: K7.3 bullet → 3-way K7.3a/b/c decomposition (K7.3c assertion-A bullet added) + K7.3.0b hash back-fill (`9958fe9`) + Completion Log catch-up (K7.3a `2ee9e85` / K7.3b `3d46d1d` / K7.3c `fe660c3` / K7.3.1a `d0692e0`) + Status/Last-update banner refresh |
| K7.4.0a | `678a81b` | docs/STAGE_K_NOTES.md — K17 finding (K-close regression gate: K13 MintRouterNotSet blast radius reaches AuMMDistributorIntegration 2-red + StageHIntegration 4-red; K7.4a/b retrofit design) |
| K7.4.0b | (this commit) | docs/STAGE_K_PLAN.md — K17 PLAN mirror: K7.4 sub-step ladder + Surfaces table +2 consumer-test rows + Completion Log catch-up (K7.3.1b `204f3e1` back-fill / K7.4.0a `678a81b`) + Status/Last-update banner refresh |
