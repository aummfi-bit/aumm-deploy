...
- **Gauge registry integration:** when `MiliariumRegistry.replaceSlot` fires, the gauge registry must (a) revoke the old pool's gauge and (b) auto-approve the new pool's gauge — no automatic boost (Stage O **O-D3**). Both happen atomically in the same transaction as the registry update.
...
- **`08_bootstrap.md` §xxiv Composition Challenge worked example:** add a step-by-step walk-through using the cbBTC delisted → tBTC replacement scenario to illustrate. Show the candidate-pool deploy, the 2/3 supermajority vote, the registry update, the auto-gauge-approval (no boost — optional Incendiary Boost per §xxi, Stage O **O-D3**), and the deprecated pool's continued existence (LPs withdraw or stay; hook keeps routing fees; no AuMM emissions).
...
**Gauge activation (non-Miliarium):** **not a vote** — permissionless `activateGauge(pool)` when `GaugeEligibility` passes + anti-spam fee (OQ-G3). **Composition replacement:** governance execution calls a **governance-only** registry entry to register the replacement pool as gauged — no automatic boost (Stage O **O-D3**; cold-start support is the optional Incendiary Boost, per §xxi); that is executional, not a separate proposal class.
...
> **Status (2026-04-26):** Two values **superseded** by E-D22 in `docs/STAGE_E_NOTES.md` (canonical record + full reasoning). (1) **Der Bodensee** — band dropped, fee re-pinned at `0.0075e18` (0.75%) **immutable** from block 0 (`swapFeeManager: address(0)` at deployment, no governance lever); reverts the 0.10% – 1.00% governance-adjustable stance and re-aligns with the constitutional "deep-friction reserve" framing in `10_constitution.md` §xxix. (2) **Miliarium genesis** — `0.0003e18` (0.03%) → `0.0002e18` (0.02%); the 0.01% – 0.30% band is unchanged. On-chain enforcement lands at E1.3a-quater (`src/vault/AureumProtocolFeeController.sol` constants — `BODENSEE_SWAP_FEE_MIN/MAX` removed; `BODENSEE_SWAP_FEE_GENESIS` renamed to `BODENSEE_SWAP_FEE` with immutability NatSpec; `MILIARIUM_SWAP_FEE_MIN/MAX/GENESIS` added) and E1.3a-sexies (`script/DeployDerBodensee.s.sol` — `swapFeeManager: governanceMultisig` → `address(0)`). Unchanged from 2026-04-15: Miliarium and non-Miliarium gauged pools' 0.01% – 0.30% band; `FEE_CHANGE_COOLDOWN_BLOCKS = BLOCKS_PER_EPOCH = 100,800`; non-Miliarium fee-change governance mechanics. The original 2026-04-15 decision body, parameter table, and spec-edits-required block are retained verbatim below for audit reference — values in the parameter table at lines 820–830 reflect the original genesis-0.03% / Bodensee-band figures and are superseded by the values above.
...
**Superseded (Stage P, 2026-07-01):** the *strategy* here — hybrid public-testnet stubs + mainnet fork — stands (Stage P P-D2 tiered stubs + P-D0b mainnet-fork e2e). Only the Holesky *name* is retired; see the OQ-16 supersession note below and `STAGE_P_NOTES.md` P-D1.
...
**Superseded (Stage P, 2026-07-01):** Holesky was sunset September 2025 — exactly the deprecation this OQ anticipated. The testnet posture is now **network-agnostic**: deploy scripts read RPC + chain-id from env, the primary Stage-P validation is the mainnet fork (P-D0b), and the live public-testnet target — Sepolia if the frontend / validation window closes before its ~Sep 2026 EOL, else Hoodi — is a P-bis decision. See `STAGE_P_NOTES.md` P-D1 (amended 2026-07-01). This note also covers the residual Holesky mentions in OQ-15, OQ-18, and the testnet-availability discussion earlier in this file: all are historical; none names a currently-committed testnet.
...
**Phase-2 addendum (2026-07-12, Stage P-bis PB2.2 — the step-2 2-hop clause BUILT per PB-D7):** the "(or a 2-hop path through ZCHF or sUSDS)" clause in step 2 above is now live in `TVLOracle` — `_twoHopRatio` composes `h1 = _directRatio(underlying, hop)` with `h2 = _directRatio(hop, SVZCHF)` as `h1 × h2 / 1e18` and returns the arithmetic mean across contributing intermediates from a governance-append-only hop roster (`addHopUnderlying`, `onlyGovernance`, seeded ZCHF + USDS at wiring, inert while empty); direct venues always win — the fallback fires only on direct-miss; no transitive hops (depth exactly 2). H-D9's Phase-1 boundary (direct venues only) is superseded and the CLAUDE.md §11 OQ-22 carry-forward closes. Witnesses: `test/unit/TVLOracle.t.sol` 55/55 (exact hop product, direct-wins precedence, mean-across-intermediates, h2-first cheap gate, no-transitive-hop, empty-roster Phase-1 regression) + `test/fork/StageHIntegration.t.sol` `StageHTwoHopFallbackTest` 12/12 (real USDC priced via the `susds.asset()` USDS hop key with der Bodensee as hop-2 venue, EMASampler propagation, governance gate). Commits: `28c653b` (PB-D7 lock) / `61cab54` (src) / `eacb600` (unit) / `78c4931` (fork). Residual: ixAetheron (02) remains ~0-priced — the F-14 venue wall (no first-hop venue exists for its underlyings) is a roster fact outside OQ-22's mechanism; explicit disposition + product-level remedy paths (Stage-O composition challenge, future governance venue) recorded in PB-D7. Label note: the pre-freeze roster wording "2-hop fee-routing fallback" was drift — the fee-routing 2-hop bridge was dropped at P-D12 (F-14) and stays dropped; the built surface is valuation-side only.
...
> **Status (2026-07-16, Stage P-bis PB2.13 / PB-D18):** Sub-item **(iii.b)'s aggregate universe is superseded** — `delta_global` re-keys from the Miliarium-only sum to the all-Active-gauge aggregate; **(iv.a) decouples onto its own Miliarium-only sum, semantics unchanged**. Sub-item **(v) and every boost-related line below are historical**: the boost path (`activateBoost`, `BOOST_FACTOR`, `GAUGE_BOOST_DURATION_BLOCKS`, the F-8 pause) was removed at P6.6 per P-D22 (2026-07-06), dead since the auto-gauge pivot removed its sole caller. The queued §xxix / §xxi spec edits at the end of this entry are amended by the reopening addendum below — the §xxix aggregate sentence would otherwise pin the superseded universe, and the §xxi boost amendment describes the removed path. (i)/(ii) discretization semantics and all §xxix numerical constants are unchanged. Original body retained verbatim for audit reference; the full reopening record is the addendum at the end of this entry.

> **Status (2026-09-18, Stage P-prime PP4.14 / PP-D57 (iii), (ix) and (x)):** Sub-item **(iv)'s divisor is superseded** — the simple arithmetic mean of option (iv.a) survives, but the Miliarium average now divides the Miliarium-only sum by the number of pools actually walked, `miliariumRegistry.miliariumPoolsCount()`, rather than the literal 28, and `CCBMultiplier.MILIARIUM_POOL_COUNT` is deleted. Under the literal, any live count N below 28 put the mean low by (28 - N)/28, 7.143% at the twenty-six live pools against a 0.1% dead zone, so a pool at the true mean was stepped down every epoch; that is seam-1 root cause D.8, ledgered as F-55. By user ruling the divisor is however many Miliarium pools are deployed. The 28-pool constellation membership is unchanged.
...
**Reopening addendum (2026-07-16, Stage P-bis PB2.13 — PB-D18):** A same-day directive pair reopened this OQ; full adjudication in `STAGE_P_BIS_NOTES.md` PB-D18. **(1) (iii.b) universe superseded.** F-8's spec text ("direction of total protocol TVL EMA", `11_formulas.md` L189) was narrowed by the 2026-04-29 resolution to the 28-pool Miliarium sum; `delta_global` now re-keys to the all-Active-gauge aggregate — `currentGlobalAgg = sum(emaSampler.tvlEMA(gaugeRegistry.gaugeAt(i)) for i < gaugeRegistry.gaugeCount())` over the P-D13 `_activeGauges` EnumerableSet (Revoked pools leave the set; raw ungated sum — newly-gauged TVL is genuine protocol growth and the 0.1% dead zone absorbs noise; the F-D18 zero-sentinel cold start carries over; `lastProtocolAggregateEMA` re-keys, its name finally matching its claim). (iii.b)'s no-additional-EMA-state / computed-at-read-time posture is preserved — only the enumeration universe widens. **(2) (iv.a) decoupled, unchanged.** `miliariumAvgEMA` keeps its own Miliarium-only sum over the `IMiliariumRegistry` roster divided by `MILIARIUM_POOL_COUNT = 28`; `updateMultiplier` walks two rosters per call (Miliarium pools legitimately appear in both). **(3) delta_basket considered and withdrawn.** The first directive proposed a third channel damping Miliarium-vs-non-Miliarium basket drift; the same-day follow-up withdrew it — the damping is inherent in F-5/F-6 one-denominator proportionality (`EmissionDistributor.recordScore` scores Miliarium and non-Miliarium gauges into one `totalScore`; every gauged eligible pool earns proportional to its EMA-60 TVL over the all-gauged total), so no multiplier lever is needed and `03_theoretical_foundation.md` §vii's no-protect-the-constellation-lever stance stands unreversed. F-8 stays two-channel; `STEP_SIZE` / `DEAD_ZONE` / clamp and the (i)/(ii) discretization are untouched. **(4) Queued spec edits amended.** Live spec text needs no edit — F-8 and §vii already state the widened reading. Of the four spec-edit bullets queued above by the 2026-04-29 resolution: the **§xxix amendment stands with one correction** — its aggregate sentence becomes "Protocol-aggregate EMA = sum of the TVL EMAs of all currently-Active gauges (no separate accumulator)", the Miliarium-average sentence unchanged; the **§xxi boost amendment is void** (boost path removed at P6.6 per P-D22); the **F-8 annotation bullet** drops its boost-composition clause and points to the corrected §xxix text; the **`03_theoretical_foundation.md` per-channel prose amendment stands**. The corrected §xxix item joins the PB2.10 user-side queue (the plan bullet gains it at PB2.13i; the g3b-queued "F-8/§vii edits" item drops — nothing in the live spec contradicts PB-D18). **(5) Companion directive.** The same directive pair attached the zero-TVL disposition (a zero-TVL pool must not earn emissions, deltas notwithstanding) — traced and recorded at PB-D18 (vi), evidenced at PB2.13h; no FINDINGS row of its own unless the evidence surfaces a defect. Code lands at PB2.13d–f; formal re-attest (`CCBEngineProofs.sol`, `ccb_multiplier.act`, `formal/RESULTS.md`) at PB2.13g.
