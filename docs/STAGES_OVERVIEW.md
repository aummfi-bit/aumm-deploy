...
- **Multisig-dissolution remainder** (§xxix ↔ K-D9, post-F-20): CLOSED at PB2.6 — PB-D2 adjudicated the wiring step (§xxix unamended); the dissolution-time `DissolutionGovernanceHandoff` rotation script + P-D36-style fork witness are built and fork-green. The re-settable set is five: emission-layer governance (`EmissionDistributor` / `BodenseeBootstrapChannel` / `TVLOracle` / `EfficiencyOracle` `setGovernanceContract`) + `SwapAndDepositToBodensee` donate authority. `VaultClassRegistry` is one-shot-sealed (PB-D11, reroutes to the PB3.4 production bind) and the Miliarium `pauseManager` accounts are inert non-exclusive pointers (PB-D2) — neither is a rotation target.
- **D36 permanent fix / F-D11:** CLOSED at PB2.7 — DESCOPED per PB-D13, the workstream-A Explicitly-descoped-with-rationale exit: the plan's one-file scope (`test/fork/DeployAureumVault.t.sol`) was stale by an order of magnitude against the measured surface (224 `vm.setEnv` sites across 16 fork files, 48 test contracts, six files running multiple sibling contracts — the race lives inside single-file `--match-path` runs too), a real fix re-opens the P10-validated deploy plumbing on the eve of the audit freeze (all 16 `Deploy*.s.sol` read fixed literal env keys inside `deploy`), and the "regression proves parallel-safe" retirement gate is unevidenceable on the Ankr free-tier posture (D35 hang pattern). `--threads 1` is promoted from F-D11 workaround to permanent canonical fork-invocation posture; the D36 branch-(b) config-struct parameterization is recorded as RB-006 (new Test-fixture isolation hardening class), revisit-gated on an RPC-posture upgrade or post-audit cadence. Zero code surface.
...
- **WK.18** thin-venue populated-roster fork sim — CLOSED at PB2.11 (PB-D16: `test/fork/WK18ThinVenuePumpSim.t.sol`, a StageN 21-pool populated-roster fork sim, 6/6 fork-green, closes the R1-blocked economic-exploit could-not-assess with fork-grade evidence; F-04's spot-tvl atomic surface stays Fixed behind the 60-day EMA maturity + freshness gate; three faces — an atomic venue pump leaves `governanceWeight` byte-identical, the cross-venue `_directRatio` mean dilutes 1/N, a sustained pump is bounded fee-negative at +99.6% spot vs +10.69% weight over 21 daily samples); **P-D42** coverage-tooling re-eval — CLOSED at PB2.9 (PB-D15: the confirmatory re-probe reproduces the P-D42 Yul stack-too-deep byte-identically on the byte-identical toolchain, forge `f83bad91` / solc 0.8.26; the execution-evidence waiver re-affirms, re-eval rides to the post-audit / newer-toolchain gate with the hevm/Act bundle).
