...
**Storage-vs-immutable invariant for `gaugeEligibility`.** Unlike `svZCHF` and `swapAndDeposit` (which are constructor-known and SHALL be `immutable`), `gaugeEligibility` is **storage** — it must be set post-deploy because `GaugeEligibility`'s constructor depends on `gaugeRegistry`'s address (mutual dependency loop resolved by deploying `GaugeRegistry` first with a placeholder, then `GaugeEligibility` with the real `gaugeRegistry` address, then a one-shot `setGaugeEligibility(address)` setter on `GaugeRegistry` per the **F-D23** parallel one-shot pattern already used at `VaultClassRegistry.setAuMT` and `GaugeEligibility.setGaugeRegistry`). The address-cast typing pattern applies identically: storage form is `address public gaugeEligibility`; setter signature is `setGaugeEligibility(address)`; call site cast is `IGaugeEligibility(gaugeEligibility)`. The storage-vs-immutable distinction does NOT change the typed-domain decision at this G-D16d entry; both halves (svZCHF + swapAndDeposit immutables; gaugeEligibility storage) follow the same address-cast discipline.
