...
**Effect on G1.4 §8e.1.** The Must match for the G1.4 scaffold lands four `bytes32 internal constant _*_SLOT` declarations (`_EXECUTING_SLOT`, `_PENDING_PAY_TOKEN_SLOT`, `_PENDING_AMOUNT_SLOT`, `_ORIGINAL_CALLER_SLOT`), each derived via `TransientStorageHelpers.calculateSlot("aureum.swapAndDepositToBodensee", "<varName>")`. NO `bool transient _executing;`-style state-variable declarations — those would not compile under solc 0.8.26 per the Toolchain provenance subsection above. The actual `tload()` / `tstore()` calls land at G1.6 with the callback body; G1.4 lands only the slot-constant surface plus the `TransientStorageHelpers` import (and `StorageSlotExtension` if the typed-slot extension wrappers are referenced at G1.4 — TBD at G1.4 §8e.1 drafting against minimal-import discipline).
