...
**SUPERSEDED — F-23 / PB-D68 (vii) (2026-08-08).** The coexistence sentence immediately above is RETIRED. It is factually correct about Balancer V3, since a pool registered with `enableDonation = true` and `disableUnbalancedLiquidity = false` does accept both kinds, but it reads as SANCTIONING Stage D's UNBALANCED fee-routing path as an intended design sitting alongside Stage G's DONATION path — and that sanction is what made F-23 look deliberate to every subsequent reader, including the four stage-passes that reviewed the hook without questioning its kind. The fee path minting redeemable BPT was a defect rather than a coexisting design: `AureumFeeRoutingHook._addLiquidityOneSidedToBodenseeViaVault` now uses `AddLiquidityKind.DONATION` and asserts `bptOut == 0`, so DONATION is the sole governing shape for every path that moves PROTOCOL-OWNED value into der Bodensee. The pool configuration itself is unchanged and this note does not touch it — `enableDonation` stays true and `disableUnbalancedLiquidity` stays false, because third-party LPs still add liquidity normally and must keep being able to. What is retired is only the claim that a protocol-fee path may legitimately use UNBALANCED.
...
## G-D16 — GaugeRegistry design decisions (G3.0 NOTES-only resolution)

Locks three Stage-G-scope `GaugeRegistry` design decisions ahead of G3.1 interface extension at `IGaugeRegistry` and G3.3–G3.5 registry implementation — NOTES-only landing (no source, no test, no PLAN edit at this prompt; PLAN cross-links resolve at Stage G closure G5.x sweep). Each sub-decision repeats the default drafted in `STAGE_G_PLAN.md` G3.0 (`STAGE_G_PLAN.md` L369–L377) and was confirmed at G3.0 Opus entry.

**G-D16a — `IGaugeRegistry` extend-in-place at the existing path.** (1) Extension target SHALL be `src/ccb/IGaugeRegistry.sol` — **not** `src/ccb/interfaces/IGaugeRegistry.sol` (that `interfaces/` subpath appears in PLAN G3.0 L373 but does not exist; the landed interface from Stage F per F-D17 was verified at G3.0 Opus entry via `find src -name "IGaugeRegistry.sol"`). (2) This G-D16a entry is the NOTES source-of-truth correcting that PLAN wording; G3.1 source work targets `src/ccb/IGaugeRegistry.sol`. (3) Stage F's `src/ccb/CCBMultiplier.sol` (per F-D17) imports the same path and calls only `isGaugeApproved(address) returns (bool)`; additive symbols (gauge status enum, status query views, three activation entrypoints, registration / revocation events, custom errors) SHALL NOT alter CCBMultiplier's import path or ABI-facing dependence — Stage F SHALL recompile cleanly and `stage-f-complete` bytecode SHALL remain unchanged. (4) Single canonical interface mirrors the project pattern across prior stages (no V2 fragmentation). (5) A split `IGaugeRegistryV2` in `src/gauge/` was rejected — it would force every Stage H / Stage I consumer to bifurcate imports; the Stage H emission distributor would not avoid importing both forks.

**G-D16b — `seedFoundingPool` parameter shape: scalar `address pool` + batch `address[] calldata pools` companion.** (1) Scalar primitive: `function seedFoundingPool(address pool) external onlyGovernanceContract` — one pool argument, **no** `seedAmount` parameter. (2) Batch convenience: `function seedFoundingPools(address[] calldata pools) external onlyGovernanceContract` SHALL internally iterate the scalar primitive across `pools`; supports a single governance transaction seeding all 28 Miliarium genesis pools (`aummfi-bit/aumm-site`, `miliarium_profiles/`). (3) Inner-bound iteration ceiling on the batch entrypoint deferred to G3.5 implementation (likely ~28 plus headroom); exact bound is **not** a G3.0 blocker. (4) **Rationale.** The Vault already stores each pool's token basket and normalized weights before `seedFoundingPool`; the founding deployment flow registers pools with Vault first, then governance calls `seedFoundingPool(pool)` to mark gauge-active. Any basket / duplicate-weight / `seedAmount` argument would replicate on-chain state. (5) **Supersedes G-D7 `STAGE_G_NOTES.md` L82 / L88 / L92** — those lines carried tentative `seedFoundingPool(pool, seedAmount)` text; G-D7 L92 reserved final shape as a downstream sub-step — **G-D16b** is that resolution. PLAN sweep at Stage G closure (G5.x) SHALL amend G-D7 L82 / L88 / L92 verbatim to this supersession; no further NOTES surgery at G3.0 besides this cross-link sentence.
...
**Re-attempt policy.** No nonce layer, no replay gate in `GaugeRegistry` for retries: a caller who received `GaugeActivationFailed` MAY pay another fee and call `activateGauge` again — second failure retains OQ-G3 non-refundable treatment. Historic failure count MUST be reconstructed off-chain from indexed `GaugeActivationFailed` emissions; contract stores **no** per-pool failure counter.

**Cross-references.** **G-D7** + **G-D13** (three activation paths + no-boost across all paths); **G-D9** (VaultClassRegistry parallel — propose-veto-finalize-revoke is class-layer counterpart to activate / revoke at gauge layer); **G-D11** + **G-D12** + **G-D21** (`SwapAndDepositToBodensee` helper, fee magnitudes, two-entry-point split); **G-D17** (Revoked-terminal invariant — composes with G-D16c step 1); PLAN G3.0 L369–L377 (defaults sourced here); PLAN G3.1+ (interface extension consumes G-D16a path lock); PLAN G3.3 / G3.4 / G3.5 (activation entry-point bodies consume G-D16b shape lock + G-D16c fee-fail policy); FINDINGS **OQ-G3** (anti-spam fee non-refundability — the load-bearing constitutional invariant G-D16c implements).

## G-D16d — GaugeRegistry dependency typing: address-immutable + cast-at-call-site (G3.2-pre-fix1a NOTES-only resolution)

Locks the typed-domain pattern for `GaugeRegistry`'s three injected dependencies — `svZCHF`, `swapAndDeposit`, `gaugeEligibility` — to the **address-immutable + cast-at-call-site** pattern already established at `src/gauge/GaugeEligibility.sol` L26–L38 (per **G2.3+** constructor body). NOTES-only landing at this prompt; PLAN G3.2 / G3.3 prose translation handled by the immediately-following sub-step **G3.2-pre-fix1b**.

**Pattern.** All three injected dependencies are stored as `address` and cast at the call site to the appropriate interface or concrete contract type:

- `address public immutable svZCHF` — anti-spam fee token; cast to `IERC20(svZCHF)` at every call site (`safeTransferFrom`, `safeTransfer`).
- `address public immutable swapAndDeposit` — Bodensee fee router (concrete `SwapAndDepositToBodensee` per **G-D11** + **G-D12**); cast to `SwapAndDepositToBodensee(swapAndDeposit)` at every call site (`swapAndDeposit(IERC20, uint256)` invocation).
- `address public immutable gaugeEligibility` — eligibility evaluator binding; cast to `IGaugeEligibility(gaugeEligibility)` at every call site (`evaluateEligibility(pool)`).

**Rationale.** (1) Project-style consistency with the **G2** `GaugeEligibility` precedent — every injected dependency at L26–L38 of `src/gauge/GaugeEligibility.sol` uses the **address-immutable + cast-at-call-site** pattern (`vaultClassRegistry`, `tvlOracle`, `efficiencyOracle`, `vault`, `_auMM`, `_auMT`); mixing typed-interface immutables into a sibling `src/gauge/` contract would introduce a stylistic split inside one stage. (2) The fictional `ISwapAndDeposit` interface — PLAN G3.2 L389 referenced `ISwapAndDeposit` as if it were shipped at G1, but G1 (`G1.4`–`G1.10`) shipped only the concrete `SwapAndDepositToBodensee.sol`; no helper interface exists, and no other Stage G consumer demands one. (3) No symmetry pressure from another consumer — `GaugeRegistry` is the first and only Stage G consumer of `SwapAndDepositToBodensee`; there is no second contract that already typed the helper, so there is no inter-contract typing contract to maintain.

**Rejected alternatives.** (1) **Typed-interface immutables per original PLAN G3.2 wording** — `IERC20 public immutable svZCHF` + `ISwapAndDeposit public immutable swapAndDeposit` + `IGaugeEligibility public gaugeEligibility`. REJECTED: requires the fictional `ISwapAndDeposit`; introduces stylistic split with G2 precedent; no offsetting benefit. (2) **Create `src/gauge/ISwapAndDeposit.sol` interface file** as a new G3.2-pre source sub-step. REJECTED: adds a file the project has lived without through G1.4–G3.1; locks a sync burden between the interface and the concrete `SwapAndDepositToBodensee` (every helper-side method change becomes a two-file edit); G2 precedent already chose the address-cast pattern for the equivalent decision. (3) **Import the concrete `SwapAndDepositToBodensee` contract type and use it as the immutable type** (e.g. `SwapAndDepositToBodensee public immutable swapAndDeposit`). REJECTED: imports the concrete contract's full transitive surface (vault interfaces, transient-storage helpers, internal state) into a registry that should ride on a narrower compile-time surface; creates heavy inheritance/import baggage in `src/gauge/GaugeRegistry.sol` for no behavioral benefit over an `address` immutable + targeted cast.

**Single-direction circular dependency (G3.3-pre-fix2a Path (A) correction).** All three injected dependencies are constructor-known and SHALL be `immutable` — the runtime cycle between `GaugeRegistry` and `GaugeEligibility` is **one-direction**: only `GaugeRegistry` calls `IGaugeEligibility(gaugeEligibility).evaluateEligibility(pool)` from `activateGauge`; `GaugeEligibility` does not call back into `GaugeRegistry` from any execution path. The reverse-direction read (`GaugeEligibility.computeEpochSnapshot` gating via `onlyGaugeRegistry`) is resolved on the `GaugeEligibility` side per **G-D22** — `GaugeEligibility` deploys first taking `gaugeRegistrySetter_` (the F-D23 one-shot setter authority — `src/gauge/GaugeEligibility.sol` L161 arg #7, NOT the registry binding itself); `GaugeRegistry` deploys second with `gaugeEligibility` as a constructor arg + `address public immutable` slot; the F-D23 setter authority then calls `gaugeEligibility.setGaugeRegistry(gaugeRegistryAddress)` to wire the reverse-direction read. No `setGaugeEligibility(address)` setter exists on `GaugeRegistry` and none is added at G3.3 — symmetric `address public immutable` typing across all three deps (`svZCHF` / `swapAndDeposit` / `gaugeEligibility`). **Amendment note (G3.3-pre-fix2a).** The original G-D16d L490 + L496 wording (landed at G3.2-pre-fix1a, commit `cde3a32`) misread `GaugeEligibility`'s constructor signature as taking `gaugeRegistry` directly; the seventh arg is in fact `gaugeRegistrySetter_` (setter authority only — the F-D23 one-shot pattern) and the eighth is `efficiencyOracle_` (the G-D23 (ii) reshape from G2.5-pre-2). The original "Storage-vs-immutable invariant" carve-out resolved a phantom second branch of the dependency cycle that does not exist; Path (A) unifies all three deps as `address public immutable`, dropping both the storage-vs-immutable split and the fictional `setGaugeEligibility(address)` setter that was never declared in `IGaugeRegistry` (`src/ccb/IGaugeRegistry.sol` exposes eight external functions — `isGaugeApproved`, `gaugeStatus`, `activateGauge`, `registerGaugeFromComposition`, `seedFoundingPool`, `seedFoundingPools`, `revokeGauge`, `setGovernanceContract` — none named `setGaugeEligibility`). PLAN G3.2 typing line + source slot relocation are propagated by **G3.3-pre-fix2b** (PLAN substring patches) and **G3.3-pre-fix2c** (`src/gauge/GaugeRegistry.sol` slot relocation from `// Storage` section to `// Immutables` section + NatSpec rewrite + `immutable` keyword addition).

**PLAN downstream.** This NOTES lock is consumed by **G3.2-pre-fix1b** (the immediately-following sub-step), which performs surgical substring patches against `docs/STAGE_G_PLAN.md` G3.2 L389 (imports + immutables + state lines) and G3.3 L395 + L399 (constructor stores + `activateGauge` body cast spellings). No PLAN edit lands at this NOTES-only G3.2-pre-fix1a sub-step. The G3.2 §8e.1 source-scaffold prompt that follows G3.2-pre-fix1b cites G-D16d as the typed-domain lock for its Must match bullets.

**Cross-references.** **G-D11** + **G-D12** (`SwapAndDepositToBodensee` concrete identity — the helper that this typing decision casts to at the registry call site); **G-D16a** (extend-in-place at `IGaugeRegistry` — the interface consumer this typing decision implements at the registry layer); **G2** / `src/gauge/GaugeEligibility.sol` L26–L38 (typed-domain precedent within `src/gauge/`); PLAN G3.2 L389 + G3.3 L395 + L399 (downstream prose patched at G3.2-pre-fix1b); STAGE_G_PLAN.md G3.2-pre-fix1b sub-step (immediately following).
...
## G-D24 — `GaugeRegistry` abstract-keyword schedule + `MockGaugeRegistry` interface-expansion stub backfill (G3.3-pre-fix3a NOTES-only resolution)

Locks the `abstract` keyword schedule for `src/gauge/GaugeRegistry.sol` across G3.3 → G3.5 and the corresponding stub backfill on `test/fork/mocks/CCBMocks.sol::MockGaugeRegistry` — both required because the G3.1 expansion of `IGaugeRegistry` (commit `8e1e84f`) from one function (`isGaugeApproved` per F-D17) to eight functions (per G-D16a additive surface — `isGaugeApproved`, `gaugeStatus`, `activateGauge`, `registerGaugeFromComposition`, `seedFoundingPool`, `seedFoundingPools`, `revokeGauge`, `setGovernanceContract`) left two latent compile failures that surfaced at the G3.3 USER VERIFY `forge build` — the first build run since G3.1 landed.

**Schedule (a) — `GaugeRegistry` `abstract` through G3.4, concrete at G3.5.**

Sub-step landing schedule for the `abstract` keyword on `contract GaugeRegistry is IGaugeRegistry`:

- **G3.2** (commit `b4e6c5d`, scaffold) added `is IGaugeRegistry` with zero function implementations — contract was abstract-but-not-marked from this commit forward; no `forge build` was run, so the missing-`abstract` Error 3656 stayed latent.
- **G3.3** (this Opus beat, source already saved on disk pending commit) lands constructor + `setGovernanceContract` + `activateGauge` (3 of 8 functions) — contract remains abstract; missing-`abstract` Error 3656 surfaces at G3.3 USER VERIFY `forge build`.
- **G3.3-fix1** (one-line source patch, lands as a separate commit immediately following the G3.3 main commit at landing) adds the `abstract` keyword to the contract declaration: `contract GaugeRegistry is IGaugeRegistry` → `abstract contract GaugeRegistry is IGaugeRegistry`. G3.3-fix1 ALSO adds `external override` to `setGovernanceContract` and `activateGauge` per the `src/gauge/GaugeEligibility.sol` L296 / L304 / L309 / L314 precedent (every interface-implementing function uses `external override` — Solidity 0.8.x requires `override` on functions implementing implicitly-virtual interface methods).
- **G3.4** lands `registerGaugeFromComposition` + `revokeGauge` + `isGaugeApproved` + `gaugeStatus` (4 more functions; 7 of 8 total) with `external override` (or `external view override`) on each — contract remains `abstract` (still 1 function short).
- **G3.5** lands `seedFoundingPool` + `seedFoundingPools` (final 2 of 8 functions) with `external override` on each, AND removes the `abstract` keyword in the same source change — contract becomes concrete at G3.5 close.

**Schedule (b) — `MockGaugeRegistry` stub backfill at G3.3-fix2.**

`test/fork/mocks/CCBMocks.sol::MockGaugeRegistry` (L48–L59) was authored at Stage F per F-D26 with only `isGaugeApproved` implemented — the only function `IGaugeRegistry` declared at Stage F per F-D17. The G3.1 interface expansion silently broke the mock's `is IGaugeRegistry` declaration: the contract no longer satisfies the interface (7 missing implementations) and is therefore abstract-but-not-marked, surfacing as Error 3656 against `MockGaugeRegistry` at the G3.3 USER VERIFY `forge build`.

**G3.3-fix2** (source patch, lands as a separate commit immediately following G3.3-fix1 at landing) adds 7 stub implementations to `MockGaugeRegistry`, each `external override` and each one of `pure`, `view`, or default-state-mutating per the interface declaration: `gaugeStatus(address) external view override returns (GaugeStatus)` returning `GaugeStatus.None` (mock default; tests can extend if needed); `activateGauge(address) external override` empty body (test contracts can compose-mock as needed); `registerGaugeFromComposition(address) external override` empty body; `seedFoundingPool(address) external override` empty body; `seedFoundingPools(address[] calldata) external override` empty body; `revokeGauge(address) external override` empty body; `setGovernanceContract(address) external override` empty body. Stage F's existing test consumers (`MockGaugeRegistry.setApproved` + `isGaugeApproved` driven by `_approved` mapping) are untouched — the 7 stubs are pure interface-completion surface and do not perturb Stage F's 263/263 green baseline.

**Why two separate source commits (fix1 + fix2) instead of one combined "G3.3-build-recovery" commit.** (1) Different risk surfaces — fix1 touches production code (`src/gauge/GaugeRegistry.sol`), fix2 touches test infrastructure (`test/fork/mocks/CCBMocks.sol`); separating preserves clean diffs for the auditor. (2) Different reviewers' mental models — fix1 is a Stage G design-schedule decision (when does the contract become concrete?); fix2 is a Stage F regression backfill (mock catch-up against an expanded interface). (3) Easier revert — if either fix later proves wrong, the other can stay landed.

**Why one combined NOTES + PLAN docs commit (G3.3-pre-fix3a + G3.3-pre-fix3b).** (1) Same logical decision drives both files — the abstract-keyword schedule lock (this G-D24 entry in NOTES) and the schedule's prose translation into G3.3 / G3.4 / G3.5 sub-step rows (PLAN amendment at G3.3-pre-fix3b). (2) Consistency with the G3.3-pre-fix2 family precedent (a = NOTES, b = PLAN, c = source) — but here the PLAN translation is a single coherent schedule edit that does not warrant a stand-alone commit independent of the NOTES lock that drives it. (3) Avoids an extra commit whose only purpose is "PLAN echo of what NOTES already said."

**Latent failure lineage — why both errors surfaced at G3.3.** (1) **G3.1** (commit `8e1e84f`, `IGaugeRegistry` interface extension): expanded the interface from 1 to 8 functions. No `forge build` was run in the G3.1 USER VERIFY block — interface-only files conventionally USER VERIFY with `wc -l` + `shasum` + `cat` + `grep -c` only; build was deferred to the first sub-step that touches an implementing contract. (2) **G3.2** (commit `b4e6c5d`, `GaugeRegistry` scaffold): added `is IGaugeRegistry` with zero implementations. USER VERIFY again did not include `forge build` — scaffold-only sub-step; build was deferred. (3) **G3.3** (this beat): first sub-step to include `forge build` in USER VERIFY (per the constructor + function-body scope warranting a compile gate). Both errors — `GaugeRegistry` abstract-not-marked AND `MockGaugeRegistry` abstract-not-marked — surfaced simultaneously, each with lineage two-to-three sub-steps deep.

**Default rule (extends §6 grep-and-confirm; mechanical addition to the §8e.1 USER VERIFY default).** When a sub-step expands an interface that is consumed by one or more concrete contracts (production OR test), the immediately-following sub-step's USER VERIFY MUST include `forge build` regardless of whether the next sub-step is a scaffold or implementation step. This catches abstract-not-marked Error 3656 + missing-`override` Error 9456 at the next build, not multiple sub-steps later. The cure: enumerate the dependent-contract list at the interface-expansion sub-step's NOTES paragraph (G3.1 should have listed `src/gauge/GaugeRegistry.sol` + `test/fork/mocks/CCBMocks.sol::MockGaugeRegistry` as the two implementing surfaces with a "next §8e.1 USER VERIFY MUST run `forge build`" pin); subsequent §8e.1 prompts inherit the pin until the dependent contracts are concrete.

**Cross-references.** **G-D16a** (`IGaugeRegistry` extend-in-place — the interface expansion that triggered both latent failures); **G-D16d** (typed-domain pattern — establishes that `GaugeRegistry`'s function bodies cast at call site, does not address the abstract-keyword discipline); **G-D17** (terminal Revoked semantics — independent of abstract-keyword discipline but composed in `MockGaugeRegistry` stub bodies through the `revokeGauge` signature); **F-D17** (Stage F consumer lock for `IGaugeRegistry.isGaugeApproved` — single-function surface preceding G3.1 expansion); **F-D26** (Stage F mock fixture origin — `MockGaugeRegistry` author convention this NOTES entry backfills); **G11** + **G14** (`forge build` audit-time corollary precedents — keyword choice divergence is a soft signal; same forcing function applies to abstract / override / data-location keywords); PLAN G3.3 / G3.4 / G3.5 (sub-step rows amended by the immediately-following sub-step G3.3-pre-fix3b to reflect the abstract-keyword schedule + `override` discipline + `MockGaugeRegistry` stub-backfill placement).
...
### G16 — Interface-touching §8e.1 fix scope must enumerate ALL inheritors across src/, test/, script/ before drafting (G3.3-fix2 duplicate-mock discovery)

**Surfaced 2026-05-12 at G3.3-fix2 USER VERIFY audit (post-Cursor save).** The G3.3-fix2 §8e.1 was scoped to `test/fork/mocks/CCBMocks.sol` only — adding `external override` to the existing `isGaugeApproved` getter and seven empty-body stub implementations for the remaining `IGaugeRegistry` interface functions per **G-D24** Schedule (b). Cursor's save landed correctly per the grep audit (all 8 functions present, em-dash count + `external override` count both matching expectations). `forge build` nonetheless continued to fail with `Error (3656): Contract "MockGaugeRegistry" should be marked as abstract` plus seven `Note: Missing implementation` entries. After ruling out the **F14** stale-artifact cache (`forge clean && forge build` did not change the result) and any character-by-character file drift (the saved file matched the §8e.1 Must match), a `grep -rn "is IGaugeRegistry" src/ test/ script/` revealed a **second `MockGaugeRegistry` declaration** at `test/unit/CCBMultiplier.t.sol:40` — separate from `test/fork/mocks/CCBMocks.sol`'s mock, instantiated via `new MockGaugeRegistry()` in the unit-test `setUp` at L100, and untouched since the Stage G interface expansion from 1 to 8 functions at G3.1.

**Why it matters.** The G3.3-fix2 §8e.1 prompt-drafting Opus beat assumed only one `MockGaugeRegistry` existed in the project — the one named in `CCBMocks.sol` (which CLAUDE.md §11 had referenced as the canonical mock at F4.1 / F-D26). The duplicate mock at `test/unit/CCBMultiplier.t.sol` is a Stage F decision to give the unit test its own contract-local mock for isolation; **G-D24** Schedule (b) said "MockGaugeRegistry stub backfill" without enumerating WHERE the mock lives. The build failure surfaced one audit round, one diagnostic round (`grep -rn "is IGaugeRegistry"`), and one follow-up §8e.1 sub-step (`G3.3-fix2-followup`) before resolution — a cost a single pre-scope enumeration grep would have avoided.

**Root cause.** Interface-touching fixes — interface expansion, abstract-keyword schedule, `override` addition, stub backfill, `error` / `event` / `enum` addition that implementations must mirror — have a structural blast radius equal to the set of contracts inheriting that interface. The §8e.1 prompt-drafting Opus beat must enumerate that set BEFORE scoping the fix's `Files:` field — otherwise the fix scope is grounded in memory of "the mock" rather than authoritative grep over the current tree. **§6** grep-and-confirm catches misexecution at the audit; **§12** ambiguity-gate catches misdirection in NOTES wording; this rule catches missing surface area in the §8e.1 scope itself.

**Default rule (extends §6 grep-and-confirm + §12 ambiguity-gate; mechanical addition to the §8e.1 prompt-drafting beat).** Whenever a §8e.1 prompt's intent involves an interface-shaped change — interface expansion, abstract-keyword schedule, `override` addition, stub backfill, `error` / `event` / `enum` addition mirrored by implementations — the prompt-drafting Opus beat must include a **pre-scope enumeration grep** of the form `grep -rn "is I<InterfaceName>" src/ test/ script/` (and `grep -rn "is <ParentContractName>" …` for non-interface base contracts). The grep result either confirms a single inheritor (proceed with the existing `Files:` scope) OR expands the §8e.1's `Files:` field to cover every inheritor. When inheritors include test-fixture duplicates with their own copies, the §8e.1's `Must match` block names each separately so Cursor's audit covers the full set. Optional §8e.1 template extension: add a `Pre-scope grep:` field above `Files:` documenting the enumeration command run during drafting and the hits returned — auditor documentation, not a Cursor-actionable instruction.

**Repair landed at G3.3-post-fix4 (this NOTES amendment).** The duplicate mock fix landed at commit `ee49c59` (`G3.3-fix2: test/fork/mocks/CCBMocks.sol + test/unit/CCBMultiplier.t.sol — MockGaugeRegistry 7-stub backfill per G-D24 Schedule (b)`) — both `MockGaugeRegistry` copies updated together in a single source commit. This NOTES entry records the rule for future interface-shaped §8e.1 fixes. `CLAUDE.md` §11 housekeeping bullet G16 lands as a Sonnet-tier follow-up after this NOTES amendment so the bullet can reference this commit hash.
