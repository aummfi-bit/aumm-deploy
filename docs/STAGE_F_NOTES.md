...
### F15 — F10 extended: a `block.number` read captured *before* a `vm.roll` loop and consumed *after* it is sunk forward across the loop, not just hoisted within one (caught at Stage P-prime PP3.2bb)

**Caught 2026-08-23 at PP3.2bb forge-test (`test/whitehat/P1_D1.t.sol`).** `test_P1_D1_dailySamplingOverTheSameWindowConvergesAwayFromTheSeed` reverted `[FAIL: identical elapsed window to the defect case: 2432000 != 2864000]` on its first run. The test's loop was written correctly per F10 — a loop-carried `blockCounter` local, incremented inside the body and passed to `vm.roll(blockCounter)`, exactly the tracked-counter pattern F10 prescribes — yet the surrounding assertion still read the wrong value. The delta, `2864000 - 2432000 = 432000`, is exactly `60 * AureumTime.BLOCKS_PER_DAY`, the loop's own total advance: the assertion's `seedBlock` had picked up the block number as it stood *after* the loop finished, not as it stood at the line where `seedBlock` was declared, one statement before the loop began.

**Root cause — a second, undefended `block.number` read.** The line `uint256 seedBlock = block.number;` sat immediately before the loop, and `seedBlock` was consumed only afterward, inside the closing `assertEq`. Under `via_ir = true` with 9999 optimizer runs, `block.number` is a pure SSA value with no declared dependency on `vm.roll` (a cheatcode `CALL` the optimizer's effect model does not model as mutating chain state — see F10). With no loop-carried dependency binding this particular read to any point inside the loop, the optimizer was free to sink it forward to its single point of use, past all sixty `vm.roll` calls, landing on their cumulative post-loop value instead of the pre-loop one. This is a distinct failure shape from F10's: F10 froze a loop-*internal* read to one stale value repeated every iteration; here an entirely separate, loop-*external* read moved to the wrong side of the loop altogether.

**Why F10's fix didn't cover it.** F10's tracked-counter pattern makes the loop's own `vm.roll` argument un-hoistable by giving it a value the optimizer can prove changes every iteration. That protects only reads consumed *inside* the loop body. `seedBlock` was declared outside the loop and read outside the loop — the counter pattern has no jurisdiction over it, because there is no loop-carried dependency to appeal to for a variable the loop never touches syntactically, only environmentally.

**Fix.** Replace the local `block.number` snapshot with a live read of the same quantity from contract state — the corrected line reads `uint256 seedBlock = sampler.emaSeedBlock(pool);`, an external `STATICCALL` on the sampler's own public `emaSeedBlock` getter that the optimizer cannot elide or reorder past the intervening `updateEMA` calls.

Confirmed as the true cause rather than a coincidental correlation: the defect-case test's gas (`299984`) was byte-identical before and after the fix, since it never touched the miswired line, while only the control case's gas moved (`402707` to `404191`) — the fix reached exactly the one assertion it was meant to reach and nothing else.

**Recurrence — the non-loop form, and why a state read is not always available (PP3.2bu, 2026-08-23).** The same class landed again the same day at `test/whitehat/P1_B2.t.sol`, this time with no loop at all: `uint256 pokeBlock = block.number;` followed one statement later by `vm.roll(pokeBlock + 1);` and then by `vw.getPastTotalSupply(pokeBlock)`. The `vm.roll` argument encoded correctly, because it was evaluated before the boundary; the second read of that same local encoded the post-roll value and reverted `FutureLookup(2296001)` against a guard that was itself correct. A single `vm.roll` is therefore boundary enough — the loop named in this entry's own title is incidental to the mechanism, not part of it, and the title is under-descriptive as a result. The fix prescribed above could not be applied here: the contract state that records the quantity, `effectiveQualBlock`, is zeroed by the very withdrawal under test, so reading it back would have returned the post-condition instead of the snapshot. The fix used instead was a compile-time constant, `MATURED_BLOCK = GENESIS_BLOCK + AureumTime.ON_RAMP_PERIOD_BLOCKS`, rolled *to* rather than derived *from* a live read — the pattern the sibling PoC `test/whitehat/P1_B1.t.sol` had already reached independently one rung earlier through its `START_BLOCK`. This entry already carried the governing rule when the recurrence happened, but had no line in the `CLAUDE.md` §11 lesson index, so it was diagnosed from first principles across a full fix cycle rather than by being looked up.

**Generalization for future stages.** F10's rule — thread an explicit loop-carried counter — defends a `block.number` (or `block.timestamp`) read that is itself inside the loop. It does not defend a read taken *before* a `vm.roll`/`vm.warp` loop and consumed *after* it: that value has no loop-carried dependency at all and is exactly as free to move as any other pure computation. The general rule this extends F10 to: never snapshot `block.number` or `block.timestamp` into a local across *any* `vm.roll`/`vm.warp` boundary, loop or not, unless the local is consumed on the same side of that boundary where it was taken. When a test needs the "before" value for comparison after such a boundary, there are exactly two safe sources, and a cached copy of the environment is neither: read it from the same place production code would — a contract's own recorded state, a public storage getter or an emitted event — or, where the block is known at authoring time, pin it to a compile-time `constant` and roll *to* that constant rather than *from* a live read. Prefer the state read where the state survives the operation under test; the constant is required where it does not.

**Cross-references:**

- **F10** (`STAGE_F_NOTES.md` L422) — the loop-internal sibling; same optimizer mechanism, opposite-side manifestation.
- **`test/whitehat/P1_D1.t.sol` L98** (`test_P1_D1_dailySamplingOverTheSameWindowConvergesAwayFromTheSeed`) — affected test; fixed by reading `sampler.emaSeedBlock(pool)` in place of `block.number`.
- **PP3.2bb** (commit `33e1349`) — the sub-step where the defect surfaced and was corrected before commit.
- **`test/whitehat/P1_B2.t.sol` L32** (`MATURED_BLOCK`) — the non-loop recurrence; fixed by a compile-time constant because the recording state is cleared by the operation under test.
- **PP3.2bu** (commit `70cd0d0`) — the sub-step where the recurrence surfaced and was corrected before commit.
