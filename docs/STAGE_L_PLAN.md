...
| L-D18 | LOCKED | `_spotRate(payToken)` internal view; identity-match `iA`/`iX` on `getPoolData(BODENSEE_POOL).tokens` (address-sorted); spot = `(bal_X·w_A·1e18).divDown(bal_A·w_X)` on `balancesLiveScaled18` + live `getNormalizedWeights()` → 18-dec X-per-AuMM (stable-per-AuMM, L-D17); reverts `TokenNotInPool`/`ZeroSpotBalance`; feeds L3.2 EMA only. | L3.1 |
