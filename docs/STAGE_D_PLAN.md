...
No hand-rolled `MockVault.sol`. Vault semantics come from Balancer's `VaultMock` (byte-identical to the audited `Vault` + test helpers), reached via `BaseVaultTest` at `@balancer-labs/v3-vault/test/foundry/utils/BaseVaultTest.sol`. Full rationale: **D-D17** / **STAGE_D_NOTES D26**.
...
- `setUp()` — custom 7-step flow; inherits `BaseVaultTest` from `@balancer-labs/v3-vault/test/foundry/utils/BaseVaultTest.sol` for deploy utilities but **replaces `setUp`** rather than extending it (construction order and `onRegister` exclusion force this — see **D-D17** / **STAGE_D_NOTES D26**). (1) Deploy the `VaultMock` stack via `deployVaultMock` (`VaultMock`, `VaultExtensionMock`, `VaultAdminMock`, `PoolFactoryMock`, `RouterMock`, `BasicAuthorizerMock`). (2) Deploy `MockERC20` fixtures for **ZCHF**, **AuMM**, and a non-ZCHF-family fee-token `Y`; deploy `MockERC4626` for **svZCHF** constructed over the ZCHF `MockERC20` so `IERC4626(svZchf_).asset()` returns ZCHF at hook construction. (3) Create the **der-Bodensee** pool (`PoolMock` over `[svZCHF, AuMM]`); register via `factoryMock.registerTestPool(pool, tokens, poolHooksContract = address(0), ...)` — Bodensee must register *without* the hook because `onRegister` L251–262 rejects it; initialize liquidity via the router. (4) Deploy `MockFeeController`; pre-fund with fee-token balances sufficient for test scenarios. (5) Construct `AureumFeeRoutingHook(vault, derBodensee, svZchf, aumm, feeController, moduleAdmin)` — Bodensee address now exists for the `DER_BODENSEE` immutable. (6) Create the **trading pools**: one hooked pool `[ZCHF, X]` for the svZCHF-no-op and ZCHF-ERC-4626-deposit branches (A/B), and a second hooked pool `[Y, SV_ZCHF]` for the nested-swap branch (C) since the hook passes `params.pool` as `swapPool` into `_swapFeeAndDeposit`; register each via `factoryMock.registerTestPool(pool, tokens, poolHooksContract = address(hook), ...)`; initialize liquidity. (7) Seed actor balances and router approvals for the scenario tests. No hand-rolled MockVault or MockRouter; routers come from the Balancer harness and Vault is byte-identical to audited production via `VaultMock`.
- Named tests:
  ...
  - `test_SetGovernanceModule_OneShot` — can be called once by `moduleAdmin`; reverts on second call; reverts from non-admin.
