...
| **D-D22** | **Final test list (6 tests); trading-pool composition AuMM/svZCHF 50/50.** Pinned at D7 entry (Gate 2 pin 4 per CLAUDE.md §11 kickoff). Five provisional §D7.1 tests confirmed unchanged (`test_Fork_SwapRoutesFeeToBodensee`, `test_Fork_BodenseeYieldCollectionReverts`, `test_Fork_RecursionGuard`, `test_Fork_RouteYieldFeePrimitive`, `test_Fork_WithdrawProtocolFeesRecipientCheck`). Sixth test added: **`test_Fork_WPFBoundToAureumVault`** — single assertion `assertEq(address(wpf.getVault()), address(vault))`, the D30 binding invariant that the WPF is bound to AureumVault, not mainnet BalancerVault. Rationale: the setUp pre-compute chain asserts `address(wpf) == wpfAddr` (nonce-prediction check only) but does not verify the `IVault` binding baked at WPF construction; `test_Fork_WPFBoundToAureumVault` closes that gap with a single call. Rejected broader WPF-specific tests (factory-role, version-string coverage) — those belong to a standalone WPF test suite post-Stage D. **Trading-pool composition confirmed: AuMM/svZCHF 50/50 weighted, genesis 0.75% swap fee, no Rate Providers on either token.** No Rate Providers rationale: the trading pool exists solely to trigger `onAfterSwap` fee routing; rate accuracy matters for Bodensee (Rate Providers on svZCHF + sUSDS per D11) but not for the hook's fee-input token — the hook receives whatever token the Vault collects the protocol fee in and routes it regardless of rate. `TRADING_POOL_SALT` is a test-internal `bytes32` constant, not a deploy artifact, not in `.env.example`. |
| **D-D23** | **`.env.example` sweep lands as D7.1z terminal housekeeping.** Pinned at D7 entry (Gate 2 pin 5 per CLAUDE.md §11 kickoff). Deferred alternative rejected: D4.5's autonomous scope expansion already added `FEE_ROUTING_HOOK` to `.env.example`, leaving the file partially documented; completing the sweep closes that gap rather than leaving a misleading partial manifest. Six missing Stage D vars added under a new `# Stage D constants` section: `AUREUM_VAULT` (output of `DeployAureumVault.s.sol`; input to downstream Stage D scripts), `WEIGHTED_POOL_FACTORY` (output of `DeployAureumWeightedPoolFactory.s.sol`; input to `DeployDerBodensee.s.sol`), `AUMM` (AuMM ERC-20 address; `AUMM` immutable in `AureumFeeRoutingHook`), `SV_ZCHF` (svZCHF ERC-4626 address; hook fee-routing target + Bodensee Rate Provider token per D11), `SUSDS` (sUSDS ERC-4626 address; Bodensee Rate Provider token per D11), `BODENSEE_SALT` (bytes32 salt passed to `wpf.create(...)` — distinct from the Vault-deploy `SALT`). `TRADING_POOL_SALT` stays out — test-only constant, not a mainnet deploy artifact. Fork tests pass inter-script addresses via `vm.setEnv`; `.env.example` is documentation of the deploy surface, not test plumbing. |
...
Bodensee must be initialized before `routeYieldFee` can run (the inner path calls `_vault.addLiquidity`, which reverts on an uninitialized pool — see **D32**). Add a private helper `_initializeBodensee()` under a "Bodensee fork init" banner, (β) pattern: `Vault.unlock` → callback that calls `IVault.initialize(pool, to, tokens, exactAmountsIn, minBptAmountOut, userData)` (six parameters; no `wethIsEth`) → per-token `transfer(address(vault), amountIn)` + `settle(token, amountIn)` → callback returns `uint256`. Seed `address(this)` with `INIT_SEED = 1_000e18` of each Bodensee token via `deal(token, address(this), INIT_SEED, true)` before the helper (D-D20-compatible; the constraint is no `setMinter` / `mint()` in `setUp`, not "no AuMM balance"). Test body: call `_initializeBodensee()`; then `deal(address(svZchf), address(controller), amount, true);` prank controller to `safeApprove` the hook; prank controller to `hook.routeYieldFee(tradingPool, svZchf, amount);` assert `YieldFeeRouted` emitted, Bodensee BPT minted to controller, `svZchf.balanceOf(address(hook)) == 0`. Same test includes unprivileged caller: expect `UnauthorizedCaller(attacker)` when not the fee controller.
...
Both the trading pool and Bodensee must be initialized; the mainnet Router is bound to mainnet Balancer's Vault, not `AUREUM_VAULT`, so `IRouter(mainnet_router).initialize(aureumPool, …)` reverts (**D32** Finding (1)). Call `_initializeBodensee()` and `_initializeTradingPool()` — (β) pattern per **D32** and §D7.1e, same shape for both helpers; seed `address(this)` with `INIT_SEED = 1_000e18` per pool token via `deal` before each. Swap — default (β): `Vault.unlock` → callback calling `IVaultMain.swap(SwapParams{…})` directly → per-token `transfer` + `settle` → callback returns `uint256`. Alternative (α): `new Router(IVault(address(vault)), IWETH(weth), IPermit2(permit2), version)` then `router.swapSingleTokenExactIn(tradingPool, svZchf, aumm, …)` with full Permit2 plumbing (`IERC20.approve(PERMIT2, max)` + `IAllowanceTransfer(PERMIT2).approve(token, router, amount, expiration)` per account; requires Permit2 live at the fork block). (β) is the default; (α)/(β) choice is write-time at D7.1g execution. Snapshot `bptSupplyBefore = IERC20(bodenseePool).totalSupply();` before the swap; after, assert `IERC20(bodenseePool).totalSupply() > bptSupplyBefore` and `svZchf.balanceOf(address(hook)) == 0`. **OQ-22 — redefined (per D32).** The mainnet Router address (`0xAE563E3f8219521950555F5962419C8919758Ea2`, task `20250307-v3-router-v2`) is a documentation anchor in `balancer_v3_reference.md`; it is not a valid call target for Aureum pools and is retired as "env vs file constant." Deferred clause: if real svZCHF routing is unavailable at the chosen fork block, relax to "hook internal call did not revert" and defer BPT-supply delta assertion to Stage E.
...
| 2026-04-25 | D4.7 — corrective controller β1 swap-leg forward (D7.2 blocker) | ✅ | D4.7a `a1c358a`, D4.7b `5f4e1db` | **D4.7a** `a1c358a`: `src/vault/AureumProtocolFeeController.sol` — five new surfaces implementing the β1 swap-leg forward missed at D4: `collectSwapAggregateFeesForHook` (entry point gated to `FEE_ROUTING_HOOK` caller via `OnlyFeeRoutingHook` custom error), `collectAggregateFeesHookSwapForward` (public forwarder, `onlyVault` guarded, Bodensee guard re-applied), `_receiveAggregateFeesSwapForward` (internal, iterates pool tokens, settles non-zero amounts directly to `FEE_ROUTING_HOOK` via `_vault.sendTo`), `SwapLegFeeForwarded(address indexed pool, address indexed token, uint256 amount)` event, and `OnlyFeeRoutingHook` error. Root gap: D17 L167–170 prescribed explicit D3/D4 scope split — D3 lands the hook-side `IAureumProtocolFeeControllerHookExtension` interface and call-site; D4 lands the controller-side implementation — but the D4.2–D4.5 execution window missed L170's controller side, surfaced at D7.2 as `unrecognized function selector 0x9992fceb`. **D4.7b** `5f4e1db`: `test/unit/AureumProtocolFeeController.t.sol` + 6 targeted tests: `test_collectAggregateFeesHookSwapForward_nonZeroForward`, `test_collectAggregateFeesHookSwapForward_zeroAmountShortCircuit`, `test_collectAggregateFeesHookSwapForward_emitsSwapLegFeeForwardedPerNonZeroToken`, `test_collectAggregateFeesHookSwapForward_yieldLegUntouched`, plus access-control fuzzes `test_collectAggregateFeesHook_revertsIfNotVault` and `test_collectSwapAggregateFeesForHook_revertsOnNonHookCaller`. `forge test --match-path "test/unit/AureumProtocolFeeController.t.sol" -vv`: 37/37 green (4 invariants × 256 runs × 32768 calls, 0 reverts). D4.7c narrow Slither pass on D4.7a surface pending before D7.2 retry. |
| 2026-04-25 | D7 — fork tests | ✅ | D7.1 `c62f16c`, D7.1z `24d5bfa` | **D7.1** `c62f16c`: `test/fork/AureumFeeRoutingHook.t.sol` (489 lines, sha `058198e0…`, 11 em-dashes) — 6 fork tests with full setUp harness per **D-D19/D-D20/D-D21/D-D22** (Gates 2+3). setUp: `vm.computeCreateAddress` pre-compute chain (D-D21 option a); inline WPF for `msg.sender` symmetry; real `AuMM` funded via `deal` (D-D20); (β) `Vault.unlock` pool-init pattern per **D32** (no Router, no Permit2). Tests: `test_Fork_WPFBoundToAureumVault` (D30 WPF/AureumVault binding), `test_Fork_BodenseeYieldCollectionReverts` (OQ-2 / D-D9), `test_Fork_WithdrawProtocolFeesRecipientCheck` (B10 `InvalidRecipient`), `test_Fork_RouteYieldFeePrimitive` (`_initializeBodensee` helper + svZCHF yield-leg forward through controller), `test_Fork_RecursionGuard` (`vm.recordLogs()` + `Vm.Log[]` count proof), `test_Fork_SwapRoutesFeeToBodensee` (`_initializeTradingPool` + `_performSwapCallback`; AuMM/svZCHF trading pool; Bodensee BPT supply delta asserted; zero hook residual). **D7.1z** `24d5bfa`: `.env.example` Stage D constants block (6 vars per **D-D23**). **D7.2** (2026-04-25): `SV_ZCHF = 0xE5F130253fF137f9917C0107659A4c5262abf6b0` (`SavingsVault ZCHF`, asset = ZCHF `0xB58E61C3098d85632Df34EecfB899A1Ed80921cB`) and `SUSDS = 0xa3931d71877C0E7a3148CB7Eb4463524FEc27fbD` (`Savings USDS`, asset = USDS `0xdC035D45d973E3EC169d2276DDab16f1e407384F`) — verified on-chain (name/symbol/`asset()`/decimals/bytecode); populated in local `.env` (untracked). Corrective **D4.7** (`a1c358a`/`5f4e1db`, own Completion Log row) required before fork-test retry — controller β1 `collectSwapAggregateFeesForHook` surface was absent. Post-D4.7 fork run: 6/6 tests pass. Full suite split-form (unit + fork run separately; global `--fork-url` without path filter triggers Ankr rate-limiting under 200+ tests): 151 unit + 9 fork = 160/160 passed, 0 failed, 0 skipped. |
| 2026-04-25 | D8 — Slither triage gate | ✅ | D8.2 `1eb38db`, D8.3 `01e8b6e`, D8.4 `33ad133`, D36 `19d9eda` | **D8.1** chat-only triage of full-stage Slither baseline (`slither . --filter-paths "lib/\|test/" --exclude-informational`, v0.11.4): 19 findings across 5 detectors on the Stage D audit surface (`src/fee_router/AureumFeeRoutingHook.sol` + `src/vault/AureumProtocolFeeController.sol`). Triage outcome: **19/19 inline-suppress, 0 fixes** — every finding reduces to one of four structural patterns: (a) Vault.unlock transient-lock pattern artifacts (`reentrancy-events` × 1, F1; `reentrancy-no-eth` × 7, F4/F9/F10/F13/F16/F17/F18); (b) deliberate balance-sweep design per D3.3.4 Q1 / Option X (`unused-return` × 5, F2/F5/F6/F11/F12); (c) bounded fee-token loop in `onAfterSwap` capped by Balancer V3's per-pool token limit (`calls-loop` × 5, F3/F7/F8/F14/F15); (d) D-D15 `SplitIsImmutable` lock keeping `_globalProtocolSwapFeePercentage` as `uint256 private` for layout symmetry with the governance-mutable yield slot (`immutable-states` × 1, F19). **D8.2** `1eb38db`: `STAGE_D_NOTES.md` ## D8 — Slither triage section (+24 lines, 444 total) — full 19-row table with per-finding (severity, file:line, detector, disposition, rationale) entries plus structural-patterns intro. **D8.3** `01e8b6e`: `src/fee_router/AureumFeeRoutingHook.sol` 13 inline-suppress sites, +26 lines (542 → 568); rationale comments contain "D8 NOTES F<n>" cross-refs for greppability; consolidated multi-detector directives at L329 (F2+F3), L357 (F5+F7), L369 (F6+F8), L407 (F11+F14), L420 (F12+F15). **D8.4** `33ad133`: `src/vault/AureumProtocolFeeController.sol` 1 inline-suppress site (F19, L98 `_globalProtocolSwapFeePercentage`), +2 lines (828 → 830). **D8.5** post-suppression Slither re-run on the same filter set: 0 findings — all 19 suppressions effective on first attempt. **D8.6** `forge build` clean; full split-form test suite 160/160 green via Part A `forge test --no-match-path "test/fork/**" -vv` (151/151 unit, ~14s wall) + Part B `forge test --match-path "test/fork/**" --fork-url $MAINNET_RPC_URL -vv --threads 1` (9/9 fork). The `--threads 1` flag on Part B is the workaround for **D36** `19d9eda` (vm.setEnv race condition between `test/fork/AureumFeeRoutingHook.t.sol` L108 and `test/fork/DeployAureumVault.t.sol` L79: both write the shared `FEE_ROUTING_HOOK` env key, and under default Foundry parallelism `DeployAureumVault.t.sol`'s write occasionally lands between `AureumFeeRoutingHook.t.sol`'s `vm.computeCreateAddress` call and `vaultScript.deploy(...)`'s `vm.envAddress` read at `script/DeployAureumVault.s.sol` L128 — clobbering the controller's `FEE_ROUTING_HOOK` immutable to `0xF86D…3a77`). Permanent fix (env-key namespacing or eliminating `vm.setEnv` from fork tests entirely) deferred to Stage E or a pending-findings stub; not a D8 regression — D8.3/D8.4 did not touch fork test files. |
| 2026-04-25 | D9 — `stage-d-complete` tag + merge to main + cross-doc logs | ✅ | merge `d04a43d`→`951c338`, tag `stage-d-complete` | **D9.0** `git push origin stage-d` — `origin/stage-d` was 39 commits behind local `stage-d` (never pushed during D8); pushed 154 objects, `18f74b9`→`951c338`; `origin/stage-d` confirmed at `951c338`. **D9.1** `git checkout main && git merge --ff-only stage-d` — fast-forward, no merge commit: `d04a43d`→`951c338` (23 files changed, 4476 insertions, 389 deletions, 10 new files). `git tag stage-d-complete && git push origin stage-d-complete && git push origin main` — all five refs (`main`, `origin/main`, `stage-d`, `origin/stage-d`, tag `stage-d-complete`) at `951c338`. **D9.2–D9.4** cross-doc updates committed together as `D9: log stage-d-complete across plan, overview, and CLAUDE.md §11` on `stage-d`: this Completion Log row (D9.2); STAGES_OVERVIEW.md Stage D Completion Log entry (D9.3); CLAUDE.md §11 refreshed to "Stage D complete; Stage E next" — stale main-tip claim `e5ceb7a` corrected to `951c338`, D9 "next" language removed, Stage D position bullet → complete, D36 `--threads 1` workaround documented (D9.4). Branch `stage-d` retained on origin per D-D14. |
