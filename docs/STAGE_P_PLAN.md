...
Granular per-stage deploy scripts for the never-scripted surfaces + the thin `DeployStageP.s.sol` orchestrator — P-D25 (user-adjudicated 2026-07-06): one source of truth per constructor call lives in the granular script; the orchestrator owns only sequence + fail-fast assertions, and the P10 fork fixture inherits it (H13 pattern). Network-parameterised (RPC + chain-id from env); no `--broadcast` (P-D3). The canonical deploy order (stage lettering is NOT the order — M/N binds precede K; `Vault.setAuthorizer` is the final mutation), the `DeployStageH` split rationale, the WH-P6 (b)/(c) new binds, and the assertion set are locked in P-D25.
- **P9.1** — `script/DeployBalancerPeriphery.s.sol`: canonical Balancer V3 `Router` + `VaultExplorer` redeployed against the Aureum Vault (D33 / O-D5; D32 immutable vault binding makes the mainnet instances unreachable). A G10-class compile-probe precedes the script — no `new Router(` / `new VaultExplorer(` exists in the tree.
...
- **P9.5** — `script/DeployStageP.s.sol` thin orchestrator: the `deploy()` delegation chain in P-D25 canonical order + the three new binds (pilot `seedFoundingPool` 01/05/14; `VaultClassRegistry.setGovernanceContract(multisig)`, WH-P6 (c); `setTrustedRouter(router, true)` strictly after the last recorder bind, WH-P6 (b)) + the four fail-fast assertions (GENESIS_BLOCK 4-way equality per P-D14; Miliarium roster completeness; trustedRouter ordering; post-run `vault.getAuthorizer()`).
