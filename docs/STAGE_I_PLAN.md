...
- **I7.2** `test/fork/DeployStageI.t.sol` (inherits `StageGIntegrationFixture`, NOT `StageIIntegrationFixture` which pre-wires all 4 calls → `EmissionRecorderAlreadySet`; runs `DeployStageH.deploy(...)` to reach the post-handoff distributor, then `DeployStageI` under `vm.startPrank(GOVERNANCE_MULTISIG)` per I-D18) — assertions: `hook.emissionRecorder() == distributor`; `distributor.auMTContractByPool(pilotPool) == hook` ×3; `setEmissionRecorder` reverts `EmissionRecorderAlreadySet` on second call; `distributor.governance() == GOVERNANCE_MULTISIG`; no AuMT-token assertions. Mirrors `DeployStageH.t.sol` L61 / L132. Cursor §8e.1.
