...
> **Status:** Code phase open — I9 (AuMT deprecation + IVotingWeight migration) COMPLETE (I9.1—I9.5f) + I4 (recorder dispatch + clock) + I5 (recorder-clock unit tests) + I6 (recorder-clock fork integration) COMPLETE; **I7.1—I7.3 COMPLETE — split-form regression green: 625/625 unit + 76/76 fork**. Per OQ-25 / I-D14, AuMT is the pool's own Balancer V3 BPT, not a separate token; Stage I is the oracle-free **recorder clock + hook dispatch** only. Decisions I-D1—I-D18 LOCKED: I-D14 (AuMT = BPT), I-D15 (value-weighted view deferred to a Stage K `VotingWeight.sol`), I-D16 (hook one-shot `emissionRecorder` slot), I-D17 (`IVotingWeight` forward stub + `VaultClassRegistry` full-rename fix-forward), I-D18 (DeployStageI wiring GOVERNANCE_MULTISIG-only); I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED + I-D5—I-D9 AMENDED by I-D14. I3 (concrete `AuMT.sol`) TOMBSTONED — superseded, removed at I9.4. I1 (I-D9), I2 (I-D10), I8 (I-D13) complete pre-reframe, unchanged. Findings I10—I19. Vault bytecode untouched. Next: I7.4(c) fast-forward merge `stage-i → main` + tag `stage-i-complete` + push. Companion to STAGE_I_NOTES.md.
>
> **Last update:** 2026-06-05 — I7 deploy + close-of-stage open. I7.1 `script/DeployStageI.s.sol` recorder-path wiring (`setEmissionRecorder` ×1 + `setAuMTContractForPool` ×3, GOVERNANCE_MULTISIG-only per I-D18; `run()` / `deploy()` / `_wire()` mirror of DeployStageH + `GovernanceNotMultisig` fail-fast; `fe6f658`) → I7.2 `test/fork/DeployStageI.t.sol` integration (inherits `StageGIntegrationFixture` + `DeployStageH.deploy()` post-handoff distributor; asserts the recorder bind + 3 pilot gates + `governance` + `NotEmissionRecorderAdmin` second-call one-shot lock; **1/1 fork green**; `08f8ee5`). I7.2-pre §12 correction — second-call revert `EmissionRecorderAlreadySet` → `NotEmissionRecorderAdmin` (admin self-zeroed on the first bind, admin gate checked first; `edd5f8d` → Finding I19). I7.3 close-of-family sweep COMPLETE: I7.3a Finding I19 (`6dab465`) → I7.3b Completion Log + Anchors (`119e382`) → I7.3c status/mode (`5106aa0`) → I7.3d CLAUDE §11 (`61b3e6f`). I7.4(a) split-form regression green — 625/625 unit + 76/76 fork. Next: I7.4(c) fast-forward merge + tag `stage-i-complete` + push.
>
> **Mode:** Opus high per §13 stage-level defaults. I7.1 + I7.2 COMPLETE — 1/1 DeployStageI fork green. I7.3 close-of-family sweep COMPLETE; I7.4(a) regression green (625/625 unit + 76/76 fork). I7.4(c) fast-forward merge + tag `stage-i-complete` + push next. The Deferred-from-I4 `setEmissionRecorder` two-flag-lock **unit**-level revert tests stay deferred to a hook-test extension (fork-covered at I6.1 happy-path bind + I7.2 second-call revert — see PLAN L134); not folded into I7. Entry mode Opus high for the I7.4 regression + close-of-stage reckoning, dropping to Sonnet for commit-message housekeeping per §13.
>
...
| I-D18 | LOCKED | `DeployStageI.s.sol` wiring authority = `GOVERNANCE_MULTISIG` broadcast only — calls `setEmissionRecorder` ×1 (I-D16) + `setAuMTContractForPool` ×3 (I-D9 amend) as the multisig, NO deployer→multisig handoff (both gates already admit the multisig post-Stage-H: hook `_emissionRecorderAdmin = moduleAdmin_`; distributor `governance` set at DeployStageH Step 7); postcondition asserts `distributor.governance() == GOVERNANCE_MULTISIG`; I7.2 inherits `StageGIntegrationFixture` + `DeployStageH.deploy(...)`, NOT `StageIIntegrationFixture` (pre-wires → second `setEmissionRecorder` reverts `NotEmissionRecorderAdmin`, admin self-zeroed on the first bind); env pilots slot-named `PILOT_POOL_01/05/14` | I-D16 hook one-shot; I-D9 distributor bind; H-D42 deploy chain; DeployStageH Step 7/8; DeployStageH.t.sol L61; StageGIntegration.t.sol L159/L83 | I7.0a |
...
- [x] (resolves at I1.7) Stage H test cohort 612/612 unit + 10/10 fork + 1/1 deploy fork green after Stage I per-pool refactor.
- [x] (resolves at I4.5) Stage D test cohort green after AureumFeeRoutingHook extension.
- [x] (resolves at I7.3) Full Stage I test cohort green (AuMT unit + StageIIntegration fork + DeployStageI fork).
- [x] (resolves at I7.4) `stage-i` fast-forward merge → `main` + `stage-i-complete` tag + push.
...
- **I5.5** Close-of-family sweep — PLAN Completion Log + status refresh. Note: the value-weighted-view tests (qualification cliff, on-ramp cap, F-9 era transition, gauge-revoked) are deferred out of Stage I with `src/governance/VotingWeight.sol` per I-D15. Cursor §8e.1.
...
- **I7.1** `script/DeployStageI.s.sol` — wire the recorder path as `GOVERNANCE_MULTISIG`, no AuMT deploys, no deployer handoff (I-D18): `hook.setEmissionRecorder(distributor)` ×1 on the canonical `AureumFeeRoutingHook` (I-D16, self-zeroes `_emissionRecorderAdmin`) + `distributor.setAuMTContractForPool(pilotPool, hook)` ×3 (I-D9 amend, `onlyGovernance`); postcondition assert `distributor.governance() == GOVERNANCE_MULTISIG` (fails fast if DeployStageH Step 7 was skipped). Both gates already admit the multisig post-Stage-H (hook `_emissionRecorderAdmin = moduleAdmin_`; distributor `governance` set at DeployStageH Step 7), so one multisig broadcast covers all four calls. Structurally mirrors `DeployStageH.s.sol` (`run()` / `deploy(deployer)` / `_wire(...)`). Env: `FEE_ROUTING_HOOK`, `EMISSION_DISTRIBUTOR`, `GOVERNANCE_MULTISIG`, `PILOT_POOL_01/05/14`. AuMM + hook + distributor are Stage inputs per H-D42, not deploy artifacts. Cursor §8e.1.
- **I7.2** `test/fork/DeployStageI.t.sol` (inherits `StageGIntegrationFixture`, NOT `StageIIntegrationFixture` which pre-wires all 4 calls → second `setEmissionRecorder` reverts `NotEmissionRecorderAdmin` (admin self-zeroed); runs `DeployStageH.deploy(...)` to reach the post-handoff distributor, then `DeployStageI` under `vm.startPrank(GOVERNANCE_MULTISIG)` per I-D18) — assertions: `hook.emissionRecorder() == distributor`; `distributor.auMTContractByPool(pilotPool) == hook` ×3; `setEmissionRecorder` reverts `NotEmissionRecorderAdmin` on second call (admin self-zeroed on the first bind, checked before the `EmissionRecorderAlreadySet` flag); `distributor.governance() == GOVERNANCE_MULTISIG`; no AuMT-token assertions. Mirrors `DeployStageH.t.sol` L61 / L132. Cursor §8e.1.
