...
> **Status:** Code phase open — I9 (AuMT deprecation + IVotingWeight migration) COMPLETE (I9.1—I9.5c). Per OQ-25 / I-D14, AuMT is the pool's own Balancer V3 BPT, not a separate token; Stage I is the oracle-free **recorder clock + hook dispatch** only. Decisions I-D1—I-D17 LOCKED: I-D14 (AuMT = BPT), I-D15 (value-weighted view deferred to a Stage K `VotingWeight.sol`), I-D16 (hook one-shot `emissionRecorder` slot), I-D17 (`IVotingWeight` forward stub + `VaultClassRegistry` full-rename fix-forward); I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED + I-D5—I-D9 AMENDED by I-D14. I3 (concrete `AuMT.sol`) TOMBSTONED — superseded, removed at I9.4. I1 (I-D9), I2 (I-D10), I8 (I-D13) complete pre-reframe, unchanged. Findings I10—I15. Vault bytecode untouched. Next: I4 (recorder dispatch + clock) → I5/I6 (recorder-clock unit + fork tests) → I7 (deploy + close-of-stage). Companion to STAGE_I_NOTES.md.
>
> **Last update:** 2026-06-04 — I9 family (AuMT deprecation + IVotingWeight migration) COMPLETE: I9.1 `src/governance/IVotingWeight.sol` (`16e1d5c`) → I9.2 `VaultClassRegistry.sol` full rename (`77a9164`) → I9.3a/b/c mock + StageGIntegration consumer + cosmetic rename (`25c4793` / `ad9783e` / `476e77e`) → I9.4 `git rm src/token/AuMT.sol src/token/IAuMT.sol` (`0b5e572`; 619/620 unit + 71/71 fork green) → I9.5a/b/c sweep (`6f3fa30` / `8d84adb` / `825f94a`). I9.5d status/mode headers + I9.5c self-row hash backfill this commit; I9.5e CLAUDE §11 refresh pending. Code phase continues at I4 (recorder dispatch + clock).
>
> **Mode:** Opus high per §13 stage-level defaults. I9 (AuMT deprecation + IVotingWeight migration) COMPLETE. Code phase continues: I4 (recorder dispatch + clock — `emissionRecorder` one-shot slot per I-D16 + `effectiveQualBlock` clock per I-D14 + `onAfterAddLiquidity` / `onAfterRemoveLiquidity` → `recordDeposit` / `recordWithdrawal`) → I5/I6 (recorder-clock unit + fork tests) → I7 (deploy + close-of-stage). Entry mode Opus high (live design on the hook callback bodies + EmissionDistributor clock math).
>
