...
> **Status:** Code phase open — I9 (AuMT deprecation + IVotingWeight migration) COMPLETE (I9.1—I9.5f) + I4 (recorder dispatch + clock) + I5 (recorder-clock unit tests) COMPLETE. Per OQ-25 / I-D14, AuMT is the pool's own Balancer V3 BPT, not a separate token; Stage I is the oracle-free **recorder clock + hook dispatch** only. Decisions I-D1—I-D17 LOCKED: I-D14 (AuMT = BPT), I-D15 (value-weighted view deferred to a Stage K `VotingWeight.sol`), I-D16 (hook one-shot `emissionRecorder` slot), I-D17 (`IVotingWeight` forward stub + `VaultClassRegistry` full-rename fix-forward); I-D1—I-D4 / I-D11 / I-D12 SUPERSEDED + I-D5—I-D9 AMENDED by I-D14. I3 (concrete `AuMT.sol`) TOMBSTONED — superseded, removed at I9.4. I1 (I-D9), I2 (I-D10), I8 (I-D13) complete pre-reframe, unchanged. Findings I10—I17. Vault bytecode untouched. Next: I6 (fork integration) → I7 (deploy + close-of-stage). Companion to STAGE_I_NOTES.md.
>
> **Last update:** 2026-06-05 — I5 family (recorder-clock unit tests `test/unit/RecorderClock.t.sol`) COMPLETE: I5.1a scaffold + 6-mock harness reuse from EmissionDistributor.t.sol (`766566a`) → I5.1b first-deposit fresh-start (`198a935`) → I5.2 weighted-average top-up — 100e18 @ G + 300e18 @ G+4000 blends to G+3000 (`9db3caa`) → I5.3a partial-withdrawal reset (`370df5b`) → I5.3b over-qualification vector — re-deposit fresh-starts, not blends (`1c9d638`) → I5.4 hook-gate `NotAuMTContract` (`739a329`); targeted 5/5 RecorderClock green (full unit count now 625, fork 71 — full regression at I6 per D35/D36 split-form). I5.5a PLAN Completion Log catch-up (`c147662`). I5.5b status/mode refresh this commit; I5.5c CLAUDE §11 pending. Code phase continues at I6 (recorder-clock fork integration).
>
> **Mode:** Opus high per §13 stage-level defaults. I5 (recorder-clock unit tests) COMPLETE. Code phase continues: I6 (fork integration `test/fork/StageIIntegration.t.sol` — `StageIIntegrationFixture` inherits `StageHIntegrationFixture` per H13; real Balancer V3 add / remove-liquidity through the canonical `AureumFeeRoutingHook` → `recordDeposit` / `recordWithdrawal` on the EmissionDistributor) → I7 (deploy + close-of-stage). Entry mode Opus high for the I6 fork-fixture design (`setEmissionRecorder` + `setAuMTContractForPool` ×3 binding + real liquidity-callback dispatch), dropping to Sonnet for repetitive assertion bodies per §13.
>
