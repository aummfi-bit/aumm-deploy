...
| 4   | The Vault's admin surface area (`pauseVault`, `enableRecoveryMode`, `setProtocolFeeController`, `setStaticSwapFeePercentage`, etc.) is gated by `AureumAuthorizer.sol`. The authorizer grants permissions to **a single Safe multisig address**. The multisig itself only acts on passed governance proposals (Snapshot to Safe transaction). On-chain, the authorizer just sees one privileged address. **Historical note (F-22 / PB-D63 (vii)):** this describes the design through Stage K; the Stage-K handoff moved the privileged principal to `AureumGovernanceAuthorizer`, and PB-D64 subsequently restored `AureumGovernance` proposal-level expressibility for `setAuthorizer`, `unpauseVault`, and `disableRecoveryMode` specifically — see CLAUDE.md §11 for current state.                                                                                                                          | Matches "no treasury, no wallet" in spirit while preserving the ability to pause/recover for the Balancer-default 4-year window. After the pause window expires, even the multisig cannot pause.                                                                                                                                                                        |
