...
Read PB-D36, PB-D37 and PB-D38 in `docs/STAGE_P_BIS_NOTES.md`. PB-D34 and PB-D35 remain the background to them, with PB-D35 (iv) and (viii) superseded. Branch tip when this section was written: `2d97a62`.

**Rung i5 landed, but not as the model this section used to describe.** PB-D35's two-file overlay was withdrawn mid-rung: an overlay is correct only for the keys it carries, and any key it omits silently inherits the other chain's value. PB-D36 replaced it with a three-file copy-and-refresh model — `.env.mainnet` and `.env.sepolia` are complete, independently-maintained canonical sets, `.env` is a derived working copy of exactly one of them, and an `AUMM_ENV_CHAIN` marker inside that copy says which. Two mechanisms this section used to prescribe went out with the overlay. The shell source preamble on each broadcast command is unimplemented at any rung and is not coming: `.env` is auto-loaded, so there is nothing left to forget to source (PB-D36 (v)). The stub-prefix pre-run count is WITHDRAWN as unsound: it returns zero against a `.env` holding nine stale Sepolia addresses, none of which carry that prefix, so it passed on precisely the contamination it existed to catch. Its replacement is an exact-match count on the chain marker, failing CLOSED on absence (PB-D36 (iv)).

**i6, i7 and i8 then landed as written.** i6 is the permanent deployed-size gate at `test/unit/BytecodeSize.t.sol`, 4/4 green — vault artifacts asserted by explicit `out-vault/` path, Aureum contracts by full source path, with a `vm.readDir` completeness check so a contract added later fails the gate rather than escaping the roster; RB-009 and RB-010 carry the general traps. i7 corrected CLAUDE.md section 2's compiler row, advanced section 11 through i7, landed the PB15 lesson, and cleared the same withdrawn parity claim from `foundry.toml`'s own header comment. i8 re-ran phase A from the live nonce `n0 = 90` on the PB-D38 resume branch, which drops `c1` from the arithmetic because the 87 stubs are reused rather than redeployed; its output is section 4's canonical table.

**What remains of i8 is its record leg.** `docs/STAGE_P_BIS_PHASE_A_RECORD.md` is untouched since rung h and still documents only the first phase A — `n0 = 0`, `c1 = 87` as an addend, and the projections section 4 now marks void. PB-D38 (vii) scopes that file to i8d alongside this one. Then **i9**: the broadcast resumes, owning both the `GENESIS_BLOCK` refresh to Sepolia head plus 100,800 per PB-D19 and the Etherscan API key, still absent.

**Funding is no longer the gate, and the 0.05 SepETH this section used to cite was never the live figure.** That was the PB3.1 faucet grant, carried into prose while section 2's on-chain read already stood at roughly 5.94 SepETH — comfortably past the runbook's 1—2 SepETH funding target, and this resume does not re-broadcast step 1, so the 87 stub deploys are already paid. CLAUDE.md section 11 still carries the stale figure; section 2 is the one to trust. What has not relaxed is section 3: the deployer EOA sends nothing outside this sequence, or all seven projections void.
