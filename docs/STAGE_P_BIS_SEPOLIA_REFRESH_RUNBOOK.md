...
**The pre-phase-4 gate, per PB-D70 (xx): all twenty-six keys present, and each DIFFERING from its generation-1 value.** `DeployStageP` drives Stage I, M and N inside its own process and those read the keys directly. This paragraph's previous form said a single missing key aborts the orchestrator partway through a sequence that has already broadcast; that was wrong on both halves and is corrected here per PB-D70 (xx). An ABSENT key fails closed at `vm.envAddress` during phase 4's pre-broadcast simulation, before anything is sent. A PRESENT stale key is the real hazard: the orchestrator's seventy-eight pool writes touch no Vault, its post-condition loop reads `isGaugeApproved` and `auMTContractByPool` back from the very registries the run just wrote, and the run completes with a generation-1 pool gauged, slotted and bound — repairable only through governance that F-21's zero-supply quorum guard holds shut for the 432,000-block EMA maturity. The gate is therefore a source-diff, run before the phase-4 command: every one of the twenty-six values in `.env.sepolia` must differ from that slot's generation-1 address in `docs/STAGE_P_BIS_SEPOLIA_DEPLOYMENT_RECORD.md` — the twenty-five rows of section 7 and slot 02's section 10 row — checked against the committed table per §8e.1's source-diff rule, never against a transcription. A10's whole-record zero-hit form does not transfer: by this point the record's generation-2 sections carry the correct values by construction, and a zero-hit expectation would fail on every correct capture. What the per-slot comparison cannot catch, a transposition between two generation-2 addresses, the capture procedure itself closes — each key diffed against forge's own per-script `broadcast/` artifact, which §8f requires for the record rows regardless.
