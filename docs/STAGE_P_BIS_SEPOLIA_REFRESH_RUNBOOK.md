...
**A2. Confirm the stubs, do not count them.** Step 1 is not run, so there is nothing to simulate and no `c1` to record. Confirm instead that the reused map is real: every address in the `STUB_` block of `.env.sepolia` must have code on chain, and each of the 14 `StubERC4626` vaults must return its recorded underlying from `asset()`. Both were verified at PB3.13e and recorded at PB-D70 (xvi); re-run them if material time has passed. A codeless address here is a hard stop rather than a note, the stub roster surviving being the only thing that makes this a refresh instead of a from-zero deployment.

**A3. Project the vault step.** `cast compute-address 0xA851478dbee97375E784e9b98c0D7D599662bF85 --nonce <n0>` is the authorizer; `--nonce <n0 + 1>` is the fee controller, which is `FEE_CONTROLLER`; `--nonce <n0 + 2>` is the vault factory. The Vault itself is a CREATE3 from that factory using the RAW `SALT` value — not the sender-hashed form — per the derivation in the closing subsection. Write it to both `VAULT` and `AUREUM_VAULT`.
